 ...
 * 저장소 전체에서 사본을 세는 술어 (2026-08-30 실측 `3` hit, 위 3개와 일치):
 *
 *   grep -rn "cosI \* y1" --include="*.ts" packages apps | grep -v "/dist/" | grep -v node_modules
 *
 ...
 *    나머지 2개는 **진근점각 단조 증가** 3점의 연속 외적 합. 시간 등간격 샘플링은 winding 이
 *    뒤집혀 deimos 에서 `176.4°` 오검출이 재현됐다 — 코드 결함이 아니라 **측정 도구 결함**이었다.
 *
 ...
 *
 *  - **`inclinationDeg` 값 자체의 IAU 정확성** — 본 계약은 **코드 라운드트립**이다.
 *    데이터가 틀려도 PASS 한다. (`verify:iau-data` 는 `mass`·`radius` 2필드만 대조한다.)
 *  - **`ω`/`Ω` 정확성** — 본 불변식은 경사만 구속한다 (D3 의 M5 가 D1 을 통과하는 이유).
 *  - **자전축** — #1130 에서 완결.
 *
 * ## 파일 구성 근거
 *
...
interface RotationCopy {
  ...
  /** D1 법선. `orbitalStateAt` 만 `h = r × v`, 나머지는 3점 외적. */
  planeNormal: (
    body: LoadedCelestialBody,
    points: readonly [SampledPoint, SampledPoint, SampledPoint],
  ) => Vec3;
}
...
const COPIES: readonly RotationCopy[] = [
  ...
  },
  {
    ...
    planeNormal: (_body, points) =>
      planeNormalFromThreePoints([points[0].position, points[1].position, points[2].position]),
  },
  {
  ...
  describe(`#1132 D1 궤도면 법선 경사 == |inclinationDeg| — ${copy.label}`, () => {
    for (const { id, inclinationDeg } of ORBITING_BODIES) {
      it(`${id} (i = ${inclinationDeg}°)`, () => {
        ...
        const measured = normalTiltDeg(copy.planeNormal(body, copy.sample(body)));
