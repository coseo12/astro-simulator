 ...
 *    쓰면 **`178.209000°`** 가 나온다 (참값 `1.791000°`). 발현 조건은 `f := (Δt / T) mod 1`
 *    이 `(0.5, 1)` — 같은 술어로 `Δt = 0.25 / 0.5 / 1.5 / 3 d` 는 `1.791000°` 로 정상이고
 *    `Δt = 1 / 1.25 / 2 / 5 / 7 d` 가 `178.209000°` 다 (측정한 `Δt` 9종). 경계 실측:
 *    `f = 0.4989 → 1.791000°` / `f = 0.5004 → 178.209000°`.
 *    ⚠️ **`f = 0` 은 조건식이 답하지 못하는 두 대역으로 갈린다** (reviewer B4 정정):
 *      · `Δt = 0` — 세 점이 같은 점이라 `|n| = 0`, `atan2(0, 0) = 0` → `0.000000°`.
 *        `Δt = 1e-12 · T` 도 `|n| = 0` 으로 같다.
 *      · `Δt = k · T` (`k ≥ 1`, `(k·T)/T mod 1` 은 배정도에서도 정확히 `0`) — **퇴화가 아니라
 *        잡음 대역**이다. `|n|` 이 `3.627e+4`~`4.735e+5` 로 작게 남아 결과가 갈린다:
 *        `1T → 178.209000°` / `2T → 1.791000°` / `3T → 178.208999°` / `4T → 178.209000°` /
 *        `5T → 1.791000°`. **`Δt = T` 를 고르면 경고 대상인 `178.209°` 를 그대로 맞는다.**
 *    (초판이 술어 없이 인용한 `176.4°` 는 위 스윕 어디서도 재현되지 않아 폐기한다
 *    — PR #1174 reviewer W2. 오검출 값이 `180 − i` 라는 것은 위 실측이 그대로 보인다.)
 *
 ...
 *    반환값 `× 2` / 로더 `semiMajorAxis` `× 2` / 로더 `eccentricity → 0`.
 *    ⚠️ **기전 설명은 반증돼 뺐다** (PR #1174 B3). 무구속 여부는
 *    설명에서 연역하지 말고 **실변이를 한 번 돌려** 확인할 것.
 *  - **자전축** — #1130 에서 완결.
 *
...
interface RotationCopy {
   ...
   * 관측 동등이다 — 법선 **크기**는 사본마다 다르지만 판정에 들어가지 않는다. 얻은 것은
   * **현재 코드에 인덱스 표현이 `0` 개**라는 것뿐이다. 선택 필드는 남아 있으므로 되살리려면
   * 없는 필드를 새로 선언해야 한다 — 위 재도입 실측이 정확히 그 경로다.
   */
