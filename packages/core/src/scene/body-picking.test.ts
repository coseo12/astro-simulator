...
// #849 — picking 표면 mock (mesh/scene/pickInfo) 은 공용 헬퍼로 추출. 렌더 크기는 helper SSoT.
import {
  MOCK_PICKING_SCENE_HEIGHT,
  MOCK_PICKING_SCENE_WIDTH,
  mockPickMesh,
  mockPickingScene,
  pickInfo,
  type MockPickMesh,
...
const W = MOCK_PICKING_SCENE_WIDTH;
const H = MOCK_PICKING_SCENE_HEIGHT;
...
describe('#713 resolvePickedBodyId — 1차 scene.pick (occlusion 최전면)', () => {
  it('predicate 통과 mesh 직격 시 metadata.bodyId 반환', () => {
    const earth = mockPickMesh({ bodyId: 'earth' });
    const scene = mockPickingScene({ pickResult: { hit: true, pickedMesh: earth } });
  ...
  it('pick hit 이지만 pickedMesh 에 metadata.bodyId 없으면 1차 무효 → fallback (없으면 null)', () => {
    const bg = mockPickMesh({ noMetadata: true });
    const scene = mockPickingScene({ pickResult: { hit: true, pickedMesh: bg } });
    const id = resolvePickedBodyId(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(id).toBeNull();
...
describe('#713 predicate — 비-allowlist / 숨은 variant 제외', () => {
  ...
  function sceneWithPredicatePick(candidates: MockPickMesh[]) {
    return {
  ...
  it('isFocusable=false (비-allowlist) body 는 predicate 에서 탈락 → no-op', () => {
    const fake = mockPickMesh({ bodyId: 'nonexistent-body' });
  ...
  it('숨은 variant (isVisible=false) 는 predicate 에서 탈락', () => {
    const hidden = mockPickMesh({ bodyId: 'mars', isVisible: false });
  ...
  it('비활성 variant (isEnabled=false) 는 predicate 에서 탈락', () => {
    const disabled = mockPickMesh({ bodyId: 'mars', isEnabled: false });
...
describe('#713 화면거리 fallback — 임계 내 최근접 1개', () => {
  it('1차 miss → 클릭 좌표 임계 내 body 선택', () => {
    ...
    const io = mockPickMesh({ bodyId: 'io', world: worldForScreen(500, 500) });
    const scene = mockPickingScene({ pickResult: { hit: false }, meshes: [io] });
    const id = resolvePickedBodyId(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(id).toBe('io');
  ...
  it('임계 내 복수 body → 화면거리 더 가까운 1개 선택', () => {
    const near = mockPickMesh({ bodyId: 'io', world: worldForScreen(502, 500) }); // 2px
    const far = mockPickMesh({ bodyId: 'europa', world: worldForScreen(508, 500) }); // 8px
    const scene = mockPickingScene({ pickResult: { hit: false }, meshes: [near, far] });
    const id = resolvePickedBodyId(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(id).toBe('io');
  ...
  it('fallback 후보도 isFocusable (allowlist) 통과 body 만 — 비-allowlist 제외', () => {
    const fake = mockPickMesh({ bodyId: 'nonexistent-body', world: worldForScreen(500, 500) });
    const scene = mockPickingScene({ pickResult: { hit: false }, meshes: [fake] });
    const id = resolvePickedBodyId(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: (bid) => bid !== 'nonexistent-body',
    });
    expect(id).toBeNull();
  ...
  it('fallback 후보도 숨은 variant (isVisible=false) 제외', () => {
    const hidden = mockPickMesh({
      bodyId: 'io',
      world: worldForScreen(500, 500),
      isVisible: false,
    });
    const scene = mockPickingScene({ pickResult: { hit: false }, meshes: [hidden] });
    const id = resolvePickedBodyId(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(id).toBeNull();
  ...
  it('빈 우주 (후보 0) → null (setSelectedBody(null) 호출 안 함 — 호출자 no-op)', () => {
    const scene = mockPickingScene({ pickResult: { hit: false }, meshes: [] });
    const id = resolvePickedBodyId(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(id).toBeNull();
  ...
  it('behind-camera (projected.z 범위 밖) body 제외', () => {
    ...
    const behind = mockPickMesh({ bodyId: 'io', world: worldForScreen(500, 500, 2) });
    const scene = mockPickingScene({ pickResult: { hit: false }, meshes: [behind] });
    const id = resolvePickedBodyId(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(id).toBeNull();
...
describe('#719 resolvePickedBodyIds — multiPick 깊이순 dedup', () => {
  const A = mockPickMesh({ bodyId: 'io', isVisible: true });
  const B = mockPickMesh({ bodyId: 'jupiter', isVisible: true });
  const C = mockPickMesh({ bodyId: 'europa', isVisible: true });

  it('ray hit 0 → 빈 배열 (호출자가 단일 fallback 위임)', () => {
    const scene = mockPickingScene({ multiPickResult: [] });
  ...
  it('ray hit 1 → 단일 [id] (#713 단일 클릭 동작 불변)', () => {
    const scene = mockPickingScene({ multiPickResult: [pickInfo(10, B)] });
    const ids = resolvePickedBodyIds(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
  ...
  it('같은 bodyId 의 LOD variant 동시 hit → dedup 후 1개 (첫 등장=최근접 유지)', () => {
    ...
    const ioMid = mockPickMesh({ bodyId: 'io', isVisible: true });
    const ioLow = mockPickMesh({ bodyId: 'io', isVisible: true });
    const scene = mockPickingScene({
  ...
  it('agy ④ — LOD fade 구간 distance 흔들림에도 dedup 후 개수·순서 안정', () => {
    ...
    const ioV1 = mockPickMesh({ bodyId: 'io', isVisible: true });
    const ioV2 = mockPickMesh({ bodyId: 'io', isVisible: true });
    const frame1 = mockPickingScene({
      multiPickResult: [pickInfo(41, ioV1), pickInfo(42, ioV2), pickInfo(50, B)],
    });
    const frame2 = mockPickingScene({
  ...
  it('predicate — 비-allowlist body 제외 (isFocusable=false)', () => {
    const fake = mockPickMesh({ bodyId: 'nonexistent-body', isVisible: true });
    const scene = mockPickingScene({ multiPickResult: [pickInfo(40, fake), pickInfo(45, B)] });
    const ids = resolvePickedBodyIds(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: (bid) => bid !== 'nonexistent-body',
    });
    expect(ids).toEqual(['jupiter']);
  ...
  it('predicate — 숨은 variant (isVisible=false) 제외', () => {
    const hidden = mockPickMesh({ bodyId: 'io', isVisible: false });
    const scene = mockPickingScene({ multiPickResult: [pickInfo(40, hidden), pickInfo(45, B)] });
    const ids = resolvePickedBodyIds(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(ids).toEqual(['jupiter']);
  ...
  it('predicate — metadata.bodyId 없는 mesh 제외', () => {
    const bg = mockPickMesh({ noMetadata: true });
    const scene = mockPickingScene({ multiPickResult: [pickInfo(30, bg), pickInfo(45, B)] });
    const ids = resolvePickedBodyIds(asScene(scene), asCamera(camera), 500, 500, {
      isFocusable: allowAll,
    });
    expect(ids).toEqual(['jupiter']);
