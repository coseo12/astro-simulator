...
// #849 — mockCamera 는 공용 헬퍼로 추출 (중복 3곳 통합).
import { mockArcRotateCamera } from './__test-utils__/babylon-mocks.js';
...
describe('#693 free-fly 패닝 감도 산출식', () => {
  ...
  it('computePanningSensibility — 유한 양수 반환 (정상 radius)', () => {
    const s = computePanningSensibility(mockArcRotateCamera({ radius: 35 }));
  ...
  it('radius 비례 — radius 2배 → sensibility 절반 (화면 px↔world 일정 비율)', () => {
    ...
    const sLow = computePanningSensibility(mockArcRotateCamera({ radius: 35 }));
    const sHigh = computePanningSensibility(mockArcRotateCamera({ radius: 70 }));
  ...
  it('tier 무관 일정 비율 — solar(r≈35) vs deep(r≈158386) 의 sensibility×radius 곱 일정', () => {
    ...
    const solar = mockArcRotateCamera({ radius: 35 });
    const deep = mockArcRotateCamera({ radius: 158386 });
...
describe('#693 zero-division 가드 (agy 고유 발견 ①)', () => {
  it('radius=0 → Infinity/NaN 발산 아닌 유한 양수 (lowerRadiusLimit 하한)', () => {
    const s = computePanningSensibility(mockArcRotateCamera({ radius: 0, lowerRadiusLimit: 0.5 }));
    expect(Number.isFinite(s)).toBe(true);
    expect(s).toBeGreaterThan(0);
  });

  it('radius=NaN → 유한 양수 (NaN 발산 차단)', () => {
    const s = computePanningSensibility(
      mockArcRotateCamera({ radius: Number.NaN, lowerRadiusLimit: 0.5 }),
    );
    expect(Number.isFinite(s)).toBe(true);
    expect(s).toBeGreaterThan(0);
  });
  ...
  it('radius=-Infinity → 유한 양수 (음/무한 방어)', () => {
    const s = computePanningSensibility(
      mockArcRotateCamera({ radius: Number.NEGATIVE_INFINITY, lowerRadiusLimit: 0.5 }),
    );
    expect(Number.isFinite(s)).toBe(true);
    expect(s).toBeGreaterThan(0);
  });
  ...
  it('lowerRadiusLimit=0 + radius=0 → EPSILON 하한으로 발산 차단 (이중 가드)', () => {
    const s = computePanningSensibility(mockArcRotateCamera({ radius: 0, lowerRadiusLimit: 0 }));
    expect(Number.isFinite(s)).toBe(true);
    expect(s).toBeGreaterThan(0);
  });
});
