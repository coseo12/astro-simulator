 ...
 *
 * ⚠️ **한정** — `continents = fbm(p * 2.4)` 의 최고 옥타브 파장은 약 `32°` 이고 마스크 텍셀은
 * `0.352°` 다. 즉 본 워프는 **텍셀 수십 개 범위에서 서서히 변하는 변위**이지 텍셀 이하 고주파가
 * 아니다. 해안선 텍셀 계단이 실사용에서 인지되면 §A4.7 재검토 조건 2·3 (`textureGrad` 승격 /
 * 해상도 승격) 으로 다루고, **여기서 fbm 을 새로 호출하지 않는다** (결정 5 위반).
 */
