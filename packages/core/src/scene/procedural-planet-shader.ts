 ...
 * rim 이 sub-pixel 이다.
 *
 * **이 근거를 지키는 게이트는 `verify:1202-atmosphere-rim` 의 G6 (박명 호 rim 기여 ≥ `0.02`) 다.**
 * 단위 테스트 U18(음수)은 필요조건만 고정한다 — `LO = -0.05` 도 음수지만 박명 호를 실측 4.2%
 * (`0.00254` / baseline `0.06086`) 만 남긴다. G1~G4 는 이 상수에 **전부 눈이 멀다** [실측 스윕]:
 * `LO ∈ {-0.25 … +0.05}` 전 구간에서 낮면 림 기여가 `0.07499` 로 불변이다. 값 변경 시 G6 재측정 의무.
 */
