...
- **[#795] 반복 운영 마찰 원인 박제 + pgrep self-match bracket fix** ([#795](https://github.com/coseo12/astro-simulator/issues/795)) — 매 세션·릴리스 반복되던 저비용 마찰 4건(squash auto-close 미발동 / `--delete-branch` worktree 충돌 / pgrep self-match 오탐 / concurrency CANCELLED 코스메틱)의 **구조 원인 + 표준 절차**를 [`docs/lessons/operational-friction.md`](docs/lessons/operational-friction.md) 에 박제 + CLAUDE.md 프로젝트 고유 보강 포인터. **pgrep/pkill self-match bracket fix**: `.claude/agents/qa.md` + CLAUDE.md 의 agent-browser 정리 명령을 `agent-browser-chrome-` → `agent-browser-chrome[-]` 로 정정(pkill 자기 셸 kill 위험 차단 = 안전 개선). hook `session-start-zombie-check.sh` 는 `grep -v` 로 이미 안전(변경 없음). agent-browser 정리 절차의 upstream 기여(Z-패턴 Phase 2)는 [#830](https://github.com/coseo12/astro-simulator/issues/830) 후속 분리(upstream 미보유 drift). qa.md 는 harness-managed 이나 기존 HARNESS-DRIFT 데코레이터(#309)가 drift 커버, CLAUDE.md 변경은 프로젝트 고유 보강 섹션(managed-block 외부).
