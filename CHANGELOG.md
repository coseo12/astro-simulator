...
### Behavior Changes (CI 진단 — r1-guard step 4분리 + verify 진단 로깅)

- **[#606] r1-guard freeze 지점 격리 진단 — ci.yml step 4분리 (install/build/start/verify) + verify 경로 stderr 진단 로깅 + ADR 가설 5 오진 발견 (MINOR — CI workflow 구조 변경)** ([#606](https://github.com/coseo12/astro-simulator/issues/606)) — ADR `20260531-606-r1-guard-playwright-freeze-forensic.md` §5 §Fix 후 박제 의무 §옵션 (f) (workflow step 분리) 의 진단 우선 변형. **measurement-first 실측으로 ADR 가설 5 오진 발견**: ADR §1 가설 5 ("R5 추가 satellite body 8 body 측정 freeze") 가 지목한 `measureBodyPxRatios`/`targetIds` 8 body 확장은 **`--measure-px-ratio` 모드 전용** — CI ci.yml:110 은 플래그 없이 호출 = **verify 모드 (`runForViewport`)** 이라 `measureBodyPxRatios` 를 절대 호출하지 않음 (dead code). 따라서 옵션 (a) (targetIds 5 body revert) / 옵션 (c) (`projectWorldToScreen` NDC guard) 는 verify 경로 밖이라 **CI 효과 0** 확정. 또한 ADR §1 의 "`pnpm build` success 후 다음 r1-guard step stuck" 기술도 부정확 — 실제 ci.yml:103 에서 `pnpm build` 는 r1-guard step **내부** (단일 step) 라 build/start/verify 어디서 stuck 인지 격리 불가. **로컬 실측 (macOS, develop tip R5 포함)**: `pnpm build` 정상 (산출물 존재) + `next start` Ready 159ms + r1-guard verify **8초 정상 종료** (freeze 0, FAIL 은 폰트 diff) → **freeze 는 Linux headless Chromium 특정** 현상 (가설 1 swiftshader + 가설 5 satellite 렌더링 결합 후보, 옵션 d 영역). **본 PR 변경**: (1) `.github/workflows/ci.yml` — r1-guard 단일 step (timeout-minutes:10) 을 4 step 분리: `Playwright 설치` (timeout 5분) / `production build` (timeout 8분) / `next start 기동 + readiness` (timeout 3분, setsid 로 다음 step 까지 유지) / `verify 실행` (timeout 5분, R1_GUARD_DIAG=1) → GitHub Actions UI 가 step 별 소요시간 + timeout 위치를 명확히 표시 → freeze 지점 step 단위 격리 (buffering 무관) (2) `apps/web/scripts/r1-ui-regression-guard.mjs` — verify 경로 (main launch / runForViewport / setupPage goto·waitForFunction·waitForTimeout / measureSunCoverage / frameState / region loop) 에 `R1_GUARD_DIAG=1` 시 stderr 단계별 타임스탬프 로깅 (`diag()` 헬퍼). stderr 는 unbuffered(동기) 라 step timeout cancel 시에도 stuck 직전 단계 보존 (stdout console.log 는 CI pipe block-buffered → 유실, 실측 #606 CI run 26717093983 에서 playwright 다운로드 100% 후 timeout 까지 stdout 0줄). **DoD**: (1) ci.yml 4 step 분리 ✅ (2) verify stderr 진단 로깅 ✅ (3) 로컬 verify 회귀 0 (8초 종료, 진단 로깅 전 단계 출력 확인) ✅ (4) CI run 으로 freeze 지점 격리 → 머지 후 측정. **머지 후 후속**: CI 측정 결과로 freeze step/단계 확정 → ADR Amendment 2 박제 (가설 5 오진 정정 + 가설 1/root cause 재정립) + root cause fix 후속 PR (옵션 d 영역 유력). **MINOR 분류** — CI workflow gate 구조 변경 (step 분리 + 진단 로깅 활성화 env). 비-범위: root cause fix (옵션 a/c/d 구현) / ADR §결정 본문 변경 (CI 측정 후 Amendment 2) / R5 박제값 변경 / chromium launch args 변경 (현 상태 유지로 freeze 재현 필요) 모두 변경 0.
