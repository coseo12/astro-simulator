...
- **R-Phase Body Allowlist 가드 도입 — defense-in-depth UI + Scene 가드 (#402)** ([#402](https://github.com/coseo12/astro-simulator/issues/402)) — `R_PHASE_BODY_ALLOWLIST = ['sun', 'mercury', 'venus']` SSoT 박제 (`packages/core/src/scene/r-phase-allowlist.ts`). FocusQuickButtons 에서 R-Phase 미활성 body (earth/jupiter/neptune) 가 HTML `disabled` + `aria-disabled="true"` + `data-r-phase-disabled="true"` + opacity 0.5 + cursor:not-allowed + tooltip (한국어 안내) 박제 — UI 1차 방어선. `simulation-core.ts` `case 'focusOn'` 가 `isRPhaseFocusable(cmd.bodyId)` 검증 → 미활성 body 입력 시 `console.warn` + `bodySelected` event emit 차단 → store/scene/UI 자동 0 변화 (URL `?focus=earth` 직접 진입 등 UI 가드 우회 case 의 마지막 방어선). R-Phase 진입 시 4곳 동시 박제 의무 (ADR §결정 4): allowlist + R-Phase ADR cross-link + 회귀 가드 expected list + CHANGELOG. **R4 (지구) 진입 시 'earth' 추가**, R6 (목성) → 'jupiter', R10 (해왕성) → 'neptune'. 사용자 D-T2 보고 (PR #399 / #401 머지 후 "수성/금성은 개선됨, 미구현 행성들은 이전 잔재" — 2026-05-03) 의 v3 reset 정책 정합성 누락 systemic 문제 Top 1 해결. ADR [`docs/decisions/20260504-r-phase-allowlist-guard.md`](docs/decisions/20260504-r-phase-allowlist-guard.md). 후속 #403 (Top 2 CelestialTree+InfoPanel) / #404 (Top 3 ScenarioPresets) / #405 (폐기 코드) 분리.
- **`apps/web/scripts/browser-verify-r-phase-allowlist.mjs` 신설 — UI/Scene 가드 정합성 회귀 가드** ([#402](https://github.com/coseo12/astro-simulator/issues/402)) — UI 가드 (DoD-2): shortcut bar earth/jupiter/neptune 의 `disabled` + `aria-disabled="true"` + `data-r-phase-disabled="true"` + `opacity-50` className 박제 검증. Scene 가드 (DoD-3): URL `?focus=earth` 등 R-Phase 미활성 직접 진입 시 `selectedBodyId` 변화 0 검증 (UI 가드 우회 마지막 방어선). CLI: `pnpm --filter @astro-simulator/web verify:r-phase-allowlist`. **CI `detect-and-test` job 통합** — `.github/workflows/ci.yml` 에 `next dev -p 3003` 기동 후 검증 step 추가. R-Phase 진입 시 본 스크립트의 `EXPECTED_ENABLED` / `EXPECTED_DISABLED` 갱신 의무 (ADR §결정 4 4곳 박제 절차).
- **기존 회귀 가드 3종 R-Phase allowlist 정합 갱신 — `verify:378-focus` / `verify:379-lod` / `verify:397-residual` SSoT 단일 import 리팩토링 (#402 보강)** ([#402](https://github.com/coseo12/astro-simulator/issues/402)) — qa #407 라운드 1 BLOCK 4건 (earth/jupiter/neptune focus cell DoD-3 FAIL + 379-lod 시나리오 B `focusTargets=['earth','mars']` 차단 + 397-residual unexpected residual) 해소. 3 verify 스크립트가 `R_PHASE_BODY_ALLOWLIST` 를 `@astro-simulator/core/scene` 에서 import — `verify:378-focus` 의 `FOCUS_BODIES` (12 cells → 6 cells), `verify:379-lod` 시나리오 B 의 `focusTargets` (earth/mars → mercury/venus, sun 제외), `verify:397-residual` 의 `FOCUS_BODIES` (12 cells → 6 cells) 모두 자동 도출. **R4 진입 시 `R_PHASE_BODY_ALLOWLIST` 1줄 변경만으로 378/379B/397 매트릭스 자동 확장** (수동 갱신 0 건). ADR `20260504-r-phase-allowlist-guard.md` §결정 4 에 **회귀 가드 갱신 매트릭스** (7 박제 위치 + 자동/수동 구분 표) 추가. 헤더 주석 — 3 verify 파일 모두 "R-Phase 진입 시 본 SSoT 자동 확장" 박제. volt #69 "숨은 상수 변형" 패턴 강화 (단일 SSoT 가 grep 만으로 drift 검출 가능).
