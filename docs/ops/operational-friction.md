...
"
# stderr: line 2: 2: command not found / line 2: 0: command not found
# exit  : 0
...
**공백 2개**를 남기므로 잔여 자체는 greppable 하다 (실측: 위 재현의 `out.md` 에 이중 공백 `2` hit).
...
| prettier **소유** (`CHANGELOG.md` 등) | **무효** — 포맷터가 지운다 | **포맷 전** 검사 + 의도본 대비 diff |
...
**2026-08-22 하루에 백틱 취급 사고가 `4`회** 났고 내역이 두 층에 걸쳐 있다 (릴리스 귀속은 서로 다르다):
