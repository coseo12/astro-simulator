...
⚠️ **마지막 행이 가장 위험하고 가장 무해해 보인다.** 위 4행의 분류축은 *stdout 유무*이지 위험도가
아니다 — **stdout 이 없는 유효 명령**은 본문상 「그냥 토큰이 사라진 것」과 **육안으로 구분되지 않는데
부작용만 남는다**. `` `rmdir zz` `` 는 같은 형태로 **실제 삭제**한다.

⇒ **유효 명령이면 stderr 신호조차 없다.** 그리고 이것은 사변적 케이스가 아니다 — 실측(`docs/**` tracked
md, 펜스 밖 인라인 스팬, 첫 토큰이 `PATH` 에 실재하면 명령으로 계수): 스팬 `24,881` 중 **실행 가능한
것이 `1,462`개**이고 그중 **파괴적 패턴이 `7`종**(`rm -rf` · `rm -f` · `git reset --hard` · `git clean -fd` ·
`pkill` · `kill -9` · `--delete-branch`)이다.

⚠️ **비율로 적지 않는다** — 실행 가능 스팬은 전체의 `5.9%` 이고 *"대부분"* 이 아니다. 논지에 «대부분» 이
필요하지도 않다: **한 개면 족하다.** 문서를 셸 인자로 넘기는 순간 **그 문서에 적힌 명령이 실행되는 면**이
열리고, 규약(Edit / quoted heredoc)은 소실과 실행을 **양쪽 다** 막는다.
...
⇒ **세 축이 다 무효이므로 남는 계약은 「의도본 대비 diff」 하나**다 (규약 첫 문단). ⚠️ **그 diff 도
본문 손상만 잡고 부작용은 알려주지 않는다** — 위 표 마지막 행처럼 실행이 일어났다면 **손상을 고치는
것으로 끝나지 않고 그 명령이 무엇을 했는지까지 점검**해야 한다.
