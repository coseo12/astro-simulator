...
| 스팬 내용 | 결과 | exit | stderr |
| --- | --- | ---: | ---: |
| `` `2` `` (없는 명령) | `앞  뒤` — 소실 | `0` | `1`줄 |
| `` `true` `` | `앞  뒤` — 소실 | `0` | **`0`줄** |
| `` `pwd` `` | `앞 /private/…/x 뒤` — **출력 주입** | `0` | **`0`줄** |
| `` `echo HI` `` | `앞 HI 뒤` — **출력 주입** | `0` | **`0`줄** |

⇒ **유효 명령이면 stderr 신호조차 없다.** 그리고 이 저장소 문서의 인라인 스팬은 대부분 **실행 가능한
명령**(`pnpm test` · `git status` · `gh pr view` …)이라 사변적 케이스가 아니다 — 문서를 셸 인자로
넘기는 순간 **그 문서에 적힌 명령이 실행되는 면**이 열린다. 규약(Edit / quoted heredoc)은 소실과
실행을 **양쪽 다** 막는다.
