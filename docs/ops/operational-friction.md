...
- `--write` 후에는 **의도 밖 파일이 안 바뀌었는지 `git diff --stat` 로 확인**하고, 특히 마크다운
  ...
  ⚠️ **`grep` 만으로는 닫히지 않는다** — 손상 형태에 따라 잔여가 남지 않거나 포맷터가 지운다.
  §9 의 **모집단을 갈라 쓴다** 표 참조 (#1147).
...
⚠️ **재현·검증 산출물은 스크래치 경로에만 만든다.** 저장소 루트에 쓰면 `git add -A` 가 함께 담는다 —
`.gitignore` 의 `out/` 는 디렉토리라 루트 `out.md` 를 걸러내지 못한다 (본 절 작성 중 실제로 1건
커밋됐고 별도 커밋으로 제거했다).

**증상**: `python3 -c "…"` 처럼 **쌍따옴표로 감싼 셸 인자**에 인라인 코드가 들어가면 셸이 그것을
**명령 치환으로 평가**한다. ⚠️ **「토큰이 사라진다」는 증상의 일부일 뿐이다** — 스팬 내용이 **유효한
명령이면 실행되고 그 출력이 자리에 주입**된다. zsh `5.9` 실측:

| 스팬 내용 | 본문 결과 | exit | stderr | 부작용 |
| --- | --- | ---: | ---: | --- |
| `` `2` `` (없는 명령) | `앞  뒤` — 소실 | `0` | `1`줄 | 없음 |
| `` `true` `` | `앞  뒤` — 소실 | `0` | **`0`줄** | 없음 |
| `` `pwd` `` | `앞 <cwd> 뒤` — **출력 주입** | `0` | **`0`줄** | 없음 |
| `` `echo HI` `` | `앞 HI 뒤` — **출력 주입** | `0` | **`0`줄** | 없음 |
| `` `mkdir zz` `` | `앞  뒤` — **소실과 구분 불가** | `0` | **`0`줄** | **디렉토리 실제 생성** |

⚠️ **마지막 행이 가장 위험한데 `본문 결과` 열에서는 가장 무해해 보인다** (부작용 열을 신설한 지금
표에서는 드러난다 — 열이 없던 초판이 위험했다). 앞 4행의 분류축은 *stdout 유무*이지 위험도가
아니다 — **stdout 이 없는 유효 명령**은 본문상 「그냥 토큰이 사라진 것」과 **육안으로 구분되지 않는데
부작용만 남는다**. `` `rmdir zz` `` 는 같은 형태로 **실제 삭제**한다.

⇒ **유효 명령이면 stderr 신호조차 없다.** 그리고 이것은 사변적 케이스가 아니다 — `docs/**` 의 펜스 밖
인라인 스팬에 **파괴적 패턴 `7`종이 실재한다** (각 종 최소 1개):

`rm -rf` · `rm -f` · `git reset --hard` · `git clean` · `pkill` · `kill -9` · `--delete-branch`

⚠️ **집계 수치는 싣지 않는다.** 초판은 «스팬 N 중 실행 가능 M, 비율 P» 를 적었다가 철회했다 —
「실행 가능」 판정을 **첫 토큰이 `PATH` 에 있는가**로 했는데 **`PATH` 는 저장소 불변량이 아니라 측정
기계의 상태**다 (로컬에만 있는 도구, CI 러너엔 없는 도구). 같은 술어로 메인·reviewer 가 각각 다른
값을 얻었고 **누구도 재도출할 수 없었다**. 위 `7`종은 **리터럴 매칭**이라 그 축이 없다.

⇒ **논지에 집계가 필요하지도 않다: 한 개면 족하다.** 문서를 셸 인자로 넘기는 순간 **그 문서에 적힌
명령이 실행되는 면**이 열리고, 규약(Edit / quoted heredoc)은 소실과 실행을 **양쪽 다** 막는다.
...
"
# stderr: 이 케이스(없는 명령 2개)에서만 "command not found" 2줄.
#         ⚠ 줄 수를 계약으로 삼지 않는다 — 유효 명령이면 0줄이다 (위 표).
#         형식·라인번호도 셸과 호출 형태에 의존한다:
#           bash 파일 실행  f.sh: line 2: 2: command not found
#           zsh  파일 실행  f.sh:2: command not found: 2
#           zsh  인라인     zsh:2: command not found: 2   (파일명 없음)
#         라인 번호는 «명령이 시작하는 줄» 이라 펜스 앞 주석 수에 따라 바뀐다.
#         로컬 에이전트 셸은 zsh 5.9 이고 CI 러너는 bash 다 — 축이 다르다.
# exit  : 0
...
⇒ **세 축이 다 무효이므로 남는 «검출» 계약은 「의도본 대비 diff」 하나**다 (SSoT 는 위 **규약**
문단이고 이 문단은 그 **보강**이다). ⚠️ **그 diff 도 본문 손상만 잡고 부작용은 알려주지 않는다** —
위 `` `mkdir zz` `` 행처럼 실행이 일어났다면 **손상을 고치는 것으로 끝나지 않고 그 명령이 무엇을 했는지까지
점검**해야 한다. 이 의무는 규약 문단에 없으므로 여기서 더한다.

⚠️ **`grep` 이 못 찾는 경우가 있고, 그 기전은 §7 과의 «합성» 이다.** 명령 치환은 지워진 자리에
**공백 2개**를 남기는 경우가 많아 잔여가 greppable 할 때가 있다 (실측: 위 재현의 `out.md` 에 이중 공백 `2` hit).
⚠️ **항상은 아니다** — 스팬이 앞뒤 텍스트에 **붙어 있으면 잔여가 `0`** 이다 (실측: `앞a`+스팬+`b뒤` → `앞ab뒤`).
그런데 **prettier 소유 파일에서는 prettier 가 그 이중 공백을 정규화해 잔여를 지운다** (격리 재현:
`exit  으로` → `exit 으로`). 실사례(PR [#1137](https://github.com/coseo12/astro-simulator/pull/1137))의
`CHANGELOG.md` 2곳 중 **1곳이 패턴 grep 을 빠져나가고 「의도본 대비 diff」로만** 드러난 것이 이것이다.

**모집단을 갈라 쓴다:**

| 대상 | 잔여 grep (`  +`) | 처방 |
| --- | --- | --- |
| prettier **미소유** (`docs/**` 등) | 유효 | 잔여 grep 으로 1차 탐지 가능 |
| prettier **소유** — **산문**, **포맷 이후** | **무효** — 포맷터가 정규화한다 | **포맷 전** 검사 + 의도본 대비 diff |
| prettier **소유** — **인라인 코드 스팬 안** | 유효 (실측: 스팬 안 이중 공백은 **보존**) | 잔여 grep 유효하나 위 산문 축과 병행 |

⚠️ **두 한정이 모두 필요하다** — 「prettier 소유 = 무효」가 아니라 **「산문에서, 포맷을 거친 뒤」** 무효다.
실측(prettier `3.9.6`): `- 산문  이중공백 / 스팬 ` + 백틱 스팬 안 이중 공백 → 산문만 단일로 정규화되고
**스팬 안은 그대로 남는다**.

§7 첫 불릿의 *"`grep` 한다"* 는 **필요조건이지 충분조건이 아니다** — prettier 소유 모집단에서는
포맷 이후 시점에 원리적으로 실패한다.

**§7 과의 관계 — 두 층이 있고, 위 처방에서 보듯 «합성» 한다**: §7 은 **도구(prettier)가 코드 스팬을
재해석**하고 §9 는 **셸이 코드 스팬을 실행**한다. 방어도 갈린다 — §7 은 버전 고정, §9 는 인용 경계다.

**2026-08-22 하루에 백틱 취급 사고가 `4`회** 났다. **세 형태 모두 도구·셸이 코드 스팬을 재해석하거나 실행**한 것이고,
`—` 는 「층이 없다」가 아니라 **「대응하는 절이 아직 없다」** 는 뜻이다 (릴리스 귀속도 서로 다르다):

| 형태 | 대응 절 | 회수 | 릴리스 (entry 기록 위치) | 근거 |
| --- | --- | ---: | --- | --- |
| prettier 이중 백틱 정규화 | §7 | `2` | v0.77.0 | PR [#1133](https://github.com/coseo12/astro-simulator/pull/1133) 코멘트 (`#1125` 구현분) |
| 들여쓴 펜스 마스킹 | — | `1` | v0.78.0 | [#1134](https://github.com/coseo12/astro-simulator/issues/1134) |
| 셸 명령 치환 | §9 | `1` | v0.78.0 | PR [#1137](https://github.com/coseo12/astro-simulator/pull/1137) |

⚠️ *"한 릴리스 사이클에 4회"* 로 적으면 **거짓**이다 — 앞 `2`건은 `## [0.77.0]` 절에 기록돼 있다.
두 PR 이 **같은 날 머지**됐다는 것이 사실이고, 그 편이 오히려 강한 진술이다.
