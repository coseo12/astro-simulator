...
- **실측 — 모집단은 `CHANGELOG.md` 로 한정한다.** 손상형 **19 줄 / 33 발생 / 31 고유 종**, **전부 코드 펜스 밖**(= 실제 렌더링 손상)이며 **19 줄 전건이 릴리스 확정 구간**이다(`[Unreleased]` 내 **0**). 예: `` R1~~R4 `` · `` 7~~12 `` · `` 92k~~122 `` — 원래 `R1~R4` 등 단일 `~` 범위 표기였다. 술어 rev `42e9618` 과 본 커밋에서 **같은 값**이다(본 라운드 편집이 `CHANGELOG.md` 에 손상형을 더하지 않았음을 실측 확인).
