...
"
# stderr: 지워진 토큰마다 1줄씩 "command not found" (2줄)
#         ⚠ 형식·라인번호는 셸과 파일 형태에 의존한다 — bash `f.sh: line 2: 2: command not found` /
#         zsh `f.sh:2: command not found: 2`. 이 저장소 에이전트 셸은 **zsh 5.9** 다.
#         라인 번호는 펜스 안 주석 줄 수에 따라 바뀌므로 값 자체를 근거로 쓰지 않는다.
# exit  : 0
...
| prettier **소유** — **인라인 코드 스팬 안** | 유효 (실측: 스팬 안 이중 공백은 **보존**) | 잔두 grep 유효하나 위 산문 축과 병행 |
...
**2026-08-22 하루에 백틱 취급 사고가 `4`회** 났다. 셋 다 **도구·셸이 코드 스팬을 재해석하거나 실행**한 것이고,
