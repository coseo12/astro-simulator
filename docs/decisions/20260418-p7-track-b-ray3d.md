...
- **상태**: Accepted as permanent approximation (P7-C #208 — **5차 D' 보강 채택**, 실 Chrome 수동 검증 2026-04-18)
- **날짜**: 2026-04-18 (수정: 실 GPU 검증 후 채택 차수 변경)
- **결정자**: Architect (P7-C #208) / Developer (구현) / User (실 GPU 시각 검증)
- **채택 차수**: **5차 (D' 보강)** — 화면공간 근사, `diskAxisX/Y` 를 world disk major axis 의 화면 투영 방향으로 대체하여 "화면 x축 고정" 해소
- **실험 보존**: 3차 (후보 E · Frustum Corner Interpolation) 코드는 `?ray3d=1` 옵트인 경로로 보존 — **lensing 왜곡은 성공**(줄무늬 관찰됨), disk mask 계산은 실패 (rEcc3d/valid3d/escapeOutcome 중 하나 이상의 경로 실패로 추정). 향후 디버깅 자산.
- **관련**:
...
### 최종 채택 경로 (2026-04-18 실 Chrome 검증 후 개정)

**5차 (후보 D · D' 보강) 채택 · Accepted as permanent approximation**.

swiftshader headless 검증에서 3차(E)가 컴파일/렌더 성공으로 잠정 채택되었으나, 실 Chrome GUI 수동 검증에서 **`?bh=2&ray3d=1` 경로에서 accretion disk 렌더가 실패** (블랙홀만 검은 원으로 표시, disk mask 계산 실패). 3차(E)의 **lensing 왜곡은 정상 동작** (별 배경 줄무늬 휨 관찰됨) 하나 disk는 `rEcc3d`/`valid3d`/`escapeOutcome` 중 하나 이상의 경로 실패로 추정.

PM 계약 M1 백업 경로에 따라 자동 전환:

- `?bh=2` 기본 경로에서 `diskAxisX/Y` uniform을 **world disk major axis 의 화면 투영 방향**으로 대체 (기존 `(1, 0)` 화면 x축 고정 해소).
- `?bh=2&ray3d=1` 경로는 **실험적 보존** — lensing 왜곡 효과만 기여, disk는 여전히 기본값(D' 보강) 사용. 향후 디버깅 자산.
- 실 Chrome 실측(2026-04-18): 오렌지/노랑/빨강 accretion disk 타원이 뚜렷이 보이며 블랙홀 중심의 shadow 유지. DoD C1/C3/C4 충족, C2는 화면공간 근사 한계 내에서 disk 장축 화면 회전 응답 구현 (5차 본질적 partial).
...
| 3차 (E)     | **Frustum Corner Interpolation — CPU far-plane 4 corners + WGSL bilinear 보간**     | **부분 실패 (lensing 성공 / disk 실패)** | headless에서 컴파일 성공 + 비검정 + fps=23. **실 Chrome 수동 검증**: lensing 왜곡(별 배경 줄무늬)은 정상, accretion disk는 렌더 실패 (블랙홀만 검은 원). disk mask 계산 경로(rEcc3d/valid3d/escapeOutcome) 중 하나 이상 실패로 추정 — 디버깅 1~2h 추가 필요로 scope out, 코드는 실험적 경로로 보존 |
| 4차 (B)     | WGSL `mat4_invert` 헬퍼                                                             | **미진입**                               | 3차(E) lensing 성공으로 E 경로 보존, 4차는 디버깅 black hole 리스크 높아 scope out                                                                                                                                                                                                                 |
| **5차 (D)** | **D' 보강 — `diskAxisX/Y` world disk major axis 화면 투영 방향으로 대체**           | **성공**                                 | 실 Chrome 수동 검증 (2026-04-18): 오렌지/노랑/빨강 accretion disk 타원 뚜렷, 블랙홀 shadow 유지. `diskAxisX/Y`가 카메라 view 행렬에서 매프레임 재계산되어 화면 x축 고정 해소. DoD C1/C3/C4 충족, C2는 화면공간 근사 partial. **PM 계약 M1 백업 경로 활성화**                                       |
