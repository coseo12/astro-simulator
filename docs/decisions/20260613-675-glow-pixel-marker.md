...
- **fix (glow 코드 0 변경 — PM 확정값 전부 보존)**: (a) 경로에서 `coreRef.current?.command({ type: 'setLodOverride', level })` 직접 발행 동승 — handler 미등록이면 no-op ((b) 가 처리), 양 경로 idempotent. **회귀 가드**: `browser-verify-glow-marker.mjs` **축 6 신설** — requestAdapter 를 scene 준비 완료까지 게이트해 race-lost 방향을 결정론 재현 (pre-fix FAIL / post-fix PASS 3중 시뮬레이션 실측)
  - **후속 실측 포인터 (2026-09-23 — [#1239](https://github.com/coseo12/astro-simulator/issues/1239) / PR [#1255](https://github.com/coseo12/astro-simulator/pull/1255)): 바로 위 「pre-fix FAIL」이 현행 트리에서는 재현되지 않는다.** 같은 변이 (= 이 Amendment 의 fix 인 `command` 직접 발행만 되돌린 상태) 를 주입하면 **현행 트리와 #1234 직전 트리 (`71d30eb`)** 에서 종전 축 6 은 **PASS** 한다. 현행 트리 실측으로 드러난 기전은 dev StrictMode 의 **버려지는 첫 마운트 체인** — 「첫 호출만 붙든다」였던 게이트가 cleanup 되는 체인의 호출을 잡았고, `__gpuTier` 를 박고 `__solarScene` 을 노출하는 살아남는 체인은 게이트 밖에서 즉시 settle 했다. ⚠️ **본 줄의 2026-06-13 기록은 삭제·수정하지 않았고, 「그때는 왜 FAIL 했는가」도 단정하지 않는다 — 그 트리는 #1239 에서 재현 대상이 아니었다.** 두 트리가 최소한 어떻게 다른지만 확인됐다: `Promise.all([instance.start(), gpuCapPromise])` 가 [#738](https://github.com/coseo12/astro-simulator/issues/738) 로 2026-06-25 에 도입돼 #1234 C3-B 에서 분리됐으므로 2026-06-13 트리에는 그 대기가 없었다. 현행 트리 기준의 게이트 교체 (호출 순번 → **호출자**) 와 실측·변이 결과는 `apps/web/scripts/browser-verify-glow-marker.mjs` 헤더 §#1239 (그쪽에도 본 항목을 가리키는 역포인터가 있다).
