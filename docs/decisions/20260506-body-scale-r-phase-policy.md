...
> **SUPERSEDED-BY [20260629-762-body-scale-monotonic-forensic.md](20260629-762-body-scale-monotonic-forensic.md) (#762)** — 행성·왜소행성의 per-body `BODY_SCALE` 개별 박제값은 `radius^0.5 × k` 압축 곡선 SSoT로 전환됐다. §결정 3의 "R-Phase 진입 시 BODY_SCALE 1줄 추가" 절차는 행성·왜소에 대해 **dead** (곡선이 자동 산출 → 단조성 단위 테스트로 가드). 위성·혜성 박제값은 정적 역산으로 유지. 본 ADR의 2축 분리(시각 활성 vs focus 활성) 정책 자체는 유효.
