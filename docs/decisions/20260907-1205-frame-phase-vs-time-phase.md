...
⚠️ **「재생 델타 `0`」은 전칭이 아니다 — 첫 프레임이 예외다** (qa 실측, PR [#1208](https://github.com/coseo12/astro-simulator/pull/1208)). `simulation-core.ts` 의 `dt` 가 `#lastFrameTime === null ? 0 : …` 이고 초기값이 `null` 이라 **렌더 루프 첫 프레임의 `dt` 는 `0`** 이다. `TimeController.tick` 은 `dtSeconds <= 0` 에서도 `false` 를 반환하므로, **재생 중이어도 프레임 1 에서는** 시간 위상이 안 돌고 프레임 위상만 돈다. 방향은 안전하다 — 신 동작이 구 동작의 **초집합**이고, 프레임 1 은 모든 body 가 `prevLevel === undefined` 라 `lodFadeState` 에 등록되지 않아 fade 도 생기지 않는다. **그래도 전칭으로 적지 않는다** — 이런 문장은 다음 실측이 반증한다.
