...
| 날짜       | 변경 요약                                                                                                                                                                        | 근거 PR/이슈                      |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| 2026-04-18 | Phase C 진단 — "EIH structural bias" 가설 기각, 측정법(LRL+Newton subtraction) 한계로 재분류. 지구 DoD 1c → 3c 확대 (rel_err 1.19%). 금성 DoD ±1.0% → ±1.5% (10c, rel_err 1.39%) | #206 / PR #212                    |
| 2026-04-18 | CI `verify-and-rust` 재검토 임계 `> 7분 → > 11분`. 실측 9m15s + 20% 마진, perihelion 5개 구조적 하한 인정                                                                        | #210 / #215 / PR #222             |
| 2026-04-19 | §Amendments 섹션 신설 — 본 ADR 의 Phase C·CI 임계 갱신 2건을 테이블로 소급 박제 (규약 도입 시범 적용)                                                                            | #226 / `docs/decisions/README.md` |

### 2026-04-18 — Phase C 진단 (상세)

본문 §Phase C 진단 섹션 참조. Single1PN 대조 실험 + centuries 수렴 실험으로 측정법 한계 확정,
LRL 각도 측정의 저이심률 비선형 잔차가 근본 원인. 대응 옵션 A(centuries 확대) 채택.

### 2026-04-18 — CI 임계 갱신 (상세)

본문 §재검토 트리거 #4 참조. 종전 `> 7분` 임계가 perihelion 5개 테스트의 구조적 하한과 상충.
Phase C centuries 확대(3c/10c)가 정당화된 시점에 임계도 실측 기반(9m15s)으로 재조정.
