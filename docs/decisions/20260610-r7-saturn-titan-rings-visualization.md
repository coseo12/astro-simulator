...
| `solar-system.json` saturn.rings                                               | 신규 5층 (D/C/B/A/F — §축 2 데이터 표)                                 | `packages/shared/data/solar-system.json`                                 | jupiter.rings 동일 스키마 (`innerRadiusKm/outerRadiusKm/densityProfile`). Cassini Division 은 B-A 층간 gap 으로 자연 표현. F ring inner 140,180 km (outer 140,680 km) = saturn 반경 **2.33배** (PM "~2.3배" 정합 — Amendment 1 라벨 정정) |
...
| F ring inner~outer       | 140,180~140,680 km         | saturn 반경의 **2.326~2.334배** — PM "~2.3배" 정합 상한 (Amendment 1 라벨 정정)     |
...
## Amendment 1 (2026-06-10) — 구현 실측 정정 3건 (reviewer #643 권고)

- **상태**: Accepted (developer 실측 + reviewer 검증, PR #643)
- **① §축 1 guard 실측 예상 정정**: "예상 ~13.1%" → **실측 56.89%** (×1.05 = 임계 59.7 박제). 괴리 원인은 측정-정의 artifact (volt #32 §재검토 트리거 #4 발동, 측정 방법 검증 우선 수행): default solar view 에서 saturn 의 view-space depth **w=5.13** (유클리드 거리 111.4 unit 대비 ×21 perspective division 부풀림). jupiter +57% (w=22.3) 와 동일 메커니즘의 연속선 — guard metric 은 sun 대비 동일 위치·동일 식이라 **synthetic 회귀 감지 metric 으로 유효** (3 viewport 결정적). scale 박제값 (wsRadius 비 0.843 = 사실 비) 정상 — 정책 식값 ≠ guard 실측 이원화 (R6 Amendment 1 선례) 유지.
- **② F ring 라벨 정정**: 산문 "F ring outer 140,180 km" → **inner 140,180 / outer 140,680 km** (데이터 표·코드·테스트는 처음부터 정합 — 산문 라벨만 오기). 본문 2곳 정정 완료.
- **③ 모바일 누적 diskArea 오염 메모 (R8 위험 인계)**: `r1-ui-regression-guard.mjs` 모바일 누적 diskArea 가 **off-screen 면적도 합산** — saturn artifact ~13%p 포함 22.96% ≤ 25% (잔여 마진 2.04%p). **R8 uranus 진입 시 spurious FAIL 위험** → R8 ADR 에서 off-screen 제외 또는 metric 의미 재정의 결정 의무 (§R8 인계 #4 신규).
