...
- **변경 내용**:
  ...
  - **「제안」 기준이 조문을 자기모순으로 만든다** — 본문의 다음 문장 _"통과하면 예외, 못 하면 금지다"_ 는 「제안」 기준에서 **카운트와 무관한 유휴 문장**이 된다 (통과 여부가 회차 계산에 아무 영향을 주지 않으므로). 「원용」 기준에서만 그 문장이 **카운트를 가르는 게이트**로 기능한다
  - **§결정 5 말미 포인터의 이중 해석** — `L285` 의 `(§재검토 조건 2)` 는 **라벨 기준**으로 §결과 · 재검토 조건 목록의 `3.` 항(`L297` **재검토 조건 2 — 모집단 이동**)을 가리키고, **서수 기준**(목록의 두 번째 항목)으로는 `2.` 항(`L294` **재검토 조건 1 — 예외 원용 2회차**)을 가리킨다. 문맥상 의도는 후자다 — `L285` 가 말하는 것은 「2번째 사례 발생 시 상설화 재검토」이고 그 조건을 담은 항은 **재검토 조건 1** 이며, 모집단 이동(`.prettierignore` 변경)은 상설화 재검토와 무관하다. **원문 `L285` 는 편집하지 않고** 아래 실효 SSoT 에서 정정된 포인터를 재기술한다
- **결정 (2026-08-31)**: **「원용」 기준으로 확정한다.** 원용 = **제안 ∧ 결정 2 술어 PASS**. 근거 셋. **(1) 조문의 두 앵커가 「원용」이다** — 표제와 원 결정(§결정 5) 양쪽이며, 「제안」은 본문 한 곳뿐이다. **(2) 「제안」 기준은 본문 자신의 게이트 문장을 유휴화한다** (위 §변경 내용) — 조문 전체를 살리는 해석은 「원용」뿐이다. **(3) 결정 5 의 재검토 취지는 _"1회성 전제가 깨진 것"_ 이다** — 술어에서 기각된 제안은 예외를 **행사한 바 없으므로** 1회성 전제를 깨지 않는다. ⇒ **결정 5(상설화 기각)의 재검토는 발동하지 않는다.** **현재 원용 카운트 = 1** (§결과 1 의 확정 구간 `21` 줄 회수 1건)
- **영향**:
  - **코드 수정 `0`** (본 ADR 은 규범 문서). 가드 스크립트(`verify-adr-index.mjs` / `verify-md-tilde.mjs` / `verify-docs-links.mjs`) **무접촉**
  - **본문 조문 무접촉** — 본 PR 이 이 파일에서 편집하는 원문은 **4행 상태 라인 1행**뿐이다
  ...
  - **인덱스 정합** — [`docs/decisions/README.md`](README.md) 의 본 ADR 행 상태 셀을 같은 PR 에서 함께 갱신한다 (`verify-adr-index` 가 대조하는 한 쌍이라 한쪽만 움직이면 가드가 FAIL 한다)
...
<!-- 메인 오케스트레이터가 cross-validate 수행 후 채운다 (developer 페르소나 직접 호출 금지 — 4행 상태 라인 및 .claude/agents/developer.md §규칙 #479). 통합 완료 후에만 4행을 Accepted 로 전이한다. -->
