...
- **(가) 소급 편집 금지** — `.claude/agents/reviewer.md` §절차 4 4항 **2계급**: _"그 시점의 사실 서술인가
  ...
  되돌릴 수 없다"_.
- **(나) 가독성 회복** — 현재 `CHANGELOG.md:1353` 은 _"Amendment 7~~12 누적 + R1~~R3 후속"_ 이라
  GitHub 에서 **`12 누적 + R1` 이 취소선**으로 렌더링된다. 미래 관찰자에게 _"철회된 내용"_ 신호를 준다.
...
- **렌더링 손상이 아니다** — 짝이 없어 취소선 노드가 되지 않는다. 실측: 회수 후
  `pnpm exec prettier --check CHANGELOG.md` **exit 0**. bare `~~` 가 0 인 상태에서 이 exit 0 은
  _"파서가 취소선 노드를 하나도 못 찾았다"_ 와 동치다 (찾았다면 delimiter 를 `~~` 로 정규화해
  `--check` 가 FAIL 한다 — §배경 격리 재현이 그 동작을 보인다).
...
4. **비목표 (조용한 스킵 아님)** — ① 소급 편집 금지 규범 **자체의 개정**: 본 ADR 은 예외 범위를
   ...
   150 발생 (결정 4) ④ `docs/**` 의 의도된 취소선 (prettier 미소유 — 손상 자체가 불가능).
