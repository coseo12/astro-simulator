...
> [!NOTE]
> **해석 주의** — 본 조항의 「원용」 기준과 말미 상호참조 `(§재검토 조건 2)` 는 파일 말미 **§Amendment 1 (2026-08-31)** 으로 효력이 확정됐다. 원문은 보존돼 있으며 실효 조문은 그 Amendment 의 `실효 SSoT` 블록이다.
...
- **결정 (2026-08-31)**: **「원용」 기준으로 확정한다.** 원용 = **제안 ∧ 결정 2 술어 PASS**. 근거 셋. **(1) 조문의 두 앵커가 「원용」이다** — 표제와 원 결정(§결정 5) 양쪽이며, 「제안」은 본문 한 곳뿐이다. **(2) 「제안」 기준은 과잉 완화(over-relaxation) 위험을 만든다** — 예외 상설화는 릴리스 불변성의 예외 범위를 넓히는 고위험 결정이므로, **술어를 통과하지 못한 실패한 시도**에 의해 상설화 재검토가 촉발되는 것은 리스크 관리 원칙에 어긋난다 (위 §변경 내용). **(3) 결정 5 의 재검토 취지는 _"1회성 전제가 깨진 것"_ 이다** — 술어에서 기각된 제안은 예외를 **행사한 바 없으므로** 1회성 전제를 깨지 않는다. ⇒ **결정 5(상설화 기각)의 재검토는 발동하지 않는다.** **현재 원용 카운트 = 1** (§결과 1 의 확정 구간 `21` 줄 회수 1건)
- **영향**:
...
- **cross-link**: 본 Amendment / 이슈 [#1138](https://github.com/coseo12/astro-simulator/issues/1138) / PR [#1137](https://github.com/coseo12/astro-simulator/pull/1137) (reviewer 라운드 2 BLOCK-1 [`5379745121`](https://github.com/coseo12/astro-simulator/pull/1137#issuecomment-5379745121)) / 원 이슈 [#1134](https://github.com/coseo12/astro-simulator/issues/1134) / 해석 정본 [`20260808-983`](20260808-983-measurement-recording-convention.md) §Amendment 1 / 전이 선례 [`20260701-779`](20260701-779-ci-alert-fatigue-concurrency.md) §Amendment 2 / 상위 §결정 2 · §결정 5 · §결과 · 재검토 조건
...
| 판정 주체 | reviewer 라운드 2 BLOCK-1 ([`5379745121`](https://github.com/coseo12/astro-simulator/pull/1137#issuecomment-5379745121)) |
