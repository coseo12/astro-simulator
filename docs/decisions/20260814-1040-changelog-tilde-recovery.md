...
- **변경 내용**:
  - **드러난 이중 표현** — 「원용」쪽 앵커가 둘이다: 조문 **표제**(`**재검토 조건 1 — 예외 원용 2회차.**`)와 **원 결정**(§결정 5 말미 `본 예외를 원용하는 **2번째 사례**`). 「제안」은 본문 첫 절 한 곳이다 (빈도 대비가 아니라 앵커의 소재를 적은 것이다 — 근거로서의 무게는 §결정 (1) 참조)
