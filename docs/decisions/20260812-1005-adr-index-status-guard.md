...
## Amendment 3 (2026-08-16) — 세 번째 범위 경계 픽스처 `F20` + **범위 라벨 최초 이동** (#1075)

### 배경

ADR [`20260814-1031-1064`](20260814-1031-1064-committed-claim-guard-rejected.md) §결정 4 가 본 가드의
**확장을 기각**하면서 세 번째 범위 경계 — _"인덱스 **행 본문의 수치·술어 내용**은 검사 대상이
아니다"_ — 를 명시했고, 그 §Developer 인계 잔여 2 가 경계 픽스처 1건을 요구했다
([#1075](https://github.com/coseo12/astro-simulator/issues/1075)). 기존 두 경계(미등재 / 산문 역참조)가
`F10` 으로 고정돼 있는 것과 같은 형식이다.

### 변경 사항

- **`F20` 신설** — 한 인덱스 행에 **수치 거짓**(표 `999건` · 정밀도 `1.0` vs 실물 `3건` · `0/76`)과
  **실행 불가능한 술어**(존재하지 않는 패턴을 세는 재도달 명령)를 동시에 심고, 상태 토큰이 양쪽 다
  `Accepted` 이므로 **위반 0 · 대조 1건**임을 단언한다. `compared.length === 1` 을 함께 요구해
  _"행이 조용히 스킵돼서 PASS"_ 라는 vacuous 경로를 닫는다.
- **헤더 §범위 경계 (iii) 추가** + 본검사 stdout 의 _"범위 밖 (의도적 미검출)"_ 한 줄에 세 번째
  경계와 `F20` 을 병기. 산문이 두 경계만 열거하고 픽스처는 셋을 고정하는 drift 를 만들지 않는다.
- **검사 로직 무접촉** — `runCheck` 이하 판정 경로는 한 글자도 바뀌지 않았다 (§결정 4 가 확장을
  기각했고, 이슈 §명시적 비-범위 가 이를 재확인한다).

### 단언 수 — 최종값 1회 일괄 (ADR 20260808-983 §(i))

[`20260813-1020`](20260813-1020-adr-index-membership-marker-rejected.md) §결정 3 절차를 그대로 적용했다.
술어 자기 검증 `grep -c 'assert(' scripts/verify-adr-index.mjs` == `--self-test` 출력의 `N passed`
== **50** (`49` → `50`, 차분 `+1`. 절대값·차분을 한 번의 출력에서 함께 도출했다).

⚠️ **범위 라벨이 처음으로 움직인다 — 갱신 대상이 1곳이 아니라 3곳이다.** Amendment 2 는
_"`F19p`·`F19q` 는 기존 `F19` 아래로 들어가므로 범위 라벨 `F1~F19` 는 불변"_ 이라 **1곳**만 갱신했다.
본 Amendment 의 `F20` 은 그 전제를 깨므로 1020 §결정 3 표의 **3곳 전건**이 대상이다:

| 위치 | `F1~F19` → `F1~F20` | 단언 수 |
| --- | :---: | :---: |
| `scripts/verify-adr-index.mjs` 호출 예시 주석 | ✅ | — |
| `scripts/verify-adr-index.mjs` self-test 섹션 헤더 | ✅ | `49` → `50` |
| `.github/workflows/project-guards.yml` | ✅ | — |

Amendment 1·2 본문의 `47` · `49` 및 _"범위 라벨 불변"_ 서술은 각각 **PR #1036 · #1077 시점 이력**이라
치환하지 않는다 — 본 Amendment 가 승계한다 (1020 §결정 3 편집 금지 계급 ②).

**1020 §결정 3 이 예고한 sweep 술어 자릿수 함정이 여기서 처음 «살아 있는» 조건이 됐다.** 다만 예고가
겨눈 시점은 *갱신 전*이 아니라 **갱신 후**다 — 좁은 판본 `F1~F1[0-9]` 도 갱신 **전** 표기 `F1~F19` 는
찾아내므로 이번 sweep 의 «대상 발견» 은 정상 작동했을 것이다. 무너지는 것은 **그 다음**이다: 갱신
후의 `F1~F20` 을 좁은 판본은 **한 건도 매칭하지 못한 채 exit 0** 을 내므로 (실측: 두 라벨을 넣고
`grep -cE 'F1~F1[0-9]'` → **`1`** / `grep -cE 'F1~F[0-9]+'` → **`2`**), 갱신 직후의 재확인과 이후
모든 sweep 이 _"해당 표기 없음"_ 으로 침묵한다 — 즉 «0 hit 은 결과가 아니라 신호» 를 읽을 사람조차
없어진다. PR #1036 reviewer Y-4 가 이 경로를 미리 닫아 뒀고, 본 Amendment 가 그 판정의 **첫 수혜
사례**다.
...
- **상태 라인·인덱스 표 행 불변** — 본 ADR 은 여전히 `Accepted` 이고 인덱스 표 행도 그대로다.
- **본검사 거동 불변** — `node scripts/verify-adr-index.mjs` 는 계속 PASS (`상태 불일치 0`).
  바뀐 것은 §범위 경계 안내 한 줄의 **문구**뿐이고 판정은 무접촉이다.
- **비-vacuous 실증** — `F20` 픽스처의 ADR 실물 상태만 `Accepted` → `Provisional` 로 주입하면
  `50 passed, 0 failed` → **`49 passed, 1 failed` exit `1`** (격리 사본 실측). 즉 이 픽스처는
  대조 경로를 실제로 통과하며, PASS 가 _"검사하지 않아서"_ 가 아니라 **경계라서**임을 보인다.

## Amendment 4 (2026-08-16) — allowlist 를 «로드하는 모듈» 로 재정의 (재수출 채택 / 들여쓰기 기각) (#1084)

### 배경 — 우회 2건 중 **하나만** 백스톱이 `0` 이다

PR [#1077](https://github.com/coseo12/astro-simulator/pull/1077) reviewer 가 `F19n` 의 import allowlist
가드에서 우회 2건을 실측했고 ([#1084](https://github.com/coseo12/astro-simulator/issues/1084)), 둘 다
base 패리티라 **선재 결함**이다. 본 Amendment 는 두 건을 **갈라서** 판정한다 — 갈리는 축은 _"정규식으로
닫을 수 있는가"_ 가 아니라 **_"이미 다른 백스톱이 잡고 있는가"_** 다.
...
재는 것으로는 _"prettier 가 원래 아무거나 잡는다"_ 를 배제할 수 없기 때문이다.

### 결정 1 — `ALLOWED_IMPORTS` → `ALLOWED_MODULE_LOADS` (이슈 §범위 2항)

allowlist 의 의미를 _"import 하는 모듈"_ 에서 **_"로드하는 모듈"_** 로 확정하고 상수명을 그에 맞춘다.
이름이 곧 의미 선언이라, 초판 이름이 재수출을 **정의상 배제**한 것이 결함의 발원이었다.

**재수출이 «로드» 라는 것은 산문이 아니라 실측이다** (#1084, 2축 · 스크래치 재현 후 폐기):

1. **사용자 모듈 축** — `export { x } from './loaded.mjs';` 한 줄뿐인 모듈을 부르면 `loaded.mjs` 의
   최상위 `console.log` 가 **찍힌다** (모듈 본문 실행 = 로드).
2. **내장 모듈 축** — 자식 프로세스 계열 모듈을 재수출하는 모듈을 부르면 `process.moduleLoadList` 의
   해당 항목이 **`false` → `true`** 로 뒤집힌다.

⇒ 본 가드가 [#1018](https://github.com/coseo12/astro-simulator/pull/1018) cross-validate 보안 축에서
근거로 삼은 **_"자식 프로세스 모듈 import 0"_ 불변식은 재수출 경로에 대해 열려 있었다.** 이 Amendment
이전의 그 문장은 «import» 라는 낱말 그대로 읽으면 참이고, 의도한 «로드» 로 읽으면 거짓이었다.

### 결정 2 — 수집과 계수를 **함께** 넓힌다 (이슈 §범위 1항 경고 준수)

이슈가 경고한 대로 한쪽만 넓히면 두 조건이 상시 불일치해 무조건 FAIL 한다. 그래서 **두 축을 대칭으로**
넓혔다 — 수집(조건 ①)과 선언 시작 줄 계수(조건 ②) 각각에 재수출 판본을 추가했다.

- **사전 구간에서 따옴표를 배제**한다. 그러지 않으면 `export const X = 'from';` 이 따옴표 **안**의
  `from` 때문에 잡혀 오탐이 된다. 추가로 `from` 뒤에 **명세자 따옴표가 곧바로** 올 것을 요구해
  `export const from = 1;` 같은 형태도 배제한다.
- **두 축의 계수는 정규식을 분리해 각각 센 뒤 더한다.** 하나의 교대 정규식으로 합치면 재수출 매치가
  소비한 구간에 다음 축의 줄 시작이 묻혀 계수가 **서로를 상쇄**할 수 있다 (사전 구간이 줄바꿈을 넘기
  때문). 분리하면 축 간 소비 간섭이 정의상 `0` 이다.
- **픽스처의 allowlist 밖 sentinel 은 `node:vm` 으로 통일**한다. 자식 프로세스 계열 모듈명을 커밋물에
  적으면 `grep -c <모듈명>` 이라는 사람의 감사 술어가 오염돼 Amendment 1 §Y-1 판정이 스스로 무너진다
  (현재 이 파일의 해당 모듈명 등장 수 = **`0`**, 본 Amendment 이후에도 `0`). #1084 의 우회 실증은 그
  모듈로 했으나 **격리 사본에서만** 했다.

### 결정 3 — 들여쓴 `import` 는 **기각**하고 헤더 §범위 경계 **(iv)** 로 박제 (이슈 §범위 3항)

침묵 기각을 하지 않는다 — 스크립트 헤더가 _"들여쓴 모듈 로드 선언은 범위 밖"_ 임과 그 근거·재검토
조건을 명시한다. 기각 근거 3축:

1. **백스톱이 `0` 이 아니다** — 위 표. CI 배선은 `.github/workflows/ci.yml` §#952 포맷 백스톱
   (`pnpm run format:check`) 이고 `scripts/` 는 `.prettierignore` 대상이 아니다.
2. **넓히면 기존 오탐 표면이 확대된다** — 이 파일은 이미 _"코드 무변경 FAIL"_ 클래스를 갖는다 (블록
   주석의 컬럼 0 `import` 줄이 세어지는 경로, Amendment 2 실측). 앵커를 공백 허용으로 넓히면 그
   표면이 **들여쓴 산문 전체**로 커진다 — 본 파일의 주석은 import 규칙을 한국어로 길게 논한다.
3. **얻는 것 < 잃는 것** — 백스톱 `1 → 2` 를 얻고 상시 오탐 표면을 얻는다.
...
| --- | --- | --- |
| 재수출 1줄 (allowlist 밖) | `50 passed, 0 failed` exit `0` | **exit `1`** — 계수 `7`/기대 `6` + 수집에 해당 모듈 등장 |
| 재수출 다중행 (prettier 분할형) | `50 passed, 0 failed` exit `0` | **exit `1`** (동일 진단) |
| 재수출 세미콜론 누락 | `50 passed, 0 failed` exit `0` | **exit `1`** — 계수 `7`/기대 `6`, 수집은 `6` (조건 ② 단독 검출) |
| **오탐 대조군** — 모듈 로드 아닌 컬럼 0 export 4종 | `50 passed, 0 failed` exit `0` | `53 passed, 0 failed` exit `0` (**불변**) |
| 들여쓴 import (§결정 3 기각 대상) | `50 passed, 0 failed` exit `0` | `53 passed, 0 failed` exit `0` (**의도된 미검출**) |
| (원복) | `50 passed, 0 failed` | `53 passed, 0 failed` — **recovery** |

**오탐 `0` 은 양성 대조군과 같은 실행에서 잰다.** 음성만 재면 수집이 통째로 고장 나도 통과하므로
vacuous 하다. 두 층에서 동봉했다:

- **실물 파일 층** — 음성 4종 + 양성 1건을 **한 번에** 주입하면 계수가 정확히 `7`(= `6` + `1`),
  수집도 정확히 7 항목이고 늘어난 것은 재수출한 모듈 **하나뿐**이다 (`52 passed, 1 failed`). 음성이
  하나라도 샜다면 값이 더 커진다.
...
| 라벨 | 겨냥 | 대응 |
| --- | --- | --- |
| `F19r` | 오탐 `0` + 양성 대조군 (한 소스·한 단언) | 신규 축 |
| `F19s` | 다중행으로 쪼개진 재수출도 1건 수집 | `F19p` 의 재수출 축 |
| `F19t` | 재수출 세미콜론 누락 · `import` 뒤 한 줄 혼재 → 계수로만 드러남 | `F19q` 의 재수출 축 |

`F19p`·`F19q`·`F19n` 은 이제 **같은 두 함수**(`collectLoads` / `countLoadStarts`)를 부른다 — 픽스처와
실물 단언이 사본으로 갈릴 수 없다 (volt [#120](https://github.com/coseo12/volt/issues/120)).

### 단언 수 — 최종값 1회 일괄 (ADR 20260808-983 §(i))

[`20260813-1020`](20260813-1020-adr-index-membership-marker-rejected.md) §결정 3 절차 적용. 술어 자기
검증 `grep -c 'assert(' scripts/verify-adr-index.mjs` == `--self-test` 출력의 `N passed` == **53**
(`50` → `53`, 차분 `+3`. 절대값·차분을 한 번의 출력에서 함께 도출했다).

⚠️ **범위 라벨 `F1~F20` 은 불변이라 갱신 대상은 다시 1곳이다.** 신규 픽스처 3건이 전부 `F19` 아래로
들어가므로 Amendment 2 와 같은 상황이고, Amendment 3 의 _"3곳 전건"_ 은 `F20` 신설이라는 **그때의
조건**에 종속된 판정이었다. 갱신 후 sweep 으로 재확인했다:

| 위치 | `F1~F20` | 단언 수 |
| --- | :---: | :---: |
| `scripts/verify-adr-index.mjs` 호출 예시 주석 | 불변 | — |
| `scripts/verify-adr-index.mjs` self-test 섹션 헤더 | 불변 | `50` → **`53`** |
| `.github/workflows/project-guards.yml` | 불변 | — |

**편집 금지 (계급 ② 이력 기록)** — 갱신 후 sweep 이 찾아낸 옛 값 전건을 분류했다. 본문 Amendment 1~3
의 `47` · `49` · `50` 과 `CHANGELOG.md` `[Unreleased]` 의 `== **50**` 은 각각 PR #1036 · #1077 ·
#1092 **시점의 측정 기록**이라 치환하지 않는다. 확정 릴리스 섹션(`CHANGELOG.md` `[0.68.0]` 등)의
`23 단언` · `47 단언` 도 소급 편집 금지다. 현재값은 본 Amendment 가 승계한다.

**부수 정정 1건** — self-test 섹션 헤더의 세부 라벨 범위 표기가 `F19a~F19o` 에 멈춰 있어 PR #1077 이
추가한 `F19p`·`F19q` 를 빠뜨리고 있었다 (선재 drift). `F19a~F19t` 로 정정한다. 본문 §Amendment 1 의
같은 표기(`:203`)는 **그 시점 기록**이라 무치환이다.

> ⚠️ 위 두 범위 표기를 **하나의 코드 스팬**으로 감싼 것은 서식 취향이 아니라
> [`20260814-982`](20260814-982-changelog-tilde-guard.md) 클래스 회피다. 근거는 **축마다 등급이 다르다**:
>
> - **[실측]** `CHANGELOG.md` 축 — 코드 스팬 **사이**에 `~` 를 둔 형태(`` `F19a` `` + `~` + `` `F19o` ``)를
>   본 PR 의 CHANGELOG 항목에 쓰자 `pnpm exec prettier`(lockfile `3.9.6`)가 그 자리를 `~~` 로 바꿨다
>   (`--check` FAIL → 출력 대조로 확인). 982 §배경이 서술한 정규화가 그대로 재현된 것이다.
> - **[미측정 · 예방]** `docs/**` 렌더링 축 — 이 디렉토리는 `.prettierignore` 대상이라 위 변환 경로가
>   **없다**. 코드 스팬 사이 단일 `~` 가 한 문단에 여럿일 때 GFM 이 이를 취소선으로 짝짓는지는
>   **본 PR 에서 측정하지 않았다** (GFM 취소선 확장이 _"one or two tildes"_ 를 받는다는 스펙상
>   가능성만 확인). 비용이 0 이라 예방적으로 단일 코드 스팬을 쓴다. 본문 §Amendment 1 `:203` 은
>   같은 형태이나 그 문단의 `~` 가 **1개뿐**이라 짝 자체가 성립하지 않는다 — 무치환 판정과 무관하게
>   현 상태로 안전하다.
...
- **상태 라인·인덱스 표 행 불변** — 본 ADR 은 여전히 `Accepted` 이고 인덱스 표 행도 그대로다.
- **본검사 거동 불변** — `node scripts/verify-adr-index.mjs` 는 계속 PASS (`상태 불일치 0`).
  변경은 `--self-test` 의 자기 배선 검사에 한정되며 인덱스 대조 로직(`runCheck` 이하)은 무접촉이다.
  본검사 stdout 의 _"범위 밖"_ 한 줄도 불변이다 — 신규 경계 **(iv)** 는 인덱스 대조가 아니라 **자기
  배선 검사**의 경계라 축이 다르고, 헤더가 그 사실을 명시한다.
- **allowlist 항목 불변** — 6항목 그대로다. 바뀐 것은 **이름과 수집 범위**이지 허용 집합이 아니다.
- **자식 프로세스 모듈 로드 0** — 이제 «import» 가 아니라 **«로드»** 기준으로 참이다 (결정 1).
