...
FAIL 한다. 실측: 그런 주석 **3줄** 주입 시 **실행 코드 `607`행 동일**인데 `48 passed, 1 failed`.
따라서 두 단언은 이 클래스를 **공유**하며(`F19o` 는 파일 전문 스캔, `F19n` 은 컬럼 0 앵커),
`F19n` 메시지에도 **산문 분기 1절**을 넣었다.

⚠️ **위 `607` 의 술어와 rev** (PR [#1077](https://github.com/coseo12/astro-simulator/pull/1077)
reviewer 라운드 2 — 초판에 **둘 다 없었다**). rev `8cec8bd` / 술어: 블록 주석을 **비탐욕 전역
제거**한 뒤 행 주석·공백 전용 줄을 빼고 계수한다.

```bash
perl -0777 -pe 's{/\*.*?\*/}{}gs' scripts/verify-adr-index.mjs | grep -vE '^\s*//' | grep -c '[^[:space:]]'
```

**술어를 바꾸면 결론이 뒤집힌다.** 줄 접두만 보는 나이브 판정(`grep -vE '^\s*(//|/\*|\*|\*/)'`)은
주입 블록의 **가운데 줄**(`import …`)이 주석 접두를 갖지 않아 살아남으므로 `607` → `608` 로 갈리고,
그러면 _"코드가 변했다"_ 는 **반대 결론**이 나온다. 값이 아니라 **술어가 판정을 만든다**. 현 head 는
같은 술어로 `609` 라 위 값은 **rev 종속**이며, 저장소 정본(`tsc --removeComments` emit `cmp` —
`27297` 바이트 동일)으로도 같은 결론이 재확인됐다.

이 항 자체가 §범위 경계 바로 앞에서 **🟡-6 과 같은 클래스의 재발**이었다 — 같은 PR 이 `#1056` 축에서는
rev 를 정확히 앵커해 놓고(`4be1207` / `38b6c8a`) 이쪽에서는 둘 다 빠뜨렸다. **한 PR 안에서 처리가
갈리면 규약이 아니라 우연**이라는 것이 이 정정의 요지다.
...
✅ **후속 이슈 [#1084](https://github.com/coseo12/astro-simulator/issues/1084) 로 승격됐다** (메인
오케스트레이터 수행, PR [#1077](https://github.com/coseo12/astro-simulator/pull/1077) reviewer 라운드 2).
⚠️ **ADR 본문 박제만으로는 부족했다** — [`20260812-970`](20260812-970-pr-base-rule-guard.md) §9 가
_"표에만 있는 후속은 유실된다"_ 를 실증하면서 처방한 것은 _"[#1030](https://github.com/coseo12/astro-simulator/issues/1030)
이 그것을 **이슈로 승격해** 닫은 사례"_ 였고, 본 항은 승격 전까지 정확히 그 **«유실 형태»** 에
머물러 있었다. 아래 **성격·백스톱·비용**은 그 이슈의 근거로 남긴다. 단 PR [#1036](https://github.com/coseo12/astro-simulator/pull/1036)
