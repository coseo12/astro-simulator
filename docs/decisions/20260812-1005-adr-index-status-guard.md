...
고정 범위 **①** 을 **_"정적 import 선언 집합 == allowlist"_** 로 넓힌다 (② `^import` 시작 줄 수 ==
allowlist 길이 · ③ 동적 모듈 로드 표현 0 은 **불변**). 수집 정규식은 시작을 `^import` 로, 종료를
...
| **`node:fs` 에 `statSync` 추가 → `prettier --write` 실분할(10줄)** | **`49/0` exit `0`** | **— (구 판정본은 `46/1` exit `1`)** |
...
오히려 `F19n` 의 같은 클래스 경로가 소멸하면서 저 표지 문장이 **배타적으로** 참이 됐다 — 정확도
상승을 무편집으로 얻는다. "같은 안내 체계로 묶는다" 는 모집단이 **1** 이면 체계가 아니다.
