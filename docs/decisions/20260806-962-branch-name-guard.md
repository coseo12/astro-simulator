...
> **부기 (2026-08-14, [#1056](https://github.com/coseo12/astro-simulator/issues/1056)) — 위 _"예외 없이 갈린다"_ 는 술어 범위를 좁혀 읽어야 참이다.** 본문은 이력이라 치환하지 않고 반례를 승계한다.
>
> 반례 **1건**: `.github/PULL_REQUEST_TEMPLATE.md` 의 release 체크리스트가 CHANGELOG 헤딩(**버전 값** 자리)을 `[vX.Y.Z]` 로 지시하고 있었다. 표의 3행과 정반대다.
>
> **원인은 결정이 아니라 실측 술어다.** §9-3 의 전수는 _"(실측 2026-08-14, `v0.71.0` 기준)"_ — **구체 값**을 훑는 술어라 `X.Y.Z` 같은 **자리표시자** 자리가 원리적으로 보이지 않는다. 자리표시자를 포함하는 술어(`git grep -nE 'X\.Y\.Z'`)로 다시 훑으면 활성 선언 중 버전 값 자리에 `v` 를 붙인 곳이 **2건**이었고(둘 다 PR 템플릿 — CHANGELOG 헤딩 지시 + 핫픽스 `패치 버전 bump`), #1056 에서 정정해 **0건**이 됐다. 구체 값 자리(`package.json` · CHANGELOG 실제 헤딩)는 정정 전에도 위반 **0** 이라 §9-3 표 자체는 무손상이다.
>
> **결정(`release/<X.Y.Z>-prep`, `v` 없음)은 반증되지 않는다** — 버전↔태그 축은 그대로다. 반증된 것은 _"예외 없이"_ 라는 **전칭 단정의 사정 범위**이며, 이 저장소가 [#1044](https://github.com/coseo12/astro-simulator/issues/1044)·#1046·#1050·#1052 에서 반복 지적한 **반증 가능한 단정** 클래스다. 전칭을 쓸 때는 그것을 지탱하는 **술어의 사각**을 함께 적는다.
