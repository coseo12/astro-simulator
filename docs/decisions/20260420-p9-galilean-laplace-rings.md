...
| 2026-04-20 | **Osculating 1Hz polling `timeScale ≫ 1` 내성 부재 — D7 인프라 완결 / UI 동적 표시 이관 (#263)** — `use-osculating-sync.ts` 의 forward-diff velocity 추정이 기본 `timeScale=86400s/s` 조건에서 noise 과다로 UI 패널 "1Hz 배지" 표시 조건 미충족. 훅·fps 폴백·WASM wiring 인프라는 완결. 해결은 Babylon 씬 저장 velocity state vector 직접 추출 (forward-diff 폐기) — [P9-followup #263](https://github.com/coseo12/astro-simulator/issues/263) 이관. v0.9.0 에서는 D8 UI 는 정적 JSON 값 표시, `?mass=jupiter×N` 은 Newton 경로에서 씬 물리에 반영됨 (거리 감소 실측 확인) | PR-3 실 Chrome 검증 발견. §재검토 조건 #3 (Osculating 왕복 오차) 의 구현 조건 확장                                                                                                                                          |
