...
| 2026-04-20 | **GLSL 배열 제약 대응 — `densityProfileR[16]` + `densityProfileD[16]` 분리** — ADR §결정 2 의 `densityProfile: Array<[number, number]>` 인터페이스는 GLSL ES 2.0 struct array 미지원 제약으로 `float[16]` 2개 배열 + `profileLength` uniform 으로 분리됨. 본질적 의도 (uniform 배열 + fragment 선형 보간) 유지. 최대 N=16 점 (Halo/Main/Gossamer 3층 × 평균 5점 = 15점 커버). `ring-shader.ts` 런타임 API 는 여전히 `Array<[r, d]>` 수용 후 내부 변환                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | PR #262 reviewer 권고 (minor). GLSL ES 2.0 WebGL/Babylon 환경 실측 검증                                                                                                                                                     |
| 2026-04-20 | **비동기 shader `onError` 폴백 미구현 (동기 exception 경로만 실동작)** — §R2 완화 는 shader 컴파일 시 `try { ... } catch` 로 감지한 동기 exception 에 한해 M1 InstancedMesh/SPS 전환. Babylon `onCompiled/onError` 비동기 콜백은 기록만 수행 (자동 전환 없음). 수동 플래그 `?ring=fallback` 은 정상 동작. §재검토 조건 #5 (shader 3차 이상 실패 시 InstancedMesh 영구 전환) 에 위임                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | PR #262 reviewer 권고 + PR 본문 솔직 고지                                                                                                                                                                                   |
| 2026-04-21 | **P10-D #261 JPL 초기 조건 교정 + D5-b scope 재조정** — Galilean 4체의 `meanLongitudeDeg`/`longitudeOfPerihelionDeg`/`longitudeOfAscendingNodeDeg` (및 연관 e/i/a) 를 JPL Horizons API 재쿼리 (2026-01-01 00:00 TDB, Jupiter-centric J2000 ecliptic) 값으로 교체. frame 을 Laplace plane → ecliptic 통일. Laplace 공명 인자 **φ₀ = 179.6929°** (평형점 180° ± 0.31°) 달성 — 신규 빠른 경로 `test_laplace_initial_phase_equilibrium` 로 실증. 그러나 100 Io 주기 적분 amp = 767° (circulation) 관찰 — 순수 Newton 다체 적분은 tidal force 미모델링으로 libration 재현 불가. **§결정 #5 DoD "amp ≤ 2°" 는 scope 재조정**: D5-b assertion 은 `test_laplace_phase_amplitude_2deg_scope_deferred` 로 `#[ignore]` 처리, tidal force 모델링 + 재개는 후속 이슈 [#282](https://github.com/coseo12/astro-simulator/issues/282) (P11+) 로 이관                                                                                                                                                                          | CRITICAL #6 §5/§7 재조정 3위치 박제 (코드 주석 + PR 본문 + 본 Amendment). volt #32 "수치 DoD 미달 시 측정 방법 검증 우선" 준수 — 본 사례는 식·데이터는 정확하나 역학 모델 누락이 드러난 케이스                              |
| 2026-04-20 | **Osculating 1Hz polling UI 동적 표시 결함 — D7 인프라 완결 / UI 동적 표시 이관 (#263)** — `use-osculating-sync.ts` 의 UI "1Hz 배지" 렌더 조건이 **timeScale 전체 범위에서 미충족**. 원인 2건 확정 (reviewer L1 권고 + QA 실측 입증): (1) forward-diff velocity 추정이 기본 `timeScale=86400s/s` 조건에서 Io 주기 1.77일 대비 비선형 noise 과다, (2) `useEffect([..., observedFps])` 의존성 배열에 `observedFps` 포함 → fps 측정 raf 가 매 frame `setObservedFps` 호출 → polling useEffect 매 frame cleanup/재실행 → `setTimeout(step, 1s)` 실행 전 취소. `timeScale=1` 10초 대기 후에도 배지 미렌더 실측 확인. 훅·fps 폴백 단계 테이블·WASM wiring 인프라는 완결. 해결 2단계: (a) `observedFpsRef` 패턴으로 의존성 배열 제거, (b) Babylon 씬 저장 velocity state vector 직접 추출 (forward-diff 폐기). [P9-followup #263](https://github.com/coseo12/astro-simulator/issues/263) 이관. v0.9.0 에서는 D8 UI 는 정적 JSON 값 표시, `?mass=jupiter×N` 은 Newton 경로에서 씬 물리에 반영됨 (거리 감소 실측 확인) | PR #264 reviewer L1 권고 + QA 실 Chrome L3-b 실측 (timeScale=1 10초 대기 → 배지 0개). §재검토 조건 #3 (Osculating 왕복 오차) 의 구현 조건 확장                                                                              |
