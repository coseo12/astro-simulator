...
2. **sun anomaly 구조적 차단** (cross-validate 이견 수용 1 — 1회성 억제 폐기): 진입 시 (a) focus 로 완화된 `lowerRadiusLimit` **원복** + (b) **진입 시점 tier 를 현 radius 기준 확정**해 escalation 산식이 진입 radius 를 보존하도록 한다. "진입 직후 1회 억제" 는 첫 프레임 이후 휠 1칸/방향키에 snap-back 위험 → 폐기. 이후 **사용자 줌 시에만** tier escalation 정상 동작.
   > **#704 body-tier 외행성 위성 예외 (cross-link — ADR `20260618-704-body-tier-zoomout-jump.md`)**: 본 항목의 "사용자 줌 시 정상 escalate" 는 inner/solar 진입에는 유효하나, **외행성계 위성(io=목성계 5.2AU 등) body tier 진입**에는 회귀를 낳았다 — io 는 `cameraFromSun` 이 본질적으로 solar 영역이라 줌아웃 즉시 body→solar 직행 escalate → rescale 급락(158386→0.53, ≈300,000× 축소 = "태양계 전체로 튕김"). #704 (옵션 c) 가 body tier 진입 시 escalation 판정을 **anchor(탐색 위성) 기준 `tierFromFocus`** 로 전환해 줌아웃해도 body tier 를 유지(위성 근방 보존, escalate 0)한다. 즉 body-tier free-fly 줌아웃은 자동 escalate 하지 **않고** entryRadius×zoomoutFactor 상한에서 멈춘다(개요 복귀는 명시적 reset). inner/solar 경로는 본 §5-2 대로 정상 escalate 유지.
