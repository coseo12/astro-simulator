...
- ~~JD `2451808.0` 의 `NI` ∩ 육지 ∩ `iceMask` 포화 픽셀 수~~ — JD `2451626.0` 원자료 `landIce` 는 전 대역 `0` 이었다 (§A11.16 기각 1). ⏩ **해소 (2026-09-18, §A11.18.1)**: U1 JD 에서 재도 `0` 이고, 원인은 표본 부재가 아니라 **대역 정의와 카메라의 조합**이다 — 가드가 카메라를 적도면에 고정하므로 (`beta = π/2`) `ndv ≥ 0.6` 은 `|φ| ≤ 53.13°` 를 요구하고 극관 게이트는 `|lat| ≥ 56.61°` 에서 시작해 **교집합이 공집합**이다. 즉 이 픽셀은 이 카메라에서 어떤 JD 를 골라도 `NI` 에 들어오지 않는다 ⇒ MN-5b 의 **픽셀 검증 가능성은 구조적으로 없다** (단위 테스트가 잡는 자리). 실측 근거는 결함 렌더 `50 px` 전량 덤프 (`diag/m3-mn5b-defect.log`)
...
#### A11.17.9 V2 전제 술어 재정의 — **Provisional** (2026-09-18, §A11.17.5 재실측 조건 3 발화 결과)
...
**⚠️ 조건 5 는 원인을 이름으로 담고도 발화하지 않았다.** §A11.17.8 이견 5 (agy) 가 MSAA 를 조건 5 에 넣게 했으나 술어가 「**도입** · **변경**」 이었다. MSAA 는 이 저장소에서 **처음부터 켜져 있고 바뀌지 않았다** (`packages/core/src/engine/engine-factory.ts:43` `new Engine(canvas, true, …)` · `:30` `antialias: true`). ⇒ 조건 5 의 술어를 **「변경」 + 「현행 값이 `V1`~`V4` 의 전제를 깨는지」** 양쪽으로 넓힌다. 「가드 술어가 계약보다 좁다」 의 변형이고, 여기서는 **트리거를 변화량으로 쓴 것**이 축이었다.

**재정의 — `V2` 를 두 술어로 나눈다 (새 임계 `1` 개).**

| 술어 | 정의 | 건강판 | 역할 |
| --- | --- | ---: | --- |
| **V2-a** (주판정) | `d2 = (O − P2) − NA` 의 `> 1 LSB px` **`== 0`** | `0` | 국소·전역 overlay 고장 전부. **새 임계 없음** — 두 비교항의 합성 경로가 정렬돼 `== 0` 이 성립한다 |
| **V2-b** (백스톱) | `O ↔ P1` 의 `> 1 LSB px` **`≤ 3`** | `1` | `O` 와 `NA` 가 **같은 결함을 공유**하는 경우 — 차분 술어인 V2-a 가 눈이 머는 축 |
| (진단 인쇄 — 술어 아님) | `d1` 의 `> 1 LSB px` · 평균 기여 상대오차 `%` | `1` · `0.0184 %` | V2 위배 시 원인이 overlay 인지 `NA` 주입인지 가른다 |

- **V2-b 임계 `3` 의 도출**: 건강판 `1` 에 본 ADR 의 1/3 관례를 역방향 적용한 `1 × 3` 이다. 임계를 **발명하지 않기 위해** 관례를 따랐다 (§A11.17.8 이견 4 가 근거 없는 `50` 을 기각한 것과 같은 규칙). 백스톱이 잡아야 하는 공통 고장은 `172 px` 규모라 `3` 으로 충분하다 [실측 — 아래 격자].
- **`d1` 을 술어로 만들지 않는 이유**: `NA` 주입 고장은 `d2` 가 이미 `172~510 px` 로 잡는다 [실측]. `d1` 을 술어로 올리면 MSAA 꼬리에 새 임계가 하나 더 생긴다 — 진단 인쇄로 남기면 임계 증가 `0` 이면서 원인 구별은 유지된다.
- **`NA` 는 새 주입 경로다** (레시피 5 의 `sunIntensity 0 · ambientIntensity 0 · rimStrength 0 · nightLightStrength = S`). 그 주입이 틀리면 전제가 조용히 무의미해지는가 — **아니다.** `d1` 과 `d2` 는 `NA` 를 공통으로 빼므로 `NA` 가 `Δ` 틀리면 **둘이 같은 `Δ` 만큼 함께** 움직인다 (구조). [실측] `NA ×1.01` → `d1 172` · `d2 172` / `×1.1` → `511` · `510` / `×0.9` → `507` · `503`. overlay 고장 5종에서는 `d1` 이 건강판 `1` 로 불변이고 `d2` 만 `11~4104` 로 오른다 ⇒ 두 축이 분리된다.
- **CI 리스크 없음** [실측] — `V2` 는 D12 변이 하네스 전용이고 CI 는 `INJECT` 없이 기본 모드만 돌린다 (`.github/workflows/shader-pixel-guard.yml:215-220` — 같은 파일 주석이 「변이 주입은 CI 에 배선하지 않는다」 로 선언한다). 따라서 §A11.17.7 의 「CI 렌더러 미확인」 은 `V2` 판정에 닿지 않는다.

**판별력 격자** [실측 `diag/m1d` · `m1f` · `m1g` · `m1h`] — `NI_land n 4104` (건강판 lit `655`). 「영향 px」 = 고장 overlay 프레임이 건강 overlay 프레임과 8-bit 채널이 다른 픽셀 수.

| 고장 | 영향 px | **V2-b** (`O↔P1`) | (C) 상대오차 | **V2-a** (`d2`) | `d1` |
| --- | --: | --: | --: | --: | --: |
| 건강 | `0` | `1` | `0.0184 %` | **`0`** | `1` |
| 전역 — `S×1.01` | `445` | `172` | `1.0424 %` | `172` | `1` |
| 전역 — `S×1.05` | `557` | `444` | `5.0097 %` | `439` | `1` |
| 전역 — `ALPHA_COMBINE` | `4104` | `4104` | `132.65 %` | `4104` | `1` |
| 국소 — 주파수 `×(1+1e-4)` | `278` | `116` | **`0.0809 %`** | `113` | `1` |
| 국소 — 주파수 `×(1+5e-4)` | `359` | `268` | **`0.4627 %`** | `267` | `1` |
| 국소 — 밝은 `7×7` 전량 kill | `19` | `17` | `1.2998 %` | `16` | `1` |
| 국소 — 어두운 `7×7` 전량 kill | `14` | `12` | **`0.2062 %`** | `11` | `1` |
| 전제 — `NA` strength `×1.01` | `0` | `1` | `0.0184 %` | `172` | **`172`** |
...
2. **`> 2 LSB px == 0` 으로 완화 — 기각.** 건강판 (최대 `2`) 은 통과하나 최소 현실 고장 (`S×1.01`) 을 잡는 근거가 `chMax 3` 인 **`10 px` 뿐**이라 여유가 가장 얇다 [실측].
3. **판정 집합에서 삼각형 경계 픽셀 제외 — 기각.** 결함 픽셀도 함께 빠질 수 있고, 판정 집합을 렌더 구조로 정의하는 것은 #1197 M-i 인접이다.
4. **가드 렌더를 `antialias:false` 로 — 기각.** `V2` 는 `0 px` 로 통과하고 판별력도 유지되나 (`167`), **전 판정량이 바뀌어** baseline 전면 재도출이 필요하고 「가드가 제품 렌더 설정을 바꿔서 잰다」 는 새 비대칭을 들인다.

**미확인 (근거로 인용 금지)**

- 재정의한 `V2-a` · `V2-b` 로 **MN-6 을 실제로 통과시켜 D6 FAIL 을 확인한 실행** — 구현 전이다. `D6` 의 변이 검출력은 **아직 미확인**이고, 그것이 이 개정의 목적이다.
- `O` 와 `NA` 가 같은 결함을 공유하는 실제 고장 — V2-b 의 존재 이유는 **[추론, 미실측]** 이다 (`installOverlay` 가 strength 를 `host.material._floats.nightLightStrength` 에서 읽는다는 코드 판독). 주입해 보지 않았다.
- 국소 고장 2종은 **화면 좌표 사각형** 기반 합성 결함이다 — 제품 코드로 도달하는 국소 결함의 분포와 같은지는 재지 않았다.
- `d1` 의 건강판 `1 px` 이 다른 JD · 다른 카메라에서도 `≤ 2` 에 머무는지.
- CI 렌더러에서의 위 전 수치 (전부 로컬 swiftshader) — 단 위 「CI 리스크 없음」 대로 `V2` 판정 경로에는 닿지 않는다.

---

### A11.18 MN-5b 예측 반증 · MN-7 — **Provisional** (2026-09-18)
...
**구조적 이유** [실측 + 도출]: 가드는 카메라를 적도면에 고정하므로 (`camera.beta = π/2`) 위도 `φ` 픽셀의 `ndv ≤ cos φ` 이고, `ndv ≥ 0.6` 은 `|φ| ≤ 53.13°` 를 요구한다. 극관 게이트는 `iceLatLo 0.84` 에서 시작해 실측 `|lat| ≥ 56.61°` 다 ⇒ **이 카메라에서 두 대역의 교집합은 공집합**이다. ⚠️ 그 `56.61°` 는 게이트 맵 2장으로 `landMask` · `iceMask` 를 복원한 **대리 측정**이고 혼입이 있다 — disk 실루엣에서 MSAA 가 배경색 `(8,9,13)` 을 섞어 `iceMask` 를 가짜로 만든 `18 px` (`iceMask ∈ {0.004, 0.008, 0.012}` · `r/R ≥ 0.9959` · `|lat| < 50°`) 를 제외한 `278 px` 가 근거다. **정본은 결함 렌더 실측 `50 px`** 이고 대리 측정은 구조 근거로만 쓴다.

**처분 — 「단위 테스트가 잡는 자리」로 남긴다** (사용자 결정 2026-09-18). 가드 판정량은 **무변경**이다.
...
`pnpm --filter @astro-simulator/core build` 가 권한 분류기에 반복 차단돼 「소스 변경 + dist 재빌드」 경로를 쓸 수 없었다. 위 두 측정은 페이지 레벨 개입으로 대체했고 (MSAA = `getContext` 후킹 · MN-5b = JS 응답 본문 셰이더 치환) 개입 유효성과 판정량 등가를 매 건 실증했다. MN-5b 의 계약 문면 판본은 `mutations/mn5b.log` (소스 + 재빌드) 가 이미 갖고 있다. ⚠️ 남은 D12 (재정의한 V2 로 MN-6 실발화) 는 소스 변이가 필요할 수 있어 빌드 권한이 선행 조건이다.
