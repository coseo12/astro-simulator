...
> 정적 속성(값의 형태)을 재는 술어는 그 형태를 보존하는 실패에 눈이 먼다. rim 은 「가장자리가 밝다」로 재기 쉽다. 그 형태를 보존하는 실패 6종:
...
| G5     | 낮면 림 호 `3` 분할, 각 호 기여                              | `> 0`     | `0.07635` / `0.05360` / `0.09488` |

**측정 조건**: headless chromium (WebGL2) · `?gpu=a&focus=earth&lod=auto&rotate=off&orbits=off` · 1280×720 · JD 2451626.0 · `beta = π/2`. **5회 반복 전 실행이 소수 5자리까지 동일 — 산포 `0`.** 임계는 baseline 의 약 1/3 (G1~G3) 이고, G4 는 낮면 림 OFF 채도 `0.47536` 의 절반을 올림한 값이다.
...
1. 대역별 표본 수 `N` 하한 미달 (`150`; 실측 최소 대역 `867`)
...
| M-2 | 바인딩 블록만 삭제 (GLSL 무변경)       | **소스 변이 + `core` dist 재빌드**                       | `exit 0` | **`exit 1` — G1·G2·G3·G5 `0`**                | `exit 0` |
| M-3 | rim 항 전체 삭제                       | **소스 변이 + `core` dist 재빌드**                       | `exit 0` | **`exit 1` — G1·G2·G3·G5 `0`** (+ 단위 3건)   | `exit 0` |
| M-4 | rim 세기 `0` 고착                      | 런타임 (`rimStrength=0`)                                 | `exit 0` | **`exit 1` — G1·G2·G3·G5 `0`**                | `exit 0` |
| M-5 | (B) 렌더 순서 — **해당 없음** (미채택) | —                                                        | —        | —                                             | —        |
| M-6 | rim 세기 ×5                            | 런타임 (`rimStrength=2.5`)                               | `exit 0` | **`exit 1` — G4 `0.35939`** (나머지 4 PASS)   | `exit 0` |
...
**사각으로 남긴 것**: M-6 은 G4 만 잡고 G1·G2·G3 은 오히려 값이 커진다 (`0.20450` / `0.20751` / `0.19089`) — 단측 술어가 눈머는 방향이다. G4 가 없었다면 ⑤ washout 은 전 게이트 초록으로 통과했을 것이고, 이것이 G4 신설의 실측 근거다. **D1 핵심(광원각 의존)을 비우는 사각은 없다** — ① 은 G1 이 부호까지 뒤집어 잡는다.
...
⚠️ **가드 프레임은 이 축을 재지 못한다** — 결정적 프레임의 위상각은 **`90.0000°`** 로 3회 재실행 전부 동일했고, 저위상각은 사용자가 카메라를 돌려야 도달한다 (같은 JD 의 `(alpha, beta)` 12×11 격자에서 최소 **2.186°** / 최대 177.814° 실측). 즉 이 상수의 근거는 **가드가 아니라 제품 프레임 범위**이며, 그것이 위 실측을 가드가 아닌 임시 스크립트로 수행한 이유다.
