...
> ⚠️ 부기 (#1202, 2026-09-07): 본 표의 **「Tier 2-4 대기 fresnel rim」 행**의 「✗ 후속」은 **Amendment 8 에서 해소**됐다. (i) 은 Amendment 7 시점 `uniforms` 39개 중 18개(46.2%)가 이미 rocky 전용이라는 실측으로, (ii) 는 알파 블렌딩·log-depth 정합·셰이더 복제의 **비용 대 효용** 판정으로 각각 닫혔다 (§A8.1). 원문은 당시 결정의 기록으로 보존한다.
...
## Amendment 8 (2026-09-07) — 지구 대기 산란 rim: 공유 셰이더 혼입 (#1202)

**상태**: Provisional (cross-validate 결과 통합 후 Accepted 전이 — CLAUDE.md §ADR Status 워크플로. 본 Amendment 는 「ADR 신규·개정」 앵커에 해당한다)

**이슈**: [#1202](https://github.com/coseo12/astro-simulator/issues/1202) — §A3.7 **재검토 조건 2 (Tier 2-4 대기 fresnel rim)** 의 발동 결과다. 그 조문이 못박은 **「공유 셰이더 varying 혼입 vs 별도 대기 레이어 비교 선행」** 을 §A8.1 에서 판정으로 수행했다.

### A8.1 결정 0 (게이트) — (A) 공유 셰이더 혼입 채택 / (B) 별도 대기 레이어 보류

| #   | 축                          | (A) 공유 셰이더                                                             | (B) 별도 레이어                                                                                        | 우위 |
| --- | --------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ---- |
| 1   | 신규 mesh 수                | `0` (high/mid 가 이미 동일 material 공유 — `body-mesh-factory.ts:121`)      | `+2` (high `:63` segments 32 / mid `:116` segments 12 각각)                                            | (A)  |
| 2   | 알파 블렌딩                 | 불필요 (`procedural-planet-shader.ts` 의 `gl_FragColor` 알파 1 고정 OPAQUE) | 필수. 본 저장소는 body 표면 알파 블렌드를 **이미 비용 사유로 기각**했다 (`body-mesh-factory.ts:220`)   | (A)  |
| 3   | log-depth 정합              | 무변경 (기존 `vFragmentDepth` 재사용)                                       | 신규 정합 필요 — §A1.8 재검토 조건 5 가 지목한 축                                                      | (A)  |
| 4   | rim 이 disk 밖으로 번지는가 | 구조적 불가                                                                 | 가능                                                                                                   | (B)  |
| 5   | 구름 인프라 공유            | `0`                                                                         | 공유 가능                                                                                              | (B)  |
| 6   | varying·uniform 증분        | varying `+1` / auto-bind `+1` + 미학 상수                                   | 신규 셰이더 1식 (`world`·`logDepthConstant`·광원 8종 중복 선언·바인딩)                                 | (A)  |

**(B) 기각의 근거는 (A) 의 전축 우위가 아니라 비용 대 효용의 불균형이다.** (B) 는 지구 1개 body 의 림 표현을 위해 (i) 24 body 씬 전체에 알파 블렌딩 back-to-front 정렬 오버헤드를 되돌리고 (`body-mesh-factory.ts:220` 이 이미 같은 비용을 사유로 기각한 값이다), (ii) §A1.8 재검토 조건 5 가 「핵심 위험 1」로 미해결 표시한 log-depth ↔ 투명 정렬 정합 리스크를 새로 들이며, (iii) `world`·`logDepthConstant`·광원 8종을 중복 선언·중복 바인딩하는 셰이더 1식을 복제한다. (A) 의 대응 비용은 varying `+1` 이다.

⚠️ **설계 초판(2026-09-07)은 이 기각을 「(B) 가 이기는 축 4·5 의 산출물이 #1202 비-범위에 걸린다」로 논증했다. 그것은 순환이다** — 그 비-범위는 (A)/(B) 미결 상태에서 쓰였으므로 기각의 닻이 될 수 없다 (cross-validate 발견 3 수용, §A4.8 부수 1 과 같은 형태의 「결론 유지 · 근거 교체」). 비-범위 1·2 는 이제 기각 근거가 아니라 **재검토 조건**으로만 남는다.

**§A3.3 Tier 표 `대기 fresnel rim` 행의 반대 근거 (i) 에 대한 응답 [실측]**: 그 행은 *"4 타입 공유 셰이더에 rocky 전용 varying 혼입 = #774 결정 1 의 역방향 침범"* 을 들었다. 그러나 Amendment 7 시점 `uniforms` 배열 **39개 중 18개(46.2%)가 이미 rocky 전용**이고 유일한 sampler `uSurfaceMask` 도 그렇다 — 전부 mars/jupiter/moon 에 무조건 바인딩된다. 규약은 코드 주석에 명시돼 있다 (Amendment 7 바인딩 블록). "rocky 전용 동거 회피" 는 **Amendment 1·3·4·7 에서 4회 연속 포기된 규약**이며, #774 결정 1 의 분리는 sun↔planet (광원 모델 자체가 없는 emissive 셰이더) 사이의 것으로 계급이 다르다.

⚠️ **§A3.3 Tier 표의 해당 행 원문은 소급 치환하지 않는다** — 당시 결정의 기록이고 그것이 왜 부족했는지가 본 Amendment 의 배경이다 (§A7.7 선례, `reviewer.md` §4 계급 2). 행 위에 dated 부기만 더한다.

### A8.2 결정 1 — viewDir 은 per-fragment (`varying vec3 vWorldPos`)

기각한 대안: `uniform vec3 uViewDirWorld` (CPU 가 카메라→body 중심 방향 산출, varying `+0`).

**기각 근거 (기하)**: 카메라가 중심에서 거리 `d`, body 반경 `r` 이면 실루엣은 `dot(N, U) = r/d` 에서 생긴다 (`U` = 중심→카메라 단위벡터). 상수 viewDir 을 쓰면 `1 - dot(N, U)` 의 림 최댓값이 `1 - r/d` 가 되어 **rim 대역 폭과 세기가 카메라 거리의 함수**가 된다 — 줌하면 rim 이 변한다. `?focus=earth` 는 근접 관측이라 무시할 수 없다: 결정적 프레임 실측 **`r/d = 0.11547`**. per-fragment 는 실루엣에서 정확히 `0` 이라 거리 불변이며, `sun-shader.ts:95` 가 *"per-fragment 정확 시선각"* 으로 같은 이유를 박제했다.

### A8.3 결정 2 — `vWorldPos` 공간 계약: 시선 기저 전용

§A2.3 결정 2 의 `vNormal`(world) / `vLocalPos`(local) 분리는 **깨지지 않는다** — 기존 두 varying 의 공간을 하나도 바꾸지 않는다. `vWorldPos` 는 제3의 공간(world position)이고 `vNormal` 과 같은 좌표계다.

**계약**: `vWorldPos` 는 viewDir 산출 전용이다. **절차 패턴(대륙/밴드/크레이터/biome/마스크 UV)의 입력으로 사용 금지** — 쓰면 패턴이 공간에 고정돼 mesh 가 미끄러진다. §A2.3 이 `vLocalPos` 를 local 로 둔 이유 그 자체다. `procedural-planet-shader.test.ts` 의 U11 이 **FRAGMENT 안 `vWorldPos` 등장 횟수를 `2` 로 고정**해 (선언 1 + viewDir 1) 정적으로 가드한다.

### A8.4 결정 3 — rim 연산은 rocky 분기 안, 합성만 밖

Amendment 4 가 `continents` 를 같은 이유로 분기 안에 둔 선례. mars/jupiter/moon 의 fragment 추가 비용은 합성 3 FMA(`rim = 0.0`)뿐. **varying 보간 `+3 float` 은 4 타입 공통**이며 이것이 (A) 의 정직한 잔여 비용이다.

### A8.5 결정 4 — 합성은 `col *= shade` 뒤, 기존 `clamp` 앞. 연산자는 **가산**

```glsl
col += rimColor * rim;
```

`mix` 는 채택하지 않았다 — §A8.11 재검토 조건 5 에 **대기**시킨다 (폐기가 아니다).

### A8.6 clamp 헤드룸 — 옳은 구현을 FAIL 시킬 수 있는 제약

셰이더 JS 미러(`lightingShadeMirror`)로 낮면/밤면 림의 post-clamp 헤드룸을 쟀다 [미러 실측]:

| 표면                        | 낮면 휘도 헤드룸 | 밤면 휘도 헤드룸 | 밤면/낮면            |
| --------------------------- | ---------------- | ---------------- | -------------------- |
| ocean 얕은 (baseColor)      | 0.0811           | 0.9234           | 11.38×               |
| ocean 최심 (baseColor\*DEEP) | 0.4926           | 0.9648           | 1.96×                |
| land 온대 (LAND_COLOR)      | 0.0589           | 0.9340           | 15.86×               |
| land 열대 (TROPICAL)        | 0.1689           | 0.9437           | 5.59×                |
| tundra (BIOME_TUNDRA_RGB)   | 0.0000           | 0.9041           | ∞ (낮면 전 채널 포화) |
| ice 극관 (ICE_COLOR_RGB)    | 0.0000           | 0.8364           | ∞ (낮면 전 채널 포화) |

**가산 합성의 낮면 상한 정리** [미러 실측]: 낮면 rim 의 ON−OFF 휘도 차분은 **`RIM_COLOR` 와 `RIM_STRENGTH` 에 무관한 천장**을 갖는다 — 그 표면색의 휘도 헤드룸이다. land 온대는 rim `r = 0.5` 에서 이미 `+0.0589` 로 천장에 닿고, `r = 2` 로 4배 올려도, `RIM_COLOR` 를 `(1,1,1)` 로 바꿔도 `+0.0589` 그대로다. ⇒ **G1 의 `M` 과 G2 의 `τ` 에는 절대 상한이 있다.**

⚠️ **`tundra` 와 `ice 극관`은 그 천장이 정확히 `0` 이다.** 실루엣은 대원이고 황도면 근처 시선에서는 극을 지나므로, 낮면 림 밴드에 극관이 들어오면 그 픽셀 기여는 `0` 이고 낮면 평균을 희석한다. 표본 수 `N` 이 충분해도 측정이 무의미해지는 구간이 실재한다 ⇒ §A8.8 「측정 불가」.

⚠️ **반직관**: rim 세기를 올리면 낮면 기여는 헤드룸에서 멈추지만 밤면 기여는 계속 자란다 — **최적 세기가 존재하고 그 너머는 마진이 단조 악화**한다. "안 보이니 세게" 가 정확히 게이트를 깨는 방향이다.

**[GPU 실측 확인]** 위 미러 정리를 실측이 재확인했다. `rimStrength` 를 `RIM_STRENGTH` 의 ×1~×5 로 스윕할 때 낮면 림의 **휘도 기여는 `0.07499 → 0.20751` (2.77배)** 에서 멎는데 (5배 입력 대비 열화된 수익), **채도 감소량은 `0.07479 → 0.35939` (4.80배)** 로 커진다. 세기 상향은 얻는 것보다 잃는 것이 빨리 커진다.

### A8.7 형태 보존 실패 모드 열거 + 술어 도달 판정

> 정적 속성(값의 형태)을 재는 술어는 그 형태를 보존하는 실패에 눈이 먼다. rim 은 「가장자리가 밝다」로 재기 쉽다. 그 형태를 보존하는 실패:

| #   | 실패 모드                                            | 형태 보존 | 술어 도달                                                                                                                                                                                    |
| --- | ---------------------------------------------------- | :-------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ①   | 광원각 무시 (순수 fresnel)                           |   보존    | **G1 도달 [실측]** — 변이 M-1 에서 G1 이 `-0.07396` 으로 부호까지 뒤집혔다                                                                                                                    |
| ②   | 바인딩 없이 uniform 기본값 잔존                      |   보존    | **G2 도달 [실측]** — 변이 M-2 에서 `0.00000`. ⚠️ 같은 변이에서 `packages/core` 단위 테스트 **1075건이 전건 통과**했다 (GLSL 소스 무변경이라 정적 assert 가 전부 눈이 먼다). #1197 M-h 동형 축 |
| ③   | 밝기는 맞고 **위치가 틀림** (rim 이 disk 전체/중심)  |   보존    | **G1 미도달** — 낮/밤 분할은 그대로 통과한다. ⇒ **G3(반경 프로파일) 신설로 닫음**                                                                                                            |
| ④   | clamp 포화로 낮면 기여 `0`                           |     —     | **술어가 도달하는 게 아니라 술어를 오작동시킨다** (옳은 구현이 FAIL). 게이트로 못 닫음 ⇒ 진단 헤드룸 인쇄 + §A8.8 「측정 불가」로 회피                                                        |
| ⑤   | 색상 탈색 (washout) — 플라스틱 하이라이트로 읽힘     |   보존    | **G1·G2·G3 전부 통과.** 가산에서는 가설이 아니라 구조적 귀결이다 (§A8.6 상한 정리) ⇒ **G4(채도 비붕괴) 신설 + M-6.** [실측] M-6 에서 G4 `0.35939` FAIL, 나머지 4 게이트는 전부 PASS          |
| ⑥   | 시간적 지글거림 — 림 밴드가 얇으면 자전 시 모아레    |   보존    | 게이트 아님. **밴드 두께 진단 인쇄** ([실측] 낮면 rim 기여 반경 프로파일의 반치폭 **7px** / disk 반경 98.32px) + 실 Chrome GUI 자전 육안 확인 1회 ⇒ `RIM_FALLOFF` 상한                        |
| ⑦   | 방위 편중 — rim 이 낮면 호 한쪽에만 몰림             |   보존    | **게이트 불가 [실측]** — 아래 M-8 참조. 초판은 G5(호 3분할 `> 0`)로 닫았다고 봤으나, 편중이 극단에 이른 상태에서도 G5 는 PASS 하고 (`0.00017 · 0.04576 · 0.00052`) 그 지점에서 **G2 가 먼저 FAIL** 한다 (`0.01543`). ⇒ **G5 삭제**, ⑥ 과 같은 처분(호 분할 진단 인쇄 + GUI 육안)                 |
| ⑧   | `RIM_NDL_LO` 부호 — terminator 박명 호 소멸          |   보존    | **G1~G4 전부 미도달 [실측]** — `LO ∈ {-0.25 … +0.05}` 전 구간에서 낮면 림 기여가 `0.07499` 로 **불변**이고 G1 은 오히려 좋아진다. 구조 원인: 낮면 대역 하한(`0.15`)이 셰이더 `RIM_NDL_HI` 와 같아 smoothstep 전이 구간이 두 대역 **사이에** 빈다 ⇒ **G6(박명 호) 신설 + M-7**       |

**③ 은 #1202 계약의 D1 술어에서 본 Amendment 가 새로 찾아낸 사각이다.** 계약이 고정한 「림 밴드를 낮면/밤면으로 나눈 차분」은 ③ 을 잡지 못한다. 계약 D2 의 *"핵심 계약을 비우는 사각이면 게이트를 보강한다"* 에 따라 **G3 을 추가**했다 (재는 것을 바꾸지 않고 축을 하나 더한다).

**⑦·⑧ 은 리뷰 라운드에서 추가된 행이고, 둘의 처분이 정반대인 것이 요점이다.** ⑧ 은 **이 Amendment 가 Phase 0 을 통째로 쓴 상수(`RIM_NDL_LO`)의 근거를 지키는 게이트가 하나도 없던** 상태였다 — 근거는 §A8.13 에 실측으로 박제됐는데 그 근거를 검사하는 술어가 없었다(#1127 「가드 이름이 LOD 인데 LOD 판정이 없음」과 같은 클래스). ⑦ 은 반대로 **게이트가 먼저 있고 실패 모드 행이 없던** 경우이며, 실측해 보니 겨냥한 것을 못 잡고 잡는 것은 G2 중복이었다(#1197 D5-b 동형). 「내가 만든 게이트」는 보존 근거가 아니다.

### A8.8 게이트 · 「측정 불가」 · 변이 3단 실증

가드: `apps/web/scripts/browser-verify-1202-atmosphere-rim.mjs` (`verify:1202-atmosphere-rim`). 표본 정의는 **전적으로 기하**다 (`dot(N, viewDir)` · `dot(N, sunDir)`) — `verify:1119` 의 색 기반 분류 술어(`b < g`)는 **재사용하지 않았다** (#1197 M-i 사각의 출처, #1202 비-범위 5).

| 게이트 | 술어                                                        | 임계     | 실측 baseline |
| ------ | ----------------------------------------------------------- | -------- | ------------- |
| G1     | 낮면 림 rim 기여 − 밤면 림 rim 기여                          | `≥ 0.025` | `0.07438`     |
| G2     | 낮면 림 rim 기여                                             | `≥ 0.025` | `0.07499`     |
| G3     | 낮면 림 기여 − 낮면 내부 기여                                | `≥ 0.025` | `0.07141`     |
| G4     | 낮면 림 채도 감소량 (`satOFF − satON`)                       | `≤ 0.24`  | `0.07479`     |
| G5     | **결번** — 낮면 림 호 `3` 분할 각 호 `> 0`                   | —         | 리뷰 라운드에서 **삭제** (§A8.7 ⑦) |
| G6     | **박명 호** rim 기여 (`-0.15 < ndl < 0` 인 림 픽셀)          | `≥ 0.02`  | `0.06086` (`N = 97`) |
| G7     | 런타임 콘솔 에러 수                                          | `== 0`    | `0`           |

**측정 조건**: headless chromium (WebGL2) · `?gpu=a&focus=earth&lod=auto&rotate=off&orbits=off` · 1280×720 · JD 2451626.0 · `beta = π/2`. **5회 반복 전 실행이 소수 5자리까지 동일 — 산포 `0`.** 임계는 baseline 의 약 1/3 (G1~G3, **G6 도 동일 관례** — `0.06086` 의 1/3) 이고, G4 는 낮면 림 OFF 채도 `0.47536` 의 절반을 올림한 값이다.

**G6 의 대역 경계에는 새 임계가 없다** — 하단은 기존 `NIGHT_NDL_MAX = -0.15`, 상단은 terminator 정의값 `ndl = 0` 이다. 이 대역은 **기존 세 대역이 구조적으로 비워 둔 구간**이고 (`DAY_NDL_MIN = 0.15` 가 셰이더 `RIM_NDL_HI` 와 같아 smoothstep 전이 구간 전체가 낮/밤 사이에 빈다), 그래서 `RIM_NDL_LO` 의 부호가 유일하게 드러난다. 좁은 대역이라 표본이 `97` 로 낮/밤(`867`)의 1/9 이므로 전용 표본 하한(`30`)을 둔다.

**G7 은 인쇄를 판정으로 승격한 것이다.** 종전에는 `consoleErrors` 를 세어 인쇄만 하고 게이트하지 않았다 — 픽셀 게이트가 전건 초록인 채로 WebGL 컴파일 경고·런타임 예외가 나는 상태를 통과시킨다. 판별력은 M-9 로 실증했다.

⚠️ **설계 스케치의 예측값을 옮겨 적지 않았다** — 위 숫자는 전부 가드의 GPU 실행 출력이다 (Amendment 7 에서 설계 예측 `0.4477` 대 GPU 실측 `0.2317` 로 절반이었던 전례).

**「측정 불가」 ≠ FAIL** — 아래 넷 중 하나라도 걸리면 가드는 PASS/FAIL 이 아니라 「측정 불가」로 즉시 종료한다. ⚠️ **종료 코드는 `2` 이며 `0` 이 아니다** (CLAUDE.md §가드 설계 원칙 — fallback 분기 절대 금지):

1. 대역별 표본 수 `N` 하한 미달 (`150`; 실측 최소 대역 `867`) — 박명 대역만 전용 하한 `30` (실측 `97`)
2. 낮면 림 밴드 평균 잔여 휘도 헤드룸 하한 미달 (`0.05`; 실측 `0.28218`) — §A8.6 극관 지배 구간
3. 측정 프레임 위상각 하한 미달 (`10°`; 실측 `90.0000°`) — §A8.13
4. **낮면 내부 평균 휘도(OFF) 하한 미달** (`0.15`; 실측 `0.76561`) — 측정 대상(절차 표면)이 화면에 없는 상태

⚠️ **4항은 설계에 없었고 dev 가 실행 중 발견해 추가했다.** 설계가 열거한 1~3 이 **전부 통과하는데 아무것도 재고 있지 않은 구간**이 실재한다 — `?gpu=a` 없이 돌려 tier-c 로 떨어진 실행에서 표본은 `N = 867` 로 정상이었고 헤드룸은 `0.964` 로 **하한보다 컸는데** 모든 대역 rim 이 `0` 이었다. 즉 1~3 은 **방향이 반대라** 이 상태에 눈이 멀다. 이는 D1 이 **재는 것**을 바꾸지 않는다 — 측정의 **유효성 전제**다.

**「측정 불가」 발화 횟수: 본 PR 의 전 실측에서 `0`.** 강제 발화 시험(하한을 실측값 위로 임시 상향)에서 전제 (3)·(4) 각각 `exit 2` 를 확인했다.

**변이 3단 실증 (원본 PASS · 변이 FAIL · 원복 PASS)** — 주입 가능성이 갈린다:

| #   | 변이                                   | 주입 방식                                                | 원본     | 변이                                          | 원복     |
| --- | -------------------------------------- | -------------------------------------------------------- | -------- | --------------------------------------------- | -------- |
| M-1 | 광원각 항 삭제 (순수 fresnel)          | 런타임 등가 (`rimNdlLo=-2.0` / `rimNdlHi=-1.9`)          | `exit 0` | **`exit 1` — G1 `-0.07396`**                  | `exit 0` |
| M-2 | 바인딩 블록만 삭제 (GLSL 무변경)       | **소스 변이 + `core` dist 재빌드**                       | `exit 0` | **`exit 1` — G1·G2·G3 `0`**                   | `exit 0` |
| M-3 | rim 항 전체 삭제                       | **소스 변이 + `core` dist 재빌드**                       | `exit 0` | **`exit 1` — G1·G2·G3 `0`** (+ 단위 3건)      | `exit 0` |
| M-4 | rim 세기 `0` 고착                      | 런타임 (`rimStrength=0`)                                 | `exit 0` | **`exit 1` — G1·G2·G3·G6 `0`**                | `exit 0` |
| M-5 | (B) 렌더 순서 — **해당 없음** (미채택) | —                                                        | —        | —                                             | —        |
| M-6 | rim 세기 ×5                            | 런타임 (`rimStrength=2.5`)                               | `exit 0` | **`exit 1` — G4 `0.35939`** (나머지 전부 PASS) | `exit 0` |
| M-7 | `RIM_NDL_LO` 부호 뒤집기               | 런타임 (`rimNdlLo=+0.05`)                                | `exit 0` | **`exit 1` — G6 `0.00000`** (G1~G4·G7 전부 PASS) | `exit 0` |
| M-8 | 방위 편중 (`ndl` 창을 위로 밀기)       | 런타임 (`rimNdlLo/Hi` 창 스윕)                           | `exit 0` | 창 `0.85` 에서 `exit 1` — 단 **FAIL 은 G2 이고 G5 는 PASS** (아래 표) | `exit 0` |
| M-9 | 런타임 콘솔 에러 1건                   | 런타임 (`console.error` 1회 주입)                        | `exit 0` | **`exit 1` — G7 `1`** (픽셀 게이트 전부 PASS) | `exit 0` |

**M-2 가 이 가드의 존재 이유다** — 그 변이에서 `packages/core` 단위 테스트 **1075건이 전건 통과**했고 (GLSL 소스가 무변경이므로 정적 assert 가 구조적으로 눈이 멀다) 오직 픽셀 게이트만 FAIL 했다. M-3 은 반대로 단위 3건(U12/U12-b/U12-c)과 픽셀 게이트가 **둘 다** 잡는다.

⚠️ **M-2·M-3 은 G6 신설 이전 라운드의 실측이라 그 표에 G6 값이 없다 — 추정으로 채우지 않는다.** 두 변이의 상태(rim ≡ 0)는 **M-4 가 런타임으로 정확히 재현**하며 거기서 G6 `0.00000` 을 실측했다. 표의 M-2·M-3 행은 당시 측정된 게이트만 적는다.

**M-7 이 G6 신설의 근거다** — `RIM_NDL_LO` 를 `+0.05` 로 뒤집어도 **G1~G4 가 전건 PASS** 이고 G1 은 `0.07438 → 0.07499` 로 오히려 좋아진다. 단위 테스트도 통과한다 (U18 은 「음수」만 요구하고, `+0.05` 가 아닌 `-0.05` 라면 그마저 통과한다). §A8.13 이 Phase 0 을 통째로 써서 확정한 상수의 근거를 **아무 술어도 지키지 않던** 상태였다.

**M-8 은 게이트를 하나 늘린 게 아니라 하나 지운 근거다** [실측, 창 하단 `0.30 / 0.50 / 0.70 / 0.85 / 0.90`]:

| 창 하단 | 낮면 호 3분할 기여                    | G5 (`> 0`) | 낮면 림 기여 | G2 (`≥ 0.025`) |
| ------: | ------------------------------------- | :--------: | -----------: | :------------: |
|  `0.30` | `0.07635` · `0.05360` · `0.09388`     |    PASS    |    `0.07466` |      PASS      |
|  `0.50` | `0.06006` · `0.05360` · `0.07230`     |    PASS    |    `0.06201` |      PASS      |
|  `0.70` | `0.02505` · `0.05360` · `0.03499`     |    PASS    |    `0.03786` |      PASS      |
|  `0.85` | `0.00017` · `0.04576` · `0.00052`     |  **PASS**  |    `0.01543` |    **FAIL**    |
|  `0.90` | `0` · `0.02512` · `0`                 |    FAIL    |    `0.00835` |      FAIL      |

편중이 **호 하나에 사실상 전부 몰린** `0.85` 에서도 G5 는 통과한다 (`> 0` 은 근사-0 을 거르지 못한다). 호가 정확히 `0` 이 되는 `0.90` 에서는 G2 가 이미 FAIL 이다. ⇒ **G5 가 단독으로 발화하는 구간이 없다** — 겨냥한 것(방위 편중)은 못 잡고 잡는 것은 G2 중복이다. 삭제하고 호 분할은 진단 인쇄로 남겼다 (§A8.7 ⑦).

**사각으로 남긴 것**: M-6 은 G4 만 잡고 G1·G2·G3 은 오히려 값이 커진다 (`0.20450` / `0.20751` / `0.19089`) — 단측 술어가 눈머는 방향이다. G4 가 없었다면 ⑤ washout 은 전 게이트 초록으로 통과했을 것이고, 이것이 G4 신설의 실측 근거다. **D1 핵심(광원각 의존)을 비우는 사각은 없다** — ① 은 G1 이 부호까지 뒤집어 잡는다. 방위 편중(⑦)은 **게이트로 닫지 못한 채 남긴 축**이고 진단 인쇄 + GUI 육안이 그 자리를 대신한다.

### A8.9 결정 5 — body 별 파라미터화 판정: **earth 전용**

rim 이 rocky 분기 안에 있고 `SURFACE_TYPE_BY_BODY` 의 Rocky 는 earth 하나뿐이라 구조적으로 earth 전용이다. `DEEP_OCEAN_FACTOR` 가 같은 축에서 earth 전용으로 결론난 §A7.7 선례 동형.

⚠️ 이 「구조적」은 조건부다 — 다른 body 를 Rocky 로 재분류하면 **대기 없는 body 가 rim 을 상속한다** (금성=농밀 / 화성=희박 / 달=없음, 셋의 요구가 다르다). 상수 주석에 적용 범위를 명시했고 아래 재검토 조건 3 을 신설한다.

**재검토 조건 3 이 발동해도 셰이더 변경은 0 이다** — rim 파라미터는 이미 uniform 이므로 body 별 값 교체만으로 확장된다. 따라서 지금 body 별 파라미터화를 **선제 구축하지 않는다** (cross-validate 발견 8 기각: 소비자 없는 인프라 + 검사 표면 +1). `if (rimStrength > 0.0)` 스킵 분기도 기각한다 — fragment 동적 분기이고, rocky = earth 하나뿐이라 한 번도 스킵되지 않는다.

### A8.10 비용

| 항목          | 전  | 후  | 증분                                          |
| ------------- | --- | --- | --------------------------------------------- |
| uniform       | 39  | 45  | **+6** (그중 `cameraPosition` 은 auto-bind — 수동 바인딩 `+5`) |
| varying       | 3   | 4   | +1 (`vWorldPos`)                              |
| sampler       | 1   | 1   | 0                                             |
| `fbm(`        | 5   | 5   | **0**                                         |
| `texture2D(`  | 1   | 1   | **0**                                         |

⚠️ uniform 증분에 `rimFalloff` 가 포함된다. **`RIM_FALLOFF > 0` 을 단위 테스트(U16)로 assert 한다** — `0` 이 바인딩되면 `pow(0.0, 0.0)` 이 GLSL 명세상 미정의다 (cross-validate 발견 7 의 잔여 수용). 셰이더 방어 분기는 넣지 않는다: `rimGeom = 1.0 - clamp(dot, 0.0, 1.0)` 이 `[0,1]` 을 보장하고, `normalize(cameraPosition - vWorldPos)` 는 `sun-shader.ts:129` 가 가드 없이 출하 중이며 이 저장소는 같은 계급의 미정의(`atan(0,0)` 극점)를 의도적으로 미조치로 결정한 선례가 있다.

### A8.11 결과 · 재검토 조건

**결과** [실측]: `?focus=earth` 에서 지구 가장자리에 대기 산란 rim 이 보이고 **낮면이 밤면보다 밝다** (균일한 테두리 링이 아니다). mars / jupiter / moon / 단색(venus) / sun 렌더 영역 픽셀 diff **`0`**, `?surface=off` **`0`** (base↔feature). earth 는 양성 대조로 `12661 px` 변화.

결정적 프레임 rim ON / OFF (headless WebGL2, 1280×720):

![1202 rim ON 1280x720](../reports/1202-atmosphere-rim/1202-rim-on-1280x720.png)

![1202 rim OFF 1280x720](../reports/1202-atmosphere-rim/1202-rim-off-1280x720.png)

실 Chrome GUI 제품 프레임 (WebGPU 백엔드, 자전 ON, 위상각 107.025°):

![1202 실 Chrome GUI 제품 프레임 1280x720](../reports/1202-atmosphere-rim/1202-gui-product-webgpu-1280x720.png)
...
5. **clamp 포화로 게이트 운용이 불가능해짐** — 낮면 림 밴드 평균 헤드룸 하한 미달로 §A8.8 「측정 불가」가 상시 발생하는 상태. 그때는 **합성 연산자와 판정량을 함께** 재검토한다 (가산 → mix / screen, 그리고 부호 있는 휘도 차분 → 절대값·색거리·RGB 거리 등). ⚠️ **판정량 변경은 CLAUDE.md §스프린트 계약 5항에 따라 사용자 명시 합의 대상이다.**
...
| --------: | ---------------------------: | -------------------: | --------------------------: |
| 2.186°    | **0.1533**                   | 0.0381               | **0.1533**                  |
| 11.663°   | 0.3139                       | 0.2022               | 0.3139                      |
| 28.724°   | 0.5786                       | 0.4806               | 0.5786                      |
| 90.000°   | 0.9933                       | 1.0000               | 0.9933                      |
| 177.814°  | **−0.0775**                  | 0.0381               | **−0.0775**                 |

`r/d = 0.11547` (`asin = 6.631°`) 이다. **도달 가능한 최저 위상각(2.186°)에서도 실루엣 `ndl` 최댓값이 `0.1533` 으로 `SOFT_TERMINATOR_WIDTH = 0.12` 를 넘는다** — `sunFactor == 1` 인 호가 존재한다. 직교 근사는 α = 177.814° 에서 **부호까지 틀린다**.

**제품 프레임 실측으로도 확인된다**: 위상각 2.186° 프레임에서 림 밴드 평균 rim 기여는 `RIM_NDL_LO = -0.25` 가 `0.04662`, `+0.05` 가 `0.04614` — **차이 1.0%. rim 은 사라지지 않는다.**

**그럼에도 `RIM_NDL_LO` 를 음수로 두는 실측 근거는 따로 있다 — terminator 박명 호다.** 위상각 90° 프레임의 림 밴드(`ndv <= 0.25`)를 `ndl` 로 구간화하면:

| `ndl` 구간 | n   | `LO = -0.25` | `LO = +0.05` |
| ---------: | --: | -----------: | -----------: |
| −0.2       | 65  | `0.00813`    | `0`          |
| −0.1       | 66  | `0.04764`    | `0`          |
| 0.0        | 62  | `0.10006`    | `0`          |

즉 `LO >= 0` 은 **terminator 를 넘어가는 박명 호를 통째로 지운다.** 그 대가는 밤면 림 평균 기여 `0.00061` 이며, G1 실측 마진 `0.07438` 의 0.8% 다.

⚠️ **가드 프레임이 재지 못하는 것은 「저위상각」 축이다** — 결정적 프레임의 위상각은 **`90.0000°`** 로 3회 재실행 전부 동일했고, 저위상각은 사용자가 카메라를 돌려야 도달한다 (같은 JD 의 `(alpha, beta)` 12×11 격자에서 최소 **2.186°** / 최대 177.814° 실측). 위 **반증** 실측(직교 근사)을 가드가 아닌 임시 스크립트로 수행한 이유다.

⚠️ **부기 (리뷰 라운드): 박명 호 축은 그렇지 않다.** 그것은 `90°` 결정적 프레임에서 그대로 측정되며, 리뷰 라운드에서 **G6 (박명 호 rim 기여 `≥ 0.02`) 로 가드에 편입**했다 (§A8.8 · §A8.7 ⑧). **종전에는 이 절의 근거를 지키는 술어가 하나도 없었다** — 단위 테스트 U18 은 「음수」만 요구해 `LO = -0.05` 를 통과시키는데, 그 값은 박명 호를 실측 `0.00254` / baseline `0.06086` = **4.2%** 만 남긴다. 「Phase 0 을 통째로 쓴 상수의 근거가 박제만 되고 검사되지 않는」 상태는 #1127 과 같은 클래스다.

⚠️ `RIM_NDL_LO` 를 더 내릴수록 밤면 기여가 커져 D1 마진이 깎인다 — **「박명 호 ↔ D1 마진」의 단일 trade-off 노브**다.

**기각한 대안 (cross-validate 발견 2)**: `dot(-viewDir, uSunDirection)` 전방 산란 항 추가. 새 물리 모델이라 #1202 비-범위 3(레일리·미 물리 정확도)이고, 그 세기 축이 곧 §A8.7 ⑤ washout 방향이며, 튜닝 자유도를 하나 더 만든다. **본 결정은 새 항 없이 기존 파라미터의 부호만 연다.**

### A8.14 사후 — 「설계의 기하 논증」도 실행으로 검산해야 한다
...
- **Visual Fidelity — shell 반경비 자체가 rendering 왜곡이다** (cross-validate 메인 고유 발견 C2, [principles.md §1](../architecture/principles.md)). 메인 재분석 인용: 구름 상층 `~10 km` / 지구 반경 `6378 km` ≈ `1.0016` (architect 미대조), Phase 0 프로토 `1.02` 는 그 약 `12` 배. 적용 트리거는 §1 트리거 1 (가시성 임계 미달) 동형이다 — [도출] 결정적 프레임 disk 반경 `98.3 px` (§A10.12 실측) × `0.0016` ≈ `0.16 px`, 즉 물리 비율이면 shell 이 표면과 서브픽셀 거리라 림 바깥 돌출이 보이지 않는다. **§의무 체크리스트 4항목**:
  1. **데이터 SSoT 보존** — 반경비는 `packages/core/src/scene/` 의 rendering-only 상수다. `solar-system.json` 변경 `0`, 구름 고도 데이터 필드 신설 `0`.
...
**결정 (cross-validate 2026-09-11 개정 — 메인 C1 · U3 반영)** — 렌더링 그룹 0 의 투명 정렬 함수를 **정렬 키 치환**으로 교체한다. 모든 투명 submesh 의 정렬 키는 사전식 3-튜플 `(alphaIndex, distance, rank)` 이다:

- **host 계열** — earth host · mid variant · low billboard · 구름 — 은 `alphaIndex` 와 `distance` 를 **host 의 값으로 치환**한다. `rank` 는 구름 `1`, 나머지 `0`.
- **계열 밖 mesh** 는 자기 `(alphaIndex, distance)` 와 `rank 0` 이다 — Babylon `defaultTransparentSortCompare` 와 **같은 결과**를 낸다.
- **비교 순서**: `alphaIndex` 오름차순 → `distance` 내림차순 (back-to-front) → `rank` 오름차순 → 끝까지 같으면 `0` (안정 정렬이 삽입 순서를 보존).
- **계열 판정은 mesh 식별자 집합 조회**다 — 이름 문자열 비교 금지. 구름·host 는 생성 시, lazy 생성되는 mid·low 는 `getVariantMesh` 의 생성 지점에서 집합에 등록한다. 조회는 `O(1)`.
- **host 의 `distance`** 는 Babylon 이 쓰는 식과 같다 — `Vector3.Distance(boundingSphere.centerWorld, cameraPosition)` (`renderingGroup.js` `_RenderSorted`). host 가 숨겨져 큐에 없을 때도 host 의 bounding 정보에서 직접 계산한다 (host 는 `setEnabled(true)` 유지라 world matrix 가 갱신된다 — 구현 시 확인).
...
**왜 이 형태인가** — 초판의 「거리를 보지 않는 쌍 규칙」 은 `d_A < d_C < d_B` 인 제3 mesh `C` 가 있으면 비교가 **순환**할 수 있었다 (A·B 는 중심이 같아 그 구간 폭이 부동소수 잡음 수준이라 실무상 거의 불가능하나, 형태로 제거할 수 있다 — cross-validate Q1 / 메인 C1). 사전식 키는 **엄격 약순서가 구조적으로 성립**하고, 계열이 한 블록으로 모이며 구름이 블록 끝에 온다. 계열 내부의 host · mid · low 순서는 `rank` 가 같아 **현행 삽입 순서 그대로**다 — 기존 fade 창 픽셀을 바꾸지 않고 구름만 블록 끝에 얹는다.

**명세 공백 해소 (cross-validate U3)**:

- **(a) `alphaIndex` 와의 선후** — 치환이 **먼저**다. 계열 전원이 host 의 `alphaIndex` 를 첫 키로 쓰므로 구름 자신의 `alphaIndex` 는 순서에 쓰이지 않는다. 구름에 `alphaIndex` 를 따로 주는 코드는 금지한다 (효과가 없고 오해만 만든다).
- **(b) low billboard 포함** — 포함한다. `setVariantAlpha` 는 level 과 무관하게 적용되고 (메인 독립 확인), billboard 는 `StandardMaterial` 이라 alpha 가 실제로 블렌드되며 fade 중 투명 큐에 들어간다. host 자식이고 `position (0, 0, 0)` 이라 중심이 같다 (`body-mesh-factory.ts` `createBodyBillboard` — 구조상 동일, 거리 값은 미실측).

**단위 테스트** — 정렬 함수에 대해 (1) 계열 밖 쌍은 `defaultTransparentSortCompare` 와 부호가 같다 (2) 구름은 같은 계열의 host · mid · low 보다 항상 뒤 (거리 잡음을 넣어도) (3) 반사성·반대칭·추이성 — 무작위 submesh 스텁 3-조합 전수로 엄격 약순서를 확인한다. C5 의 fade 재현은 `material.alpha` 기전에 **의도적으로 결합**돼 있다 — 결정 4 가 그 기전 때문에 존재하므로 기전이 바뀌면 결정 4 자체가 재검토 대상이다 (코드 주석에 기록).

**기각**:

- ❌ **host 계열을 불투명 큐에 머물게 하기** (`transparencyMode = MATERIAL_OPAQUE` — `Material.needAlphaBlendingForMesh` 가 `_hasTransparencyMode` 를 먼저 보므로 `alpha < 1` 이어도 불투명 큐. cross-validate U1). 원인 (투명 큐 진입) 을 고치는 방향이라 **실측으로 비교했다** [§A10.12-U1]:
  - **(i) fade 창 픽셀이 바뀌고 새 아티팩트가 생긴다.** 현행 fade 창은 disk 전면이 mid 정착 프레임과 같다 (`onlyM 28085` · `neither 0` · 행 방향 H/M 전환 `0`). 불투명 강제는 high · mid 두 구가 depth 로 경쟁해 대부분 high (`onlyH 27723`) 이면서 **어느 variant 정착 프레임에도 없는 픽셀 `139`** 와 mid 조각 `223`, 행 방향 전환 `469` 회가 생긴다 — 테셀레이션이 다른 두 구의 depth 교차 패턴이다. 정지 프레임에서는 독립 2회 로드 결정적 (`0 px`) 이나, 줌이 LOD 를 넘기는 **카메라 이동 중** 거동은 미실측이다.
  - **(ii) 범위** — 절차 셰이더 전 body 에 걸면 mars fade 창도 바뀐다 (`onlyH 13622` · `onlyM 27` · `neither 3` · 전환 `45`). 계약 D6 (비-earth 무회귀) 과 충돌하므로 가능한 범위는 earth 전용뿐이다.
  - **(iii) earth 전용으로 채택해도 결정 4 가 없어지지 않는다** — low billboard 는 `StandardMaterial` 이라 투명 큐에 남고 구름과 중심이 같다. 삽입 순서 의존이 billboard 쪽에 남는다. 그리고 fade 창의 알파 무반응은 **범위 밖 기존 거동**이라 (메인 처분: PR 코멘트 기록), 그 픽셀을 바꾸는 것은 범위 이탈이다.
  - 구름 존재 자체는 불투명 강제에서도 복원됐다 (구름 유/무 full frame `31643 px` vs 양성 대조 `31622`) — **기각 사유는 구름이 아니라 지구 표면 쪽에 있다.**
...
- ❌ `alphaIndex` — 기본값이 `Number.MAX_VALUE` (실측 `1.7976931348623157e+308`) 라 구름 쪽을 **올려서** 뒤로 보낼 수 없다. host 계열을 내리면 전역 순서가 바뀐다. cross-validate 대안 B 의 「host `100` / 구름 `200` 계층화」 도 같은 사유다 — 지구가 **거리와 무관하게** 다른 모든 투명 mesh (`MAX_VALUE`) 보다 먼저 그려져, 지구 **뒤**에 있는 달의 fade variant 가 지구 위에 그려진다 (게이트 (c) 기전).
...
- **결정**: 매 LOD pass 에서 `cloud.isVisible = (host.isVisible || midVariant?.isVisible === true)`. 즉 **구름은 rim 을 싣고 있는 variant 가 보이는 동안만 보인다** — rim 의 low 소멸 (§결정 3 상속) 과 **같은 시점**에 사라진다.
  ...
  - mid ↔ low fade: mid variant 가 `isVisible` 인 동안 (fade 전 구간) 보이고, fade 종료의 `hideVariantEntirely` 와 **같은 프레임**에 사라진다. 절차 셰이더는 `material.alpha` 를 출력에 쓰지 않으므로 rim 도 이 시점에 사라진다 — 구름에 별도 알파 fade 를 붙이지 않는다 (새 타이밍 상수 `0`). 이 창에서 fade 중인 low billboard 는 host 계열이라 결정 4 에 따라 구름보다 **먼저** 그려진다 (구름이 billboard 위). ⚠️ 이 창의 구름·billboard 조합에는 게이트가 없다 — D1 GUI 육안이 대신한다.
  - cross-validate Q2 (「mid ↔ low 하드 팝핑은 새 결함」) 는 기각했다 — 팝은 기존 제품 거동이고 (절차 셰이더는 alpha 를 출력에 쓰지 않는다), 구름에만 알파 fade 를 붙이면 **구름만 흐려지고 표면은 팝**하는 새 비대칭이 생긴다 (§A10.16).
...
> ⏩ **결과 (2026-09-11, #1215 Phase 1b — dev 실측)**: 위 제외 근거 문장은 **반증됐다** — D1 캡처에서 밤면 구름이 ambient 조명으로 밝은 회색으로 보이고 사용자가 그 밝기를 유지로 결정했다 (#1215 코멘트 `5632521560`). 그래서 대역을 미리 정하지 않고 D1 승인 파라미터의 baseline 으로 다시 판단했고 **낮면 내부를 유지**한다 [실측, **로컬** swiftshader 5회 산포 `0` — CI 렌더러와는 소수 4~5 자리 차이, 아래 반려 라운드 부기]: 낮면 내부 `N 7225` · C1 량 `0.034201` · OFF 채널 `255` 포화 `3000` 제외 후 C2 표본 `4225` · 맑은 하늘 비율 `0.55598` / 밤면 내부 `N 7225` · C1 량 `0.02294` · 포화 `0` · 비율 `0.51599`. 포화 제외 후에도 C2 표본이 하한 위이고 C1 량은 낮면이 더 크다 — 대역을 옮겨 얻는 판별력이 없다. 도출 임계: `T_DELTA 0.0114` (`0.034201 ÷ 3`) · `T_CLEAR 0.1853` (`0.55598 ÷ 3`) · `MIN_EXPECTED 2408` (`⌊7225 ÷ 3⌋`). 원문은 소급 수정하지 않는다 (근거 문장만 교체 기록).

> ⏩ **결과 (2026-09-11, #1215 PR #1217 반려 라운드 — reviewer B1 · cross-validate X1~X4)**: 위 「측정 불가」 전제 2 · 3 이 **주 쌍에만** 걸려 있어 `changed == 0` 을 보는 C6 · C7a · C8b 가 대상이 비어도 통과했다 (reviewer 변이 R2 — rotate ON 에서 지구 scaling `0` — 가 exit `0`, C7 쌍 disk `n = 0`). 새 상수 없이 전제 적용 범위를 넓혔다 [실측 — 원본 실행 값]:
>
> | 쌍 | 쓰는 게이트 | 전제 2 (낮면 내부 N ≥ `2408`) | 전제 3 (OFF 낮면 휘도 ≥ `0.15`) | 전제 7 (C2 표본) |
> | --- | --- | --- | --- | --- |
> | main | C1 · C2 · C6 | ✅ `7225` | ✅ `0.76919` | ✅ `4225` |
> | mid · fade | C5a · C5b | ✅ `7225` | ✅ `0.88277` | ❌ — C2 를 안 재고 `1109` 라 걸면 상시 측정 불가 |
> | c7Load | C7a | ✅ `6407` | ✅ `0.90962` | ❌ — `350` |
> | c8Load | C8b | ✅ `7225` | ✅ `0.80339` | ❌ |
> | low | C6 | ✅ `7225` | ❌ — 대역 OFF 휘도 `0.03559` = 씬 배경색 `(8, 9, 13)` 의 휘도 `0.03559` (대역이 배경이다) | ❌ |
>
> ⚠️ **한계 (reviewer B2)** — C6 의 판정 근거는 `isVisible === false` 구조 읽기이고, 픽셀 항 (`changed == 0`) 은 배경 위에 구름이 그려지는 경우 (MC-5 `14211 px`) 만 잡는다. low 프레임에 지구가 실제로 그려졌는지는 **보증하지 않는다** — 전제 2 는 ray-sphere 기하 계수이고 C1 은 high 프레임 값이다 (reviewer 주입: low 정착 뒤 두 페이지의 지구 계열 mesh 를 `layerMask = 0` 으로 빼도 exit `0`).
>
> 추가: **전제 8** — fade 정지 재현의 투명 큐에 `earth-lod-mid` 가 있어야 한다 (**하네스 설정만** 묻는다 — 구름이 투명 큐에 있는가는 제품 속성이라 C3 · C5b 가 FAIL 로 잰다. 메인 결정, X2). **전제 6 확장** — 비유한 · 퇴화 기하 (반경 ≤ 0) 와 쌍별 페이지 기하 불일치 (X3 — `≥` 술어에서는 통과 방향) 를 측정 오류로. **C5b** 에 `∧ C5a PASS` 결합 (reviewer A1). baseline 산포 `0` 은 **로컬** swiftshader 5회에 한한다 — CI 렌더러 (run `34591856729`, `5f72be7`) 와는 소수 4~5 자리 차이 (C1 `0.034201` vs `0.034194`, 맑은 하늘 비율 `0.55598` vs `0.55624`), 임계와는 멀어 무변경. 판별 실증: 반려 대응 변이 6건이 전건 `exit 2` — rotate ON 지구 NaN quaternion (전제 6 기하 무효) · 지구 scaling `0` (전제 6 — 기하 무효 또는 쌍 기하 불일치. 쌍 기하 대조 → 반경 검사 → 전제 2 · 3 세 겹이 각각 독립으로 막는다 — 앞 두 겹을 모두 제거해도 전제 2 · 3 이 `exit 2`, reviewer 실행) · 카메라 이탈 (전제 2 · 3) · fade 정지 mid alpha 제거 (전제 8) · settle timeout 강제 (전제 1) · `verify:1202` ON 페이지 구름 부재 (ON 전제 5), 원복 전건 PASS · 원본 판정량 무변경. ⚠️ MC-9 (구름 `needAlphaBlending` 거짓) 는 전제 8 첫 판본 (구름 · mid 둘 다 요구) 에서 C3 FAIL 이 가려져 `exit 2` 였다 — 전제가 제품 속성을 섞은 오류라 mid 만 묻도록 좁혔고, 재실행에서 **FAIL** (C2 · C3 · C5a · C5b) 로 돌아왔다. 원자료 `docs/reports/1215-cloud-layer/phase1-1b-mc-mutations.json` `reviewRound`. 원문은 소급 수정하지 않는다.

| # | 술어 | 겨냥 | 위배 시 |
| --- | --- | --- | --- |
| **C1** | 낮면 내부 평균 `abs(lum(ON) − lum(OFF))` ≥ `T_DELTA` | 결함 문장 「구름이 안 보인다」의 **문자 그대로의 부정** (판별력 하한) | FAIL |
| **C2** | 낮면 내부 중 **OFF 픽셀의 어느 채널도 정확히 `255` 가 아닌** 픽셀만 표본으로, **맑은 하늘 비율** (채널 최대차 `== 0` 인 픽셀 비율) ≥ `T_CLEAR` | **과다 상한** — 표면 전부 덮음 / 얇은 베일 전면. C1 은 이 방향에 눈이 멀다 (§A8.8 `M-6` 동형). 포화 제외 이유: OFF 가 이미 `255` 인 채널 (극관 등) 은 흰 구름이 덮어도 diff `0` 이라 「맑음」 으로 오계수된다 (cross-validate U2 — 새 상수 `0`) | FAIL |
...
7. **C2 표본 (포화 제외 후) `N < MIN_EXPECTED`** — 같은 `MIN_EXPECTED` 를 재사용한다 (새 상수 `0`). 제외된 픽셀 수는 매 실행 인쇄한다. ⚠️ cross-validate 가 제안한 `0.95` 비율 · LSB 허용오차 `2/255` 는 **발명 임계라 쓰지 않는다**
...
- `== 0` 술어 (C2 의 맑은 하늘 · C6 · C7 · C8) 는 새 임계가 아니다. 근거: 페이지 로드 간 결정성이 실측됐다 (Phase 0 결정성 대조군 — 실 GPU 전 화면 `0`, swiftshader disk 내부 `0`). GPU 1-LSB 지터가 실측되면 **보고**한다 — 허용오차를 먼저 넣지 않는다 (α = `0` 픽셀의 `ALPHA_COMBINE` 결과는 `dst × 1`).
- C1 은 포화 픽셀을 **제외하지 않는다** — 포화 픽셀은 C1 량을 낮추는 쪽 (FAIL 방향, fail-closed) 이라 공허 통과를 만들지 않는다. baseline 이 그 효과를 포함한다.
...
| MC-4 | 정렬 키 치환 함수 설치 삭제 | 소스 + dist 재빌드 | C5 뒤 항 (architect 실측 동형: `changed 0`) |
| MC-12 | 구름 `rank` 차순위 제거 (계열 치환은 유지 — 계열 전원 `rank 0`) | 소스 + dist 재빌드 | C5 뒤 항 — 동률이 삽입 순서로 돌아가 lazy mid 가 구름 뒤. `rank` 가 판별력의 실제 담지자임을 실증 |
...
#### A10.12-U1 cross-validate U1 대안 (host 계열 불투명 강제) 실측 비교

같은 측정 조건. fade 창 정지 재현 = `setLodOverride('mid')` 정착 후 host · mid 를 매 프레임 `isVisible = true` + `material.alpha = 0.999` 로 덮어씀 (`setVariantAlpha` 와 같은 속성). 불투명 강제 = 두 variant 재질에 `transparencyMode = 0` (`MATERIAL_OPAQUE`) — U1 의 프로덕션 형태와 같은 속성. 분류 기준 H = high 정착 프레임, M = mid 정착 프레임, disk `n = 29144`. 원자료 `phase1-architect-u1-opaque-fade.json` · `phase1-architect-u1-determinism.json`. 콘솔 에러 전 시나리오 `0`.

| body · 시나리오 | 큐 | disk 분류 | 행 방향 H/M 전환 |
| --- | --- | --- | ---: |
| earth — 현행 fade | transparent `[earth, earth-lod-mid]` | M 과 전부 동일 (`onlyM 28085` · `eqBoth 1059` · `neither 0`) — H≠M 픽셀 `28085` | `0` |
| earth — 불투명 강제 fade | opaque `[earth, moon-lod-mid, earth-lod-mid]`, transparent `[]` | `onlyH 27723` · `onlyM 223` · **`neither 139`** · `eqBoth 1059` | `469` |
| mars — 현행 fade | transparent `[mars, mars-lod-mid]` | M 과 전부 동일 (`onlyM 13650` · `neither 0`) | `0` |
| mars — 불투명 강제 fade | opaque 3개, transparent `[]` | `onlyH 13622` · `onlyM 27` · **`neither 3`** | `45` |

| 구름 결합 (earth, full frame 변화 px) | 값 | 의미 |
| --- | ---: | --- |
| mid 정착: 구름 없음 ↔ 구름 | `31622` | 양성 대조 |
| 현행 fade: 구름 없음 ↔ 구름 | `2002` | disk 안 구름 소멸 (disk 밖 halo 만 남음 — 위 표의 disk `0` 과 정합) |
| 불투명 강제 fade: 구름 없음 ↔ 구름 | `31643` | 구름 복원 — 큐 transparent `[cloud-probe]` 1개 |

**결정성 대조** (별도 실행, earth): H × 2 · 현행 fade × 2 · 불투명 강제 fade × 2 의 독립 로드 쌍 — disk 내부 · 림 링 (`R ~ 1.1R`) · 바깥 **전부 `0` px**. ⚠️ 1차 실행에서는 불투명 강제 독립 2회 로드가 full frame `22745 px` 달랐다 (disk 분류는 동일). 대조 실행에서 **재현되지 않았고 원인은 규명하지 않았다** — 원인으로 승격하지 않는다 (§A10.15).

### A10.13 재검토 조건 결과 기록 (계약 D8)

| 조건 | 결과 |
| --- | --- |
| **§A1.8 조건 5** (대기/구름 — 알파 + depth 우선순위, log-depth 핵심 위험 1) | **발동·처리.** 알파 = ALPHABLEND (§A10.1). depth: log-depth 기록 필수 (없으면 `11736 → 159 px`), `disableDepthWrite` 는 블렌드 draw 에서 무효 → `true` 명시 (§A10.5). 우선순위: 신규 결함 (fade 창 동률 정렬) 을 정렬 키 치환으로 해소 (§A10.6 — 불투명 강제 대안은 실측 비교 후 기각) |
| **§A3.7 조건 3** (구름 / 야간 도시 불빛 — 차등 offset) | **구름: 발동·처리** — host 자식의 local 상대 자전, jd 순수 함수, `?rotate=off` 구조적 상속 (§A10.8). **야간 도시 불빛: 미발동** — 계약 비-범위. 인프라 (자식 shell · 정렬 키 치환 · 가시성 파생) 재사용 여부는 그 이슈에서 판정 |
...
- 정렬 키 치환 설치 후 fade 창에서 구름이 **복원되는지** (결함은 실측했고 처방은 아직 실행되지 않았다 — C5 가 첫 실행)
- 정렬 함수 교체의 프레임 시간 영향
- low billboard 와 구름의 거리 동률 (구조상 중심 동일 — host 자식 `position (0, 0, 0)` — 거리 값은 미실측)
- mid ↔ low fade 창에서 구름 · billboard 조합의 시각 결과 (게이트 없음 — D1 GUI)
...
- 결정 1 (host 자식 shell) · 결정 6 (jd 순수 함수 자전) · 결정 8 (`clouds && surfaceDetail`) · #1214 시그니처 5종 적용.
- `hash13` / `fbm` 4벌째 부채 — §A10.14-6 재검토 조건으로 이미 기록.
- 차원 검산 (메인 C3) — `CLOUD_ZONAL_WIND_MS × 86400 / body.radius` 는 `radius` 가 m (`solar-system.json` earth `6378137.0`) 이라 rad/day 로 맞다.

#### 이견 수용 (원안 수정)

| # | 원안 | 수정안 | 수용 근거 |
| --- | --- | --- | --- |
| **U1** | 결정 4 기각 목록에 「불투명 큐 유지」 부재 | 대안을 **실측 비교**하고 기각 사유를 박제 (§A10.6 · §A10.12-U1). 결정 4 는 유지 | 원인 (투명 큐 진입) 을 고치는 방향은 옳았고 비교 없이 둔 것은 누락이었다. 다만 agy 의 「부작용 0」 은 실측에서 반증됐다 — 새 아티팩트 `139 + 223 px`, mars fade 창 변경, billboard 잔여 규칙 필요 |
| **C1** (메인) | 거리를 보지 않는 쌍 규칙 | host 값 치환 사전식 키 `(alphaIndex, distance, rank)` + 구름 `rank 1`, 식별자 집합 조회 | 비추이성 (`d_A < d_C < d_B`) 을 형태로 제거. 계열 내부 순서는 현행 그대로 |
| **U2** | C2 표본 = 낮면 내부 전체 | OFF 채널 하나라도 정확히 `255` 인 픽셀 제외 + 제외 후 `N` 을 「측정 불가」 7 에 결합 | 포화 채널은 흰 구름이 덮어도 diff `0` → 「맑음」 오계수. 새 상수 `0` |
| **U3** | `alphaIndex` 선후 · billboard 포함 미정 | 치환이 `alphaIndex` 보다 먼저 · billboard 포함 | `setVariantAlpha` 가 level 무관 (메인 확인), billboard 는 host 자식 · 원점 |
| **C2** (메인) | Visual Fidelity 체크리스트를 차등 속도에만 적용 | shell 반경비에도 4항목 박제 (§A10.3) | 반경비 자체가 rendering 왜곡 (물리 `≈ 1.0016` 대비 프로토 `1.02`) |

#### Claude 재분석으로 기각한 외부 모델 제안

| agy 주장 | 기각 근거 |
| --- | --- |
| U2 처방의 `0.95` 비율 · LSB 허용오차 `2/255` | 발명 임계. 포화 제외 (새 상수 `0`) 로 대체 |
| 대안 B `alphaIndex` 계층화 (host `100` / 구름 `200`) 가 전역적으로 옳다 | 지구가 거리와 무관하게 먼저 그려져 지구 **뒤**의 달 fade variant 가 지구 위에 그려진다 (게이트 (c) 기전) — 원 기각 사유 유지 |
| Q2 결정 5 의 mid ↔ low 하드 팝핑 = 새 결함 | 기존 제품 거동 (절차 셰이더는 alpha 를 출력에 안 쓴다). 구름만 알파 fade 하면 구름만 흐려지고 표면은 팝하는 새 비대칭 |
| [Critical] LOD fade 알파 무반응 = 버그 방치 | 범위 밖 기존 거동 — 본 이슈가 만든 것이 아니다. PR 코멘트 기록 (§검증 강도 게이트 기본 처분) |
| Q3 `G6` 만 구름 OFF 로 옮긴 혼합 구조 = False Negative | 계약 재조정 2 의 사용자 합의 선택지. 구름 ON 박명 값은 진단 인쇄로 남고, `G1`~`G4` 도피는 §A10.14-1 이 사용자 합의 게이트로 막는다 |
| C2 `== 0` 이 GPU 1-LSB 지터로 상시 FAIL | α = `0` 은 `dst × 1`, 로드 간 결정성은 실측 (swiftshader disk 내부 `0`, 본 개정 대조 실행 `0`). 지터가 실측되면 보고 |
| 음수 JD 에서 `%` 가 음수 → 축 반전 | 음수 각은 유효한 회전이고 `(jd − epoch)` 가 `0` 을 지날 때 각도는 연속이다. 기존 `self-rotation.ts:80` 과 같은 식 |
| 정렬 함수 GC · 프레임 시간 (High) | `?focus=earth` 정착 투명 큐는 `0`~`1` 개 (Phase 0 실측). 식별자 집합 조회로 구현 (반영). 프레임 시간은 §A10.15 미확인 |
| C5 의 fade 기전 화이트박스 결합 | 의도다 — 결정 4 가 그 기전 때문에 존재한다. 코드 주석 기록 + §A10.14-7 |
| 확장성 `O(K²)` | 계열 판정은 집합 조회 `O(1)` |

#### 고유 발견 (후속 분리)

- **LOD fade 창의 알파 무반응** (agy [Critical] 의 실체) — 절차 셰이더 variant 는 fade 창에서 cross-fade 가 아니라 **삽입 순서가 늦은 쪽이 전면 대체**한다 (본 개정 실측: earth · mars 현행 fade 창 = mid 정착 프레임과 disk 전면 동일). 범위 밖 기존 거동이라 이슈화하지 않고 **PR 코멘트에 기록**한다 (메인 처분).
- 불투명 강제 1차 실행의 `22745 px` 차이 — 재현 안 됨, 원인 미규명 (§A10.15).

#### Claude 편향 셀프 체크

- **원인 수정 서사** — 「원인을 고치는 대안이 결과를 고치는 원안보다 낫다」 는 서사에 메인도 architect 도 끌리기 쉽다. 채택 여부를 실측으로 넘겼고, 결과는 원안 쪽이었다 — 단 **원안의 형태는 수정됐다** (C1).
- **순수주의** — 정렬 함수 교체가 전역 표면을 하나 만든다는 초판 △ 는 남는다. 사전식 키로 형태 안전성을 확보했고, 계열 밖 쌍의 결과가 기본 비교와 같다는 것을 단위 테스트로 묶는다.
- **결합 간과** △ — 지구 픽셀 가드 (`783` / `1119` 등) 의 구름 ON 결과는 여전히 미실측이다 (§A10.14-2).
- **낙관적 일정** △ — 변동 없음 (D1 사용자 왕복 · §A10.14-1 · 2 발화 시 정지).
...
- ⚠️ **본 Amendment 는 설계 박제다.** 가드 임계 수치는 정하지 않는다 — §A10.11 과 같이 **도출 규칙**만 두고 수치는 dev 가 D1 승인 파라미터의 baseline 으로 채운다. 「실측」 라벨은 실행 출력에만 붙였고, 「도출」은 실측값에서 산술로 얻은 것, 「구조」는 코드 경로 판독, 「추론」은 실행되지 않은 예측이다. 실행되지 않은 것은 §A11.14 에 모았다.

**측정 조건 (아래 「실측」 공통)**: 로컬 headless chromium `--use-angle=swiftshader` · 1280×720 · `packages/core` 를 `808b574` (develop tip) 에서 빌드 · `next dev`. 결정적 프레임 = `verify:1202` 레시피 (`?gpu=a&focus=earth&lod=auto&rotate=off&orbits=off` · JD 고정 · pause · `beta = π/2` · `waitForLodSettle`, 전 실행 `timedOut false`). **프로덕션 코드 `0` 줄** — 지구 표면 머티리얼 uniform 을 `onBindObservable` 로 덮어쓰는 런타임 주입만 했다 (`verify:1202` `injectFloats` 선례). 콘솔 에러 전 페이지 `0`.

- **게이트 맵** — `sunIntensity 0` · `ambientIntensity 1` · `ambientGround = ambientSky = (1,1,1)` · `baseColor (0,0,0)` · `landColor = biomeTropicalColor = biomeTundraColor = (1,1,1)` · `iceColor (0,0,1)` · `rimStrength 0`. 셰이더 식상 `R = landMask · (1 − iceMask)`, `B − R = iceMask` 가 된다 (8-bit).
- **구름 알파 맵** — 위와 같되 `baseColor` · `iceColor` 도 `(1,1,1)` (표면 전면 `255`) + 구름 머티리얼 `cloudColor (0,0,0)`. `ALPHA_COMBINE` 이므로 `R = 255 · (1 − α)`.

### A11.1 결정 0 (게이트) — Q2 = **(A) 표면 셰이더 혼입 확정**

| # | 축 | (A) 표면 셰이더 혼입 | (B) host 자식 shell | 근거 |
| --- | --- | --- | --- | --- |
| 1 | 신규 mesh · 투명 큐 | `0` · `0` | `+1` · `+1` | [구조] |
| 2 | 구름이 불빛을 가리는 순서 | **구조적** — 불빛은 표면 `col` 의 일부이고 구름은 host 계열 draw 뒤에 블렌드된다 (§A10.6). [실측 — 게이트 맵이 같은 합성 경로를 탄다] JD `2451808.0` 밤면 내부 육지에서 구름 ON/OFF 게이트 가중 감쇠 `0.2619` | `rank` 가 정수 `0`(host 계열) · `1`(구름) 이라 사이 순위 신설 필요 — `verify:1215` `MC-12` 전제 변경 | [파일] `cloud-layer.ts` `HOST_FAMILY_RANK = 0` 주석 (구름 `1`) |
| 3 | LOD low 소멸 | **구조적** — low billboard 는 `StandardMaterial` 이라 표면 셰이더 미진입 (§A10.6 (b)). mid variant 는 같은 절차 셰이더를 쓴다 | 가시성 파생 재사용 필요 — 누락 시 low 에서 그려짐 (§A10.7 실측 `29144 px`) | [파일] `body-mesh-factory.ts` `createBodyMeshMid` |
| 4 | `hash13` / `fbm` **함수 사본** | 증가 `0` | 5벌째 (§A10.14 조건 6 발화) | [grep] FRAGMENT 안 `fbm(` `5` · `hash13(` `9` (정의 포함) |
| 5 | `verify:675-glow-marker` 도달 경로 | `0` | 가시성 파생 누락 시 누수 | [실측] 40 AU · 100 AU 에서 지구 `level low` (`pxDiameter 0.786` · `0.314`, 분포 `0/0/32`) |
| 6 | §A3.3 685행 「별도 mesh/레이어」 | 다름 → 부기 | 일치 | [파일] |
| 7 | 광원 입력 | 기존 `ndl` 재사용 | 광원 uniform 중복 선언·바인딩 (§A8.1 축 6) | [구조] |

**(B) 가 이기는 축은 6 (원문 문면 일치) 하나이고, 실측으로 (B) 우위가 드러난 축은 없다.** 축 5 는 계약 위험 R3 의 「원거리라 low 일 것이다 [추론]」을 실측으로 확정한 것이다. ⇒ 사용자 재결정 불요.

⚠️ 셀프 체크 — §A8.1 초판의 순환논증 (비-범위를 기각 근거로 인용) 을 재생산하지 않았는지: 위 표의 어느 행도 #1226 비-범위를 근거로 쓰지 않는다.

### A11.2 §A10.14 조건 5 — 인프라 3건 재사용 판정

| 인프라 | 판정 | 근거 |
| --- | --- | --- |
| 자식 shell (§A10.3) | **비재사용** | 불빛은 고도 `0` · 알파 `0` 의 표면 발광이다. shell 을 쓰면 구름과의 순서를 `rank` 사이에 끼워야 한다 (§A11.1 축 2) |
| 정렬 키 치환 (§A10.6) | **비재사용 — 단, 의존한다** | 불빛은 투명 mesh 를 만들지 않는다. 그러나 「구름이 불빛 위」는 구름이 host 계열 뒤에 그려진다는 §A10.6 의 보장 위에 선다. 따라서 §A10.14 조건 7 (fade 기전 변경) 이 발동하면 불빛 차폐도 함께 재검토한다 |
| 가시성 파생 (§A10.7) | **비재사용** | 표면 셰이더 항은 rim 과 같이 low 소멸을 **상속**한다 (§A8.11 조건 6 원문의 「상속」). 계약 D7 (3) 은 「구조적으로 해당 없음」 |

### A11.3 결정 1 — 불빛 항의 형태와 합성 위치

rim 과 같은 배치다 — 연산은 rocky 분기 안, 합성은 `col *= shade` 뒤 · 기존 `clamp` 앞, 연산자는 **가산** (발광이라 밤면 명암에 곱해지지 않는다).

```text
// rocky 분기 안 (landMask · iceMask · ndl 은 이미 계산돼 있다 — 재계산 금지, volt #21)
float nightFactor = 1.0 - smoothstep(-nightLightTwilightWidth, 0.0, ndl);
float lightGate   = landMask * (1.0 - iceMask) * uMaskEnabled;
lights = nightLightStrength * nightFactor * lightGate * lightDensity;   // lightDensity: 결정 3

// 분기 밖, col *= shade 뒤 · col += rimColor * rim 옆 · clamp 앞
col += nightLightColor * lights;
```

- **`nightFactor` (Q5)** — `ndl ≥ 0` 에서 **정확히 `0`**. 계약 D3 (`ndl ≥ 0.15` 불변 `== 0`) 이 구조적으로 성립한다 (`x + 0.0` 은 정확). `nightLightTwilightWidth > 0` 을 단위 테스트로 assert 한다 — `0` 이면 `smoothstep` 의 `edge0 >= edge1` 로 GLSL 명세상 undefined 다 (§A3.7 조건 1 의 #1197 부기 — `smoothstep(landThresholdLo, 0.0, …)` — 와 같은 클래스).
- **입력 공간** — 분포 입력은 전부 `p = normalize(vLocalPos)` 파생으로 제한한다 → 표면과 함께 돈다 (painted-on), `?rotate=off` 는 신규 게이트 없이 상속. **`vWorldPos` 사용 금지** (§A8.3 계약 — `procedural-planet-shader.test.ts` U11 이 FRAGMENT 안 등장 횟수 `2` 로 고정해 이미 가드한다).
- **OFF = `nightLightStrength = 0`** (같은 프로그램). rim 의 `rimStrength = 0` 선례다. 비-rocky 타입은 `lights = 0.0` 이라 합성이 정확한 no-op (rim 합성과 같은 형태).
- ⚠️ **밤면 림 clamp 결합** — rim 과 불빛이 같은 픽셀에 가산되면 `clamp` 가 비선형이 된다. `verify:1202` `G1` (밤면 림) · `G6` (박명 호) 은 rim ON/OFF 두 프레임에 불빛이 똑같이 있어 상쇄될 것으로 보나 **[추론]** 이다 — D10 전·후 인쇄로 확인한다.

### A11.4 결정 2 — 게이트 `landMask × (1 − iceMask) × uMaskEnabled` (Q1 (c) · Q1-b)

계약 Q1 이 architect 에게 넘긴 「`maskLand` 직접 / `landMask`」 선택에 셋째 안을 더해 비교했다.

| 안 | 원거리 (`R < 16 px`) · 마스크 미도착 | 판정 |
| --- | --- | --- |
| (α) `maskLand` | 표시 대륙은 절차 경로인데 불빛은 마스크 대륙 → **표시된 바다 위 불빛** | 기각 |
| (β) `landMask` | 표시와 일치하나 불빛 분포가 절차 대륙으로 **전환** (계약 R6). 고주파 분포 노이즈가 작은 disk 에서 over-resolution shimmer 를 낼 축 (§A4.3 결정 4 논거의 연장) | 기각 |
| **(γ) `landMask × uMaskEnabled`** | **마스크 경로가 꺼지는 곳에서 불빛도 꺼진다** — 분포 전환 `0`, 신규 uniform `0`, 로드 실패 시 §A4.3 결정 7 graceful degradation 과 같은 경로 | **채택** |

- **(γ) 의 대가** — 반경 `16 px` (지름 `32 px`) 경계에서 불빛이 켜지고 꺼진다. 그 크기의 밤면 불빛은 수 px 이지만 인지 여부는 **미실측**이다 → D13 줌 관찰 항목.
- **[실측] 전환 지점** — `?focus=earth` 에서 카메라 반경을 `1~96` 배로 늘린 스윕: `uMaskEnabled` 는 지름 `32.77 px` 에서 `1`, `24.58 px` 에서 `0` (임계 `SURFACE_MASK_MIN_DISK_PX = 16` 반경과 정합). **같은 스윕에서 지구 LOD 는 지름 `2.05 px` 까지 `high` 였고 mid · low variant 는 생성되지 않았다.** 즉 focus 상태에서는 마스크 OFF 구간이 **표면 셰이더가 실제로 그려지는 구간**이다. ⚠️ §A4.3 결정 4 의 「이 규칙이 실제로 걸리는 구간은 mid variant 의 원거리 끝」 은 **focus 가 없는** 경우의 서술일 수 있다 — focus 에서 high 가 유지되는 원인 코드는 확인하지 못했다 (§A11.14).
- **Q1-b 극관 억제** — `(1 − iceMask)`, 이미 계산된 값이라 신규 샘플 `0`. ⚠️ **픽셀 가드 사각 [실측]**: 결정적 프레임 밤면 내부 (`ndl ≤ −0.15` ∧ `ndv ≥ 0.6`) 의 `iceMask` 포화 픽셀이 JD `2451626.0` 에서 `0` 이다. 극관 억제를 제거하는 변이는 신규 가드가 판별하지 못한다 → 단위 테스트 (GLSL 토큰 또는 JS 미러) 로 고정하고, `verify:783` DoD 4 (밤면 극 휘도) 의 전·후 값을 D10 에서 관측한다 (계약 R2). 현행 상수는 `ICE_LAT_LO = 0.84` · `ICE_LAT_HI = 0.92` 다 [파일 — `procedural-planet-shader.ts`. §A3.3 결정 3-b 표의 `0.88 / 0.96` 은 출발값]. `783` 의 극 대역 하한 (sin `0.88`) 은 그 램프의 **한가운데**라 대역 하단의 `iceMask` 는 `latJ` 요동 전 기준 `0.5` 다 [도출 — `smoothstep(0.84, 0.92, 0.88)`] — 극 대역 안 육지 불빛이 **절반만 억제되는 구간**이 있다.

### A11.5 결정 3 — 분포 `lightDensity`: 후보만 두고 선택은 D1

- **noise 함수 사본 증가 `0`** (§A10.14 조건 6 미발화). 호출 증가는 사본과 별개인 fragment 비용이다.
- **후보** (D1 에서 캡처 비교 ≥ 2):

  | 후보 | 형태 | rocky fragment 추가 hash | 위험 |
  | --- | --- | ---: | --- |
  | (d1) value noise 군집 | `smoothstep(lo, hi, valueNoise(p * K))` | `+8` (`hash13` 8회) | 주파수 과다 시 shimmer |
  | (d2) 셀 점 분포 | `step(th, hash13(floor(p * K)))` | `+1` | 3D 격자 셀 경계가 보일 수 있음 |
  | 공통 | 대규모 밀도 변조에 기존 `continents` 재사용 | `0` | §A3.3 결정 3-c 와 같은 상관 (대륙 형상과 연동) |
...
- core 옵션 `nightLights?: boolean` — core 기본 `false` (`clouds` 동형). **`nightLightStrength` 바인딩 값 = `nightLights && surfaceDetail ? NIGHT_LIGHT_STRENGTH : 0`**. `surfaceDetail = false` 면 절차 머티리얼 자체가 없으므로 `?surface=off` 픽셀 동일성은 구조적이다.
- `?clouds=` 와 **독립** — 두 옵션은 서로를 읽지 않는다.
- ⚠️ `?nightlights=off` ↔ develop tip 동일성 (계약 D9 (a)) 은 **프로그램이 다르다** (GLSL 에 항이 추가됨). 선례: Amendment 8 에서 가산 항 `col += rimColor * rim` 을 추가한 뒤 rim `0.0` 인 mars / jupiter / moon 과 `?surface=off` 가 diff `0` 이었다 [§A8.11 실측]. 선례이지 보증이 아니므로 D9 (a) 가 직접 잰다.

### A11.7 §A8.11 조건 1 — 불빛 진입분 결과 · rim 이관

- **(B) 재평가** — 불빛은 (A). §A8.1 축 5 (구름 인프라 공유) 는 구름 shell 이 실재하므로 값이 살아 있지만, **불빛은 그 인프라를 쓸 속성이 없다** (고도 · 알파 · 별도 정렬이 필요 없는 표면 발광). 이는 「(A) 로 정했으니 축 5 는 `0`」 (§A10.2 가 금지한 순환) 이 아니라 불빛의 물리적 배치에서 온 판정이다.
- **rim 이관: 검토 후 유지** (무처분 아님). 조건부 재개 트리거 「낮면 림 헤드룸이 하한 `0.05` 에 접근」 은 **구조적으로 미발화** — 불빛 항은 `ndl ≥ 0` 에서 정확히 `0` 이고 낮면 림 대역은 `ndl ≥ 0.15` 다. 오히려 공유 셰이더의 가산 발광 항이 rim · 불빛 둘이 되어, rim 만 shell 로 옮기면 두 항의 합성 위치가 갈라진다 — 이관 비용이 느는 방향이다. D10 의 `verify:1202` 전·후 수치가 확인한다.

### A11.8 §A10.14 조건 6 — **미발화**

함수 사본 증가 `0` (§A11.1 축 4). 공용 GLSL 추출 재판단은 열지 않는다.

### A11.9 Visual Fidelity 판정 (계약 D0 (5))

- **적용 트리거 해당 없음** — principles.md §1 「적용 트리거」 4종 (mesh px diameter 임계 · fusion · 점유율 · 사실 비례 역전) 은 전부 기하이고 밝기는 목록에 없다 [파일]. 더 근본적으로, 불빛은 데이터 SSoT 의 물리량 (반경 · 거리 · `colorHint`) 을 **과장하는 것이 아니라 데이터에 없는 표현을 더한다** — 야간광 데이터 필드가 `0` 이므로 「왜곡」 이 성립하지 않는다.
- 4항목은 판정 결과로 기록한다: (1) **데이터 SSoT** — `solar-system.json` 변경 `0`, 신규 에셋 `0` (기존 마스크 재사용 — 에셋 수 불변, §A4.3 결정 6 「마스크 소비는 rocky 분기 내부에서만」 과 정합) (2) **rendering 시점 분리** — physics 엔진 무관 (3) **D-T2** — UI 는 도시 · 인구 정보를 표기하지 않는다 (비-범위) (4) **baseline** — mesh 크기 · 위치 불변이라 해당 없음.
- ⚠️ 트리거 목록에 밝기 · 발광 축이 없다는 사실만 기록한다 (목록 개정은 본 Amendment 범위 밖).
- 천문 정확성: 어느 대륙이 밤인지는 §A4.6 한계를 그대로 상속한다.

### A11.10 결정 5 — 신규 가드 `verify:1226-night-lights` 의 측정 설계 (도출 규칙만 — 수치는 dev)

계약 D2~D12 의 술어 · 변이 표를 따르되, 실측으로 드러난 **판별력 결손 3건**을 설계로 보강한다. 보강 중 계약 문면을 바꾸는 것은 §A11.11 사용자 결정 항목으로 올렸다.

**페이지 (JD 하나당)** — P1 `&clouds=off` · P2 `&clouds=off&nightlights=off` · P3 구름 ON · P4 구름 ON `&nightlights=off`. **게이트 맵 G** 는 P2 에서 제품 캡처 뒤 같은 페이지에 주입해 찍는다 (같은 기하). 게이트 맵은 **불빛 강도 `0` 페이지에서 불빛과 무관한 uniform 으로** 렌더되므로 불빛 결함이 G 를 움직일 수 없다.

**표본 집합**

- `NI` = 밤면 내부 (`ndl ≤ NIGHT_NDL_MAX` ∧ `ndv ≥ INNER_NDV_MIN`), `DI` = 낮면 내부 — 기하 (계약 그대로).
- `NI_land` = `NI` ∩ {증폭 게이트 맵 `R > 0`}, `NI_sea` = `NI` ∩ {증폭 게이트 맵 `R == 0`}.
- **증폭 게이트 맵** = 위 게이트 맵에서 `ambientIntensity` 를 `255` 로 둔 판본 — `R > 0` 이 `landMask · (1 − iceMask) ≥ 약 0.5/255²` 와 같아진다 [도출]. 증폭이 필요한 이유 [도출]: 비증폭 맵에서 `R == 0` 인 해안 픽셀은 게이트가 `(0, 0.5/255)` 일 수 있고, 거기 더해지는 sub-LSB 불빛이 **기존 `col` 의 8-bit 반올림 경계를 넘겨** 계약 D5 `== 0` 을 옳은 구현에서 깰 수 있다. 증폭 맵에서 남는 그 대역은 쌍선형 보간 가중 `~5e-4` 텍셀 폭이라 사실상 비어 있다 (마스크 바다 텍셀은 정확히 `0` — §A4.3 결정 2 의 65 계조). dev 는 두 맵의 `R == 0` 불일치 픽셀 수를 인쇄한다.

**보강 1 — D4 표본은 `NI_land` (계약: D2 대역 = `NI`)** [도출 — 실측 표본 수]

`NI` 전체의 「불빛 ON/OFF 변화 없는 비율」 은 바다 비율이 하한을 **구조적으로** 채운다. 바다 비율은 JD `2451626.0` 에서 `6462 / 7225 = 89.4%`, JD `2451808.0` 에서 `3130 / 7225 = 43.3%` 다 [실측 게이트 맵]. 겨냥 변이 MN-4 (분포 게이트 제거 — 육지 전면 발광) 에서도 비율은 바다 비율 아래로 내려가지 않고, baseline ÷ 3 임계는 그보다 낮을 수 있어 **MN-4 가 통과한다.** `NI_land` 로 좁히면 MN-4 에서 비율이 `0` 근처로 떨어진다.
- ⚠️ M-i 클래스 (#1197 — 술어 도달 전 집합 필터가 결함을 흡수) 점검: 집합은 불빛 강도 `0` 의 G 에서 정해지므로 불빛 결함은 집합을 옮기지 못한다. `landMask` 자체를 바꾸는 결함은 집합과 대상이 함께 움직여 흡수되는데, 그 축은 `verify:1119` 대륙 IoU 가 잰다.

**보강 2 — 계약 「측정 불가」 전제 7 의 D6 양성 대조는 fail-open 이다** [실측]

계약은 「같은 실행에서 밤면 내부 구름 ON/OFF 휘도차 > 0」 을 전제로 둔다. JD `2451626.0` 에서 그 값은 양이다 (`NI` 전체 구름 알파 평균 `0.23792`, `#1215` 실측 휘도차 `0.02294`). **그러나 불빛이 사는 `NI_land` `763 px` 중 구름이 덮은 픽셀은 `6` 개, 게이트 가중 감쇠 `0.00006`** 이다. D6 는 신호가 `0` 인데 전제는 통과한다 — **전제가 판정 표본과 다른 집합에 걸린** #1215 B1 클래스다.
- 정정: 전제 7 을 **`NI_land` 에서 P4 ↔ P2 휘도가 다른 픽셀 수 ≥ `MIN_EXPECTED_CLOUDED_LAND`** (baseline ÷ 3) 로 건다. 두 페이지는 불빛 OFF 이고 구름 유무만 다르다. ⚠️ 구름 자체의 소멸은 이 전제를 `exit 2` 로 만들지 FAIL 로 만들지 않는다 — 그 제품 속성은 `verify:1215` C1 이 FAIL 로 잰다 (하네스 전제와 제품 게이트의 분리, #1215 X2).
- D6 판정량 = `NI_land` 평균 `lum(P3) − lum(P4)` 와 `lum(P1) − lum(P2)` 의 비. 구름의 가산 색은 ON/OFF 차분에서 상쇄된다 (clamp 미포화 전제 — D10 인쇄).

**보강 3 — D2 · D4 · D5 에 육지/바다 표본 하한 전제**

`|NI_land| < MIN_EXPECTED_NIGHT_LAND` (D2 · D4 · D6) · `|NI_sea| < MIN_EXPECTED_NIGHT_SEA` (D5) → `exit 2`. 하한은 baseline ÷ 3. [실측] `|NI_land|` 는 JD 에 따라 `763 ~ 4095` 로 5배 이상 달라진다 (§A11.11 표).
...
| JD | `NI` N | `NI_land` | `NI_land` ∩ 구름 | 게이트 가중 구름 감쇠 | `DI` N |
| --- | ---: | ---: | ---: | ---: | ---: |
| `2451626.0` (계약) | `7225` | `763` | `6` | `0.00006` | `7225` |
| `2451717.0` | `7205` | `1989` | `1056` | `0.23901` | `7205` |
| **`2451808.0`** | `7225` | **`4095`** | **`2194`** | **`0.26190`** | `7225` |
| `2451899.0` | `7238` | `2644` | `577` | `0.07708` | `7238` |

| 선택지 | 내용 | 결과 |
| --- | --- | --- |
| (a) D6 만 `2451808.0` | 나머지는 계약 프레임 | 페이지 수 증가, 프레임 두 벌 관리 |
| **(b) 신규 가드 전체를 `2451808.0` (권고)** | 기존 가드 (`1202` · `1215` 등) 는 무변경 | `NI_land` 5.4배 · D6 신호 확보. 프레임 한 벌 |
| (c) `2451626.0` 유지 + D6 의도적 스킵 | 3위치 박제 | **비권고** — 핵심 계약 「구름이 불빛을 가린다」 가 무검사 |

- 선택 기준은 **불빛과 무관한 량** (육지 · 구름 표본 수) 이고 불빛 파라미터는 아직 없다 — 가드 통과 값으로 프레임을 고르는 C1 클래스가 아니다. ⚠️ **4점 표본이지 최적 탐색이 아니다.**
- `2451808.0` 에서 `verify:1202` 의 기존 전제 상수 (`MIN_DAY_LIT_LUM` 등) 가 성립하는지는 위상각 `90°` 와 표본 수만 쟀다 — 나머지는 dev 첫 실행이 확인한다.
...
[실측] `773` 의 earth 프레임 (`?gpu=a&focus=earth&lod=auto&rotate=off`, JD 미고정 — 기본 시각에서 약 2.8 초 진행) 은 위상각 `107.27°` 였다. `773` 은 화면 반평면으로 낮/밤을 나누는데 (계약 E7), 그 「낮면」 절반 `15261 px` 중 **`5110 px` (`33.5%`) 가 `ndl < 0`** 이다 — 불빛이 켜지는 픽셀이 `773` 의 낮면 평균에 들어간다. 「밤면」 절반 `15507 px` 은 전부 `ndl < 0` 이다. 게이트 평균은 밤면 절반 `0.28642`, 낮면 절반 중 `ndl < 0` 픽셀 `0.12953`.

[도출 — 이 실행의 disk 전체 평균 (낮면 `142.67` · 밤면 `30.58`, Rec.601 8-bit) + 계약 E8 의 OFF 낙차 `52.77%` 인용. `773` 은 `DISK_SAMPLE_RADIUS = 0.95` 라 절대값은 E8 과 다르다 — 보정하지 않았다] 게이트 `1` 당 평균 불빛 휘도를 `X` (분포 밀도 평균 포함, `nightFactor ≈ 1` 상한 근사) 라 하면 낙차 마진 `0.1` 조건은
...
⏩ **R1 도출이 실측으로 재현됐다 (qa D12 독립 재현, 2026-09-18 — §A11.20)**. MN-3 · MN-4 (분포 게이트 제거 → 밤면 전면 발광) 에서 `773` 의 밤면 평균이 **`76.1`** 로 위 예산 상한 (`밤면 ≤ 약 58`) 을 넘고 `773` 이 **FAIL** 한다 (`149.9/76.1` · `139.9/76.1`). 즉 「밤면 평균이 상한을 넘으면 `773` 이 선발동한다」 가 도출이 아니라 관측이 됐다.

⚠️ **그러나 계약 Q3 (i) 「기존 가드가 불빛 과다를 잡는다」 는 절반만 성립한다** [실측]. **MN-1 (황혼 게이트 제거 — 낮면에도 켜짐) 에서 `773` 은 PASS** 다 (`137.3/33.3`, 기준 `136.7/33.3` 과 사실상 불변). 기전은 위 문단이 이미 적은 **`773` 의 화면 반평면 정의** (계약 E7) 다 — 낮면 누출은 밤면 평균을 올리지 않으므로 낙차 판정에 걸리지 않는다. ⇒ **MN-1 축은 신규 가드 `D3` 의 고유 검출력**이고, 기존 가드로 대체되지 않는다.

**R3 (`verify:675-glow-marker`)** — [실측] 40 · 100 AU 에서 지구는 `low` 다 → (A) 에서 불빛이 cluster 계수에 닿는 경로는 **없다** [구조]. D10 에서 증가분을 전·후 인쇄하는 것은 유지한다 (관측).

**낮면만 재는 가드 [구조]** — `verify:1119` (`ndl < DAY_SIDE_MIN_NDL (0.15)` 픽셀 제외 — 스크립트 422행) · `verify:1215` C1 · C2 (낮면 내부) · `verify:1202` G2~G4 (낮면 림) 은 불빛 항이 `ndl ≥ 0` 에서 정확히 `0` 이라 **판정량 불변이 예상된다**. 예상이므로 D10 표에서 확인한다.

### A11.13 Concrete Prediction (구현 후 `git diff --stat` 재현)

| 영역 | 파일 | 예측 |
| --- | --- | --- |
| 셰이더 | `procedural-planet-shader.ts` | GLSL rocky 분기 + 합성 1줄 + uniform (강도 · 색 · 황혼 폭 · 분포 파라미터) + 바인딩 + 헤더 주석 |
| noise 함수 정의 | 4 셰이더 파일 | `float fbm(` · `float hash13(` 정의 수 **불변** |
| FRAGMENT 불변식 | `procedural-planet-shader.ts` | `texture2D(` `1` · `vWorldPos` `2` · `varying` `4` **불변** |
| 구름 · LOD · 정렬 | `cloud-layer.ts` · `body-mesh-factory.ts` · 정렬 함수 | **`0` 줄** |
...
- 불빛 항이 들어간 뒤의 모든 기존 가드 값 (`773` · `783` · `1119` · `1202` · `1215` · `675` · `756` · `1204` · `1205`) — 전부 D10
- 밤면 림에서 rim + 불빛 가산의 `clamp` 비선형 (§A11.3)
- 분포 후보 (d1)/(d2) 의 외형 · shimmer · fps 비용 (hash 증분은 식에서 센 값이다)
- (γ) 게이트의 반경 `16 px` 경계 on/off 가 인지되는지
- focus 상태에서 지구가 지름 `2 px` 까지 `high` 로 유지되는 **원인 코드** (관측만 했다)
- JD `2451808.0` 에서 `verify:1202` 기존 전제 상수 · 대역의 성립 (위상각 · 표본 수만 쟀다)
- 증폭 게이트 맵과 비증폭 맵의 `R == 0` 불일치 픽셀 수 (§A11.10 — 설계 논증이고 실행하지 않았다)
...
- ~~JD `2451808.0` 의 `NI` ∩ 육지 ∩ `iceMask` 포화 픽셀 수~~ — JD `2451626.0` 원자료 `landIce` 는 전 대역 `0` 이었다 (§A11.16 기각 1). ⏩ **해소 (2026-09-18, §A11.18.1)**: U1 JD 에서 재도 `0`, **JD 4대 거점 + 카메라 방위각 7종 전부 `0`** 이고, 원인은 표본 부재가 아니라 **가드가 고정한 카메라 고도**다 — `beta = π/2` 에서 `NI` 안 최대 `|lat|` 이 `46.63°` 인데 극관은 `|lat| ≥ 56.61°` 에서 시작한다. `beta` 를 낮추면 열린다 (`70°` → `25 px` · `30°` → `833 px`). ⚠️ 초판은 이를 「구조적으로 불가」 로 적었고 그 전칭 주장은 **반증됐다** (§A11.18.1). 또 `?rotate=off` 가 자전축 기울기를 `0` 으로 두므로 obliquity 경로는 이 가드에서 발화하지 않는다. 실측 근거는 결함 렌더 `50 px` 전량 덤프 (`diag/m3-mn5b-defect.log`) + `diag/m3b-jd-ice.log` · `m3c-alpha-ice.log` · `m3c2-beta-ice.log`
...
- **낙관적 일정** △ 미통과 — U1 · U2 사용자 왕복 + D1 육안 왕복 + D10 FAIL 시 정지가 모두 남아 있다.
- **결합 간과** △ 미통과 — `773` 반평면 · `783` 극 대역 하단 · `1202` 밤면 림 clamp 는 구조 판독과 도출뿐이다.
- **폐기 프레이밍** ✓ — (B) 기각을 비-범위 인용 없이 축별 근거로만 적었다 (§A11.1 셀프 체크).
- **순수주의** △ — 결정 2 (γ) 는 원거리에서 불빛을 끈다. 「원거리에서도 불빛은 있다」 는 입장과 충돌하므로 외부 검토가 필요하다.

**메인이 cross-validate 프롬프트에 넣을 명시 질문**:

1. (γ) 의 반경 `16 px` 경계 on/off 와 (β) 의 분포 전환 — 어느 쪽이 인지 비용이 큰가
...
1. **MN-5b 극관 억제 제거가 M-i 클래스로 흡수된다** (질문 3 의 2) — 기전이 반대다. `(1 − iceMask)` 를 뺀 결함에서 불빛이 새로 켜지는 픽셀은 육지 ∩ 극관 포화인데, 그 픽셀은 G 에서 `R = landMask · (1 − iceMask) = 0` 이라 **`NI_sea` 로 분류된다**. D5 는 `NI_sea` 무변화를 요구하므로 그 자리에서 잡힌다 — 배제가 아니라 검출 경로다. 판별 불가의 실제 원인은 표본 부재 (JD `2451626.0` 원자료 `nightInner.landIce 0`) 이고, U1 JD 에서의 표본 수를 §A11.14 에 추가했다. ⏩ **반증 (2026-09-18, §A11.18.1)**: 「`NI_sea` 로 분류되어 D5 가 잡는다」 는 실측에서 어긋났다 — 그 픽셀은 `NI` 대역 자체에 들어오지 않고 (`50/50` 전부 `ndv < 0.6`), `9/50` 은 `R > 0` 이라 `R == 0` 전제도 성립하지 않는다. 「판별 불가의 실제 원인은 표본 부재」 도 정정된다 (원인은 대역 정의와 카메라의 조합).
...
- **D1 대리 변형**: `a` = 후보 a 그대로 · `a-d1` / `a-d2` = 전 페이지 host 에 `nightLightLo/Hi` 를 `0.65/0.80` · `0.72/0.85` 로 주입. 사용자 방향 「a 기반 밀도 하향」 의 **대리**다 — dev 가 병행 중인 최종 파라미터가 아니다.

#### A11.17.1 기전 판정 — 구름 자체 휘도는 **판정량에서 상쇄**되고, 교란은 **원안 주입에만** 있다 [실측]

`NI_land` 평균 (`n = 4104`). `α` = 구름 알파 맵 (§A11 README 레시피 — P4 페이지에 백색 표면 + `cloudColor (0,0,0)` 주입, `R = 255·(1 − α)`).

| 변형 | 구름 OFF 불빛 기여 `P1 − P2` | 구름 자체 항 `P2 − P4` | 건강판 분자 `P3 − P4` | 예측 `(1 − α)·(P1 − P2)` | 잔차 평균 / 절대 평균 |
| --- | ---: | ---: | ---: | ---: | --- |
| `a` | `0.11984` | `−0.02608` | `0.08882` | `0.08884` | `−0.00001` / `0.00023` |
| `a-d1` | `0.06172` | `−0.02608` | `0.04486` | `0.04487` | `−0.00001` / `0.00013` |
| `a-d2` | `0.03365` | `−0.02608` | `0.02421` | `0.02423` | `−0.00002` / `0.00008` |

- **(i) 건강판 판정량에는 교란이 없다.** D6 분자 `lum(P3) − lum(P4)` 는 픽셀마다 `(1 − α)·[lum(P1) − lum(P2)]` 와 일치한다 (평균 잔차 `≤ 2e-5`). 구름의 자체 휘도 `α·K` 는 P3 · P4 에 **똑같이** 들어가 차분에서 빠진다. §A11.10 보강 2 의 「구름의 가산 색은 ON/OFF 차분에서 상쇄된다 (clamp 미포화 전제)」 가 실측으로 확인됐다. 선행 실측 1 의 음수 부호 (dev 값 `−0.02608` 을 소수 5자리까지 재현) 는 D6 판정량 자체의 결함이 아니다.
- **(ii) 교란은 원안 MN-6 주입 (P3 단독 `earth-cloud` `setEnabled(false)`, 이하 M6a) 에만 있다.** M6a 프레임은 P1 과 **full frame `0 px` 차이** — 픽셀 동일이다. 그래서 분자가 `[P1 − P2] + [P2 − P4]` 로 정확히 분해되고, 비는 `1 + self/off` 가 된다 (`a`: `1 − 0.02608 / 0.11984 = 0.78238`, 실측 비와 일치). `self < 0` — 밤면 구름이 어두운 육지보다 밝다 (메인 육안 관찰과 정합) — 이라 비가 `1` **아래로** 내려간다.
- **(iii) 원안의 비는 D1 밀도에 역방향으로 의존한다.** 분모 `off` 가 줄수록 `self/off` 의 크기가 커져 FAIL 에서 **멀어진다** — `0.78238 → 0.57749 → 0.22501`. 사용자 D1 방향 (밀도 하향) 은 원안 MN-6 을 더 확실하게 통과시킨다.

#### A11.17.2 후보 비교 [실측 — 「판정」 열의 `K` 는 도출 규칙 (baseline 감쇠율 ÷ 3) 을 **같은 실행의 건강판**에 적용한 예시다. 임계 확정이 아니며, 이 구성에서 건강판 PASS 는 산술상 자명하므로 판별력은 주입판 판정에서만 읽는다]

`NI_land` 비 `R = mean(lum(X) − lum(P4)) / mean(lum(P1) − lum(P2))`. `K` 예시 — `a` `0.08626` · `a-d1` `0.09110` · `a-d2` `0.09350`.

| 후보 | 주입 (프로덕션 코드 `0` 줄) | `R` — `a` / `a-d1` / `a-d2` | 판정 (`R ≤ 1 − K`) | 처분 |
| --- | --- | --- | --- | --- |
| 건강판 (P3) | — | `0.74121` / `0.72671` / `0.71949` | PASS ×3 | 기준. R3 재로드 비 소수 5자리 동일 |
| **C-1 원안** (§A11.10) | P3 에서 `earth-cloud` `setEnabled(false)` | `0.78238` / `0.57749` / `0.22501` | **PASS ×3 — 미검출** | **기각** (§A11.17.1 (ii)(iii)) |
| C-2 양쪽 비활성 | P3 · P4 모두 구름 비활성 | `1` [도출] | 판정 전 `exit 2` [도출 — M6a ↔ P1 픽셀 동일이므로 P4 쪽도 P2 와 같아져 전제 7 표본 `0`] | 기각 (§A11.16 기존 판단 유지 — FAIL 이 아니다) |
| C-3 원안 + 집합 축소 | C-1 을 `NI_land ∩ {P1 ≠ P2} ∩ {P4 ≠ P2}` (불빛 ∩ 구름) 에서 판정. 그 집합의 `K` 예시 `0.16303` / `0.16361` / `0.15954` | `0.84638` / `0.83899` / `0.82010` | FAIL / FAIL / **PASS** | **기각** — 밀도에 따라 판정이 뒤집힌다. 집합이 제품 프레임 (`P1 ≠ P2` 는 불빛 자신) 으로 정의되는 M-i 인접 구조이기도 하다 |
| **C-4 채택 — 불빛 항만 구름 블렌드 뒤에 가산** | §A11.17.3 레시피 | `1.00004` / `1.00007` / `1.00044` | **FAIL ×3** | **채택** |
| C-5 CPU 합성 | 스크립트가 `P4 + (P1 − P2)` 를 픽셀 합성 | `1` (산술 항등) | — | 기각 — 제품 렌더 경로를 한 번도 타지 않는 동어반복이라 판별력 실증이 아니다 (실행하지 않았다) |

⚠️ §A11.10 MN-6 행의 「**표현 불가** — 불빛은 표면 `col` 의 일부라 구름보다 나중에 합성될 경로가 없다」 는 **제품 코드 경로**에 대해서만 참이다. 변이 주입은 제품 경로 밖에서 합성 순서를 만들 수 있고, C-4 가 그 반례다.

#### A11.17.3 채택안 — MN-6 (Q2=(A) 판본) 「불빛 항을 구름 블렌드 뒤로 옮기는 동형 주입」

**결함 문장과의 동형**: 결함 「불빛이 구름 위에 그려짐」 의 합성식은 `C = (1 − α)·S + α·K + L` 이고, 건강판은 `C = (1 − α)·(S + L) + α·K` 다 (`S` 표면 · `K` 구름색 · `L` 불빛 항). C-4 는 host 의 불빛 세기를 `0` 으로 두어 표면을 P4 와 같게 만들고, 불빛 항**만** 싣는 host clone 을 구름이 속한 rendering group 0 **전체 뒤** (group 1) 에 `ALPHA_ADD` 로 그린다 — 앞 식 그대로다. clone 의 셰이더 출력은 식상 `clamp(col·0 + 0 + nightLightColor · lights)` 다 (`sunIntensity 0` · `ambientIntensity 0` 이면 `shade = 0`, `rimStrength 0`). 구름 알파가 불빛에 곱해지는 경로가 레시피에 없으므로 `R = 1` 은 `α` 분포와 무관하다 (구조).

**Developer 인수인계 — 박제 대상 레시피 (raw text)**
...
- 원 설계의 「P3 단독 구름 비활성」 은 쓰지 않는다. 전제 7 (`NI_land` 에서 `P4 ≠ P2` 표본 하한) 은 §A11.10 보강 2 그대로다 — 채택안은 P4 를 건드리지 않으므로 전제는 건강판과 같은 값이다 [실측 `2083 px`, dev 선행 실측 `item1_changedPx_NI_land 2083` 과 일치].

**주입 유효성 전제** — 변이 결과를 쓰기 **전에** 같은 실행에서 확인한다. 하나라도 위배되면 그 변이 실행은 PASS/FAIL 이 아니라 **무효** (주입 도구 고장) 로 보고한다. 가드 본체의 「측정 불가」 전제가 아니라 D12 변이 하네스의 전제다.

| # | 확인 | 실측 (`a`) |
| --- | --- | --- |
| V1 | overlay 를 세기 `0` 으로 설치한 프레임 ↔ 설치 전 (P4) — full frame 변화 `0 px` (설치 자체의 부작용 없음) | `0 px` (3 변형 모두) |
| V2 | 구름 OFF · 불빛 OFF 페이지 (P2) 에 세기 `S` overlay 를 얹은 프레임의 `NI_land` 평균 기여 = P1 기여, 픽셀당 차이 `≤ 1 LSB` (overlay 가 불빛 항을 충실히 재현) | `0.11984` vs `0.11984` · 최대 `|Δlum| 0.00392` (= 1/255). `a-d1` `0.06173` vs `0.06172` · `a-d2` `0.03367` vs `0.03365` |
| V3 | 1 단계만 적용한 P3 프레임 ↔ P4 — full frame `0 px` (host 불빛 차단) | `0 px` |
| V4 | 6 단계 원복 후 ↔ 주입 전 P3 — full frame `0 px` | `0 px` |
| — | P3 판본 (1~5) ↔ P4 페이지에 overlay 만 얹은 판본 (M6b) — full frame `0 px` · 주입 전 대비 변화 `1662 px` | 둘 다 실측 |

즉 원본 (P3) → 변이 (1~5) → 원복 (6) 3단이 **같은 페이지에서** 성립했다 (`mn6-probe-p3inject-a.json`).
...
#### A11.17.4 D6 판정량 — 정의 무변경, 정밀화 1건

- **판정량 무변경**: §A11.10 보강 2 그대로 — `R = mean_NI_land(lum(P3) − lum(P4)) / mean_NI_land(lum(P1) − lum(P2))`, 판정 `R ≤ 1 − K_OCC`, `K_OCC` = baseline 감쇠율 `(1 − R_baseline)` ÷ 3. 판별력 하한은 계약 결함 문장의 **문자 그대로의 부정**이다 — 「불빛이 구름 위에 그려지지 않는다」 ⇔ 구름 알파만큼 불빛이 감쇠된다 ⇔ `R < 1`. 그 결함의 합성식을 그대로 구현한 C-4 에서 `R = 1.0000x` 가 나왔다.
- **정밀화 1 — D2 선행 결합**: D6 는 **D2 PASS 뒤에만** 판정한다. 분모 (구름 OFF 불빛 기여) 가 `0` 근처면 비가 정의되지 않거나 8-bit 반올림이 지배한다. 불빛 소멸은 D2 가 FAIL 로 잡는 **제품 속성**이라 「측정 불가」 전제로 올리지 않는다 (#1215 X2 — 하네스 전제와 제품 게이트의 분리).
- **기각 — 판정 집합 축소**: `NI_land ∩ {P4 ≠ P2}` 에서 재면 `K` 여유가 커진다 [실측 `K` 예시 `0.16303` vs `0.08626`, 건강판 `R` `0.51090` · C-4 `0.99997`]. 그러나 판정 집합이 구름 제품 프레임으로 정의돼 구름 투명화 결함이 집합과 대상을 함께 옮기고 (#1197 M-i 인접), 채택안의 판별에 필요하지 않다.

#### A11.17.5 D1 파라미터 의존성

- **채택안 MN-6 의 FAIL — 측정 범위에서 D1 비의존.** `R = 1.00004 / 1.00007 / 1.00044`. `|1 − R|` 은 기여가 줄수록 커지지만 (8-bit 반올림의 상대 비중) `K` 예시보다 약 `200` 배 이상 작다 [도출 — 최소 여유 `a-d2` `0.09350 / 0.00044 ≈ 212`].
...
- **재실측이 필요한 조건** — 도출 규칙은 그대로 두고 수치만 다시 잰다:
  ...
  3. 분포가 구름 noise 와 **상관을 가질 수 있는 변경** (패턴 교체 · `clusterMix` · 구름과 같은 입력 공간이나 주파수 사용), **또는 현행 분포와 구름 noise 가 이미 입력 공간 · 시드 · 격자 주파수를 공유하고 있음이 확인될 때** — 가중 평균 기전의 전제 (공간 독립) 가 흔들린다. ⏩ **술어 개정 (2026-09-18, §A11.19 합의 3)**: 뒤의 절은 신설이다. 변화량만 트리거로 쓰면 「현행 상태가 이미 전제를 깨고 있는 경우」 를 영구히 놓친다 (조건 5 에서 실제로 그 일이 났다 — §A11.17.9).
  4. **구름 파라미터** (`cover` · `opacity` · `frequency`) · **측정 JD** · 구름 drift 변경, **또는 현행 측정 JD · 카메라 리그에서 검증 대상 대역의 가시성 전제가 성립하지 않을 때** — `α` 분포가 바뀐다. ⏩ **술어 개정 (2026-09-18)**: 뒤의 절은 신설이고 **실제 사례가 있다** — §A11.18.1 의 극관은 카메라 고도 `beta = π/2` 고정 때문에 `NI` 대역 밖에 놓이는데, 아무도 JD·구름 파라미터를 「변경」하지 않으므로 원 술어로는 영구 침묵이었다.
  5. 레시피가 의존하는 **Babylon 내부 필드** (`_options` · `_floats` 계열 맵 · group 자동 depth clear) 가 엔진 업그레이드로 바뀜, 또는 **렌더 파이프라인 설정** (MSAA · 톤매핑 · 후처리 · 그룹 0 뒤에 그려지는 레이어 도입 · `renderingGroupId` 1 을 쓰는 제품 코드 등장) 변경, **또는 현행 값이 `V2` 의 전제를 깨는지** — `V2` 가 먼저 깨지는 자리다. ⏩ **술어 개정 (2026-09-18, §A11.17.9)**: 뒤의 절은 신설이다. **이 조건은 원인 (MSAA) 을 이름으로 담고도 발화하지 않았다** — MSAA 는 처음부터 켜져 있고 「변경」되지 않았기 때문이다.

#### A11.17.6 앞 절과의 관계 (원문 소급 수정 `0`)

- §A11.10 계약 변이 표 MN-6 행 — 주입과 기대값 미확정 문구를 본 절 §A11.17.3 이 **대체**한다. 기대 「D6 FAIL」 과 전제 7 은 유지된다.
- §A11.10 보강 2 「구름의 가산 색은 ON/OFF 차분에서 상쇄된다」 — 실측으로 **확인** (§A11.17.1 (i)).
- §A11.14 「MN-6 주입 시 D6 판정량의 부호」 — **해소**. dev 실측 `−0.02608` 을 본 실측이 같은 값으로 재현했다.
- §A11.16 수용 1 — 교란의 존재는 확인했다. 처방은 「기대값을 부호에서 도출」 이 아니라 **주입 교체**다 (부호가 음이면 원안은 구조적으로 미검출이고, 양이더라도 D1 밀도에 의존한다).
- 계약 #1226 D12 MN-6 행 「Q2=(A): 합성을 구름 블렌드 뒤로 옮기는 동형 주입 → D6 FAIL」 — 문면 그대로 성립한다. 계약 변경 없음.

#### A11.17.7 미확인 (실행되지 않은 것 — 근거로 인용 금지)

- CI 렌더러 (GitHub Actions swiftshader) 에서의 위 실측값 — 전부 로컬 swiftshader, 변형당 1회 (+ 건강판 재로드 1회)
- WebGPU · 실 GPU 에서 overlay 레시피의 거동 (group 1 depth 재사용 · `ALPHA_ADD`)
- **D1 최종 파라미터** — `a-d1` · `a-d2` 는 `lo/hi` 만 바꾼 대리다. 황혼 폭 · 색 · `clusterMix` · 주파수 · 패턴 변경은 재지 않았다
- JD `2451808.0` 외의 JD
- C-2 (P3 · P4 양쪽 구름 비활성) 의 `exit 2` — M6a ↔ P1 픽셀 동일에서 도출했을 뿐 실행하지 않았다
- C-5 (CPU 합성) — 실행하지 않았다
- Babylon `ShaderMaterial.clone()` 이 기본적으로 `_options` 를 공유하는지 (레시피는 복사를 강제했다)
- mid LOD · fade 창 · `?rotate` ON 에서 overlay 레시피의 거동 — focus `high` 정착 프레임만 쟀다
...
- **낙관적 일정** ✓ — 일정 주장이 없다. 남은 왕복 (D1 재승인 · cross-validate) 은 본 절 결정과 독립이다.
- **결합 간과** △ 미통과 — 레시피가 scene 설정 (group 1 자동 depth clear) 을 바꾼다. V1 이 full frame 부작용 `0` 을 보지만 focus `high` 한 프레임에서만이다.
- **폐기 프레이밍** ✓ — C-1 기각은 3 변형 실측 수치로만 근거를 댔다.
- **순수주의** △ — 채택안은 Babylon 내부 필드 (`_options` · `_floats` 계열) 에 의존한다. 「공개 API 만」 원칙과 부딪칠 수 있어 주입 유효성 전제 V1~V4 로 보완했다. 외부 검토가 필요하다.

**메인이 cross-validate 프롬프트에 넣을 명시 질문 (제안)**:
...
#### A11.17.9 V2 전제 술어 재정의 — **Accepted** (cross-validate agy 2026-09-18, §A11.19 반영 완료)

> ⚠️ **이 절의 초판 (2026-09-18 Provisional) 처방은 cross-validate 와 후속 실측으로 반증됐다.** 반증된 것은 **처방**이고, 원인 규명 (MSAA 4x resolve) 은 그대로 유효하다. 무엇이 어떻게 뒤집혔는지는 아래 §「초판 처방의 반증」 에 남긴다 — 회수 키이므로 지우지 않는다.

**발화 경위.** D1 최종 승인값 `g1` 의 군집 게이트는 분포 패턴 변경이므로 §A11.17.5 **재실측 조건 3** 을 발화시켰다. 그 재실측에서 `V1` · `V3` · `V4` 는 충족이고 변이도 픽셀을 `988` 바꾸는데 **`V2` 만** 위배됐다 — 4회 전건 (`repeat2` · `repeat3` · `MN6_SKIP_DEPTH_WRITE=1` 판본 포함) `exit 2`.

[실측 `docs/reports/1226-night-lights/phase2/mutations/mn6.log:21`] `V2` = overlay 기여 `0.038509` vs P1 `0.038502` · 최대 `|Δlum| 0.005059` · **1 LSB 초과 `1 px`**. §A11.17.3 원문의 `V2` 실측 (후보 `a`) 은 최대 `|Δlum| 0.00392` (= 1/255) 로 정확히 1 LSB 였다 — `g1` 에서 `≈ 1.3/255` 로 올라가 술어 `≤ 1 LSB` 를 넘겼다.

**원인 — MSAA 4x resolve. 개입 실험으로 확정.**

환경 [실측 `diag/m1b-mn6-quant.log`]: `ANGLE (Vulkan 1.3.0 SwiftShader)` · **`gl.SAMPLES 4`** · `SAMPLE_BUFFERS 1` · post-process `0` · tone mapping `false` · DPR 1 · renderWidth = canvas = 스크린샷 (스케일 `1`).

- 문제 픽셀은 좌표 `(701, 395)` 하나다. `ndl −0.583147` · `ndv 0.694246` · `r/R 0.7197` — **terminator 도 disk 윤곽도 아니다** (경계는 `ndl 0` · `r/R 1`). 실제 정체는 밝은 불빛 군집 경계 + **삼각형 경계**다: sub-pixel faceId 가 `2739` · `2740` **두 개**이고 이웃 `(702,395)` 이 10배 이상 밝다 (국소 기울기 `76`, `NI_land` 평균 `18.51`).
- **3방향 분해** [실측] — `P2 = q(S)` · `H = q(S+N)` (= P1, full frame `0 px` 로 동일) · `NA = q(N)` (불빛 항만) · `O` = overlay 2-pass:
  - `d1 = (H − P2) − NA = [+1, −2, −1]` — **host 단일 패스 자신이 이미 분해 불가**. 히스토그램 `{0:3701, 1:402, 2:1}`
  - `d2 = (O − P2) − NA = [0, 0, 0]` — **overlay 는 host 의 불빛 항을 완전 재현**. 히스토그램 `{0:4087, 1:17}` (MSAA 켜진 채로 전량 `≤ 1 LSB`)
  - ⇒ `V2` 가 실패한 원인은 overlay 결함이 아니라 **비교 기준 `P1` 자체가 2 LSB 흔들리는 것**이다.
- **개입** [실측 `diag/m1e` · `m1f`] — `HTMLCanvasElement.prototype.getContext` 후킹으로 `antialias:false` (`glSamples 0` 확인) 하면 2 LSB 픽셀이 소멸하고 `V2` 가 `0 px` 로 통과한다. `d2` 도 `{0:4104}` 로 전량 `0`.
- **배제** [전부 실측] — `gl.disable(DITHER)` 시 히스토그램·대상 픽셀 비트 동일 (Vulkan 백엔드에 dither 상태 없음과 일치) / `readPixels` 3×3 블록이 PNG 값과 동일 (캡처·합성 경로 무관) / Geometry 객체 공유 + overlay world matrix 가 host 와 원소 전부 동일 (기하 차이 없음) / `MN6_SKIP_DEPTH_WRITE=1` 로 §A11.17.3 보강 1 을 철회해도 같은 값 (§A10.5 「블렌드 draw 에서 무효」 와 일치) / `d2 = 0` 이 정렬·깊이 축도 배제한다.
- **강도 의존성이 단조가 아니다** [실측] — `0.45` → `0 px` · `0.9` → `1 px` · `1.8` → `0 px`. 반올림 knife-edge다. ⇒ **`> 1 LSB px == 0` 은 MSAA 하에서 구조적으로 보장되지 않는다.**
- 이 기전은 **서로 다른 합성 경로 2개를 비교하는 술어에만** 걸린다. `D3` · `D5` 는 같은 경로의 두 프레임 비교라 무관하다 [실측 — 전 실행 `0 px`].

**⚠️ 조건 5 는 원인을 이름으로 담고도 발화하지 않았다.** §A11.17.8 이견 5 (agy) 가 MSAA 를 조건 5 에 넣게 했으나 술어가 「**도입** · **변경**」 이었다. MSAA 는 이 저장소에서 **처음부터 켜져 있고 바뀌지 않았다** (`packages/core/src/engine/engine-factory.ts:43` `new Engine(canvas, true, …)` · `:30` `antialias: true`). ⇒ 조건 5 의 술어를 **「변경」 + 「현행 값이 전제를 깨는지」** 양쪽으로 넓혔다. 「가드 술어가 계약보다 좁다」 의 변형이고, 여기서는 **트리거를 변화량으로 쓴 것**이 축이었다.

⏩ **cross-validate 가 같은 결함을 두 곳 더 찾았다 (§A11.19 합의 3)** — 조건 **3** (분포 ↔ 구름 noise 상관) 과 조건 **4** (구름 파라미터 · 측정 JD) 도 「변경」만 트리거로 썼다. 특히 조건 4 는 **실제 사례를 이미 갖고 있었다**: §A11.18.1 의 극관이 카메라 고도 고정 때문에 `NI` 대역 밖에 놓이는데, 아무도 JD 를 「변경」하지 않으므로 영구 침묵이었다. 세 조건 모두 §A11.17.5 에서 개정했다.

**초판 처방의 반증**

초판은 `V2` 를 **`V2-a`** (`d2 = (O − P2) − NA` 의 `> 1 LSB px == 0`, 주판정) + **`V2-b`** (`O ↔ P1` 의 `> 1 LSB px ≤ 3`, 백스톱) 로 나누고 `d1` 을 진단 인쇄로 두었다. 세 갈래로 반증됐다.

1. **`V2-b` 임계 `3` — 「의사 형식주의」 (agy [High], 수용).** 원 관례 `K_OCC = (1 − R_baseline)/3` 의 `/3` 은 *물리적 감쇠율*의 유의 변위 한계를 잡는 안전율인데, 이를 단일 관측 정수 `1` 에 곱한 `1 × 3` 은 형식만 빌린 것이다 — `1` 은 표준편차가 아니라 서브픽셀 knife-edge 좌표 `(701,395)` 한 곳의 이산 아티팩트다. 구면 메시에는 폴리곤 경계가 수천 개 있어 JD·카메라·텍스처 변경으로 오차 픽셀이 튈 수 있다.
2. **`NI_land` 한정이 누출 결함을 원천적으로 놓친다 (agy [High], 실측 실증).** `V2-a` · `V2-b` 가 **둘 다** `NI_land` (4104 px) 안만 본다. `V1` 은 세기 `0` 으로 설치해 누출이 구조적으로 `0` 이고 `V4` 는 원복 후이므로, **세기 `S` 에서 `NI_land` 밖을 보는 술어가 없었다.** [실측 `diag/m1j-leak-scope.log`] 주간 반구 `21×21` 사각형에만 기여를 더하는 누출 고장 (게이트·`nightFactor` 우회) 에서 `NI_land` 판정량이 **건강판과 완전히 같다** (`> 1 LSB` `1` · `≥3` `0` · 성분 `0` · `d2` `0` · 상대오차 `0.0184 %`). `disk` 로 넓히면 `442 px` (`≥3` `441` · 단일 성분 크기 `441`) 로 FAIL 한다.
3. **`d2` 는 범위를 넓힐 수 없다 (실측, 기전 확정).** `NA` 는 disk **안에서만** 순수 불빛 기여이고 밖에서는 **배경** (clear color `(8,9,13)` + starfield) 이다. 따라서 `d2 = (O − P2) − NA ≈ −배경` → 건강판 `disk` `271 px` · full frame **`891,103 px`** (배경 픽셀 전량). 확장하려면 배경을 뺀 기준 프레임이 필요하고 현 레시피는 그것을 만들지 않는다. ⇒ `V2-a` 를 주판정으로 두는 한 위 2번을 닫을 수 없다.

부수로 **`d1` 연동 동적 임계** (agy 대안 A) 도 기각했다 — 허용치를 다른 측정량에 연동하면 **그 측정량이 새 공격면**이 된다. [실측] `NA` 고장이 `d1` 을 `172` / `511` / `507` 로 부풀리므로, `NA` 주입이 틀어진 상태에서 허용치가 `173~512` 로 열려 L1′(`12`) · L1(`17`) · L2(`116~384`) · 전역 a(`172`) · 전역 b(`444`) 까지 통과할 수 있다 [도출].

**확정 — `V2` = `O ↔ P1` 을 `disk` 범위에서, 술어 둘, `NA` 불도입** (사용자 결정 2026-09-18)

**핵심 논거 (구조)**: `O ↔ P1` 이 같으면 `(O − P2) = (P1 − P2)` 이고 우변은 host 의 불빛 기여다. 즉 **「overlay 기여 = host 기여」 가 `NA` 없이 직접 따라온다.** 초판이 `NA` 를 들인 이유는 「건강판 `0`」 을 얻기 위해서였는데, 아래 술어가 `NA` 없이 그것을 달성한다.

| # | 술어 (판정 집합 = `disk`, `n 30768`) | 건강판 | 잡는 형태 |
| --- | --- | ---: | --- |
| **V2-①** | `O ↔ P1` 의 **`chMax ≥ 3` 인 px `== 0`** | **`0`** | **오차 크기** — MSAA resolve 실측 상한이 `2 LSB` 이므로 `3` 이상은 신호다 |
| **V2-②** | `O ↔ P1` 의 `> 1 LSB` 픽셀에서 **크기 `≥ 2` 인 4-이웃 연결 성분 `== 0`** | **`0`** | **공간 구조** — MSAA 오차는 고립 1픽셀로만 나타난다 [실측 건강판 성분 히스토그램 `{1: 1}`] |
| (진단 인쇄 — 술어 아님) | `> 1 LSB` px 계수 · `chMaxHist` · 최대 `\|Δlum\|` · 평균 기여 상대오차 | `1` · — · `0.005059` · **`0.0059 %`** (`disk`) | 위배 시 판독용 |

- **새 임계 `0` 개.** 둘 다 `== 0` 이고, 두 상수 (`3` · `2`) 는 임계가 아니라 **MSAA 오차의 실측 상한** (`chMax ≤ 2`) 과 **최소 비고립 크기** (`2`) 다. §A11.17.8 이견 4 의 「수치 발명 금지」 를 위반하지 않는다. ⏩ **보강 (2026-09-18, reviewer 권고)**: 두 술어가 **`== 0` 불변성 검사**이므로 표본이 비면 증거 `0` 으로 통과한다 (#1201 §감사 시그니처 2 동형 — reviewer 가 `v2.n` 을 보는 판정부가 없음을 `git grep` 으로 적발했다). `v2.n === 0` 을 **「측정 불가」** 로 올렸다 — 임계가 아니라 **공집합 판정**이라 위 「새 임계 `0` 개」 는 유지된다.
- ⚠️ **진단 인쇄의 상대오차는 두 범위를 함께 낸다** — 판정 집합이 `disk` 라 그 범위 값은 `0.0059 %` 이고, ADR·보고서가 인용해 온 `NI_land` 값 `0.0184 %` 도 계속 재현되도록 둘 다 인쇄한다 (새 프레임·새 상수 없이 이미 찍는 게이트 맵만 재사용). 위 표의 셀은 **판정 집합 기준**이다.
- **범위가 `disk` 인 근거** [실측 `diag/m1i-conn-scope.log`]: `O ↔ P1` 의 건강판 기준선이 `NI_land` (`n 4104`) → `disk` (`30768`) → full frame (`921600`) 에서 **완전히 불변**이다 — 같은 한 픽셀 `(701,395)` 뿐이고 `≥3` 과 「성분 ≥2」 는 세 범위 전부 `0`. 표본이 `225배` 커져도 MSAA 꼬리가 늘지 않았다. `disk` 를 고른 것은 full frame 이 starfield·배경을 판정 집합에 들이는 데 비해 이득이 없기 때문이다 (누출 고장은 `disk` 에서 이미 `442 px` 로 잡힌다).
- **둘을 함께 두는 이유 — 검출 토대가 다르다.** [실측] 같은 `≥3` 술어가 `O↔P1` 에서는 전역 `S×1.01` 을 `10` 으로 잡는데 `d2` 에서는 `0` 으로 **놓친다** (그 고장의 `d2` 는 `2` 에 `172 px` 이 몰려 있다). 즉 **작지만 뭉친 오차**는 크기 술어의 사각이고, 그 형태를 V2-② 가 잡는다 (그 판본의 성분 크기 ≥2 가 `29` 개). 역으로 큰 오차가 흩어져 있으면 V2-① 이 잡는다. 각각 단독으로도 측정한 전 고장을 잡지만 (아래 격자) 사각의 방향이 직교하므로 둘을 남긴다 — 임계 증가가 `0` 이라 비용이 없다.
- **`NA` 불도입으로 함께 닫힌 것** — 새 주입 경로 `0` 이므로 (i) 「전제의 전제」 (주입이 틀리면 전제가 무의미해지는 회귀) 가 성립하지 않고 (ii) cross-validate 가 [High] 로 지적한 **「`NA` 와 `O` 가 같은 결함을 공유하면 차분 술어가 눈이 먼다」 축이 소멸**하며 (iii) 「`d1` 을 술어로 올릴지」 논쟁도 소멸한다. 초판이 `V2-b` 를 백스톱으로 둔 이유가 (ii) 였는데, `NA` 를 쓰지 않으면 막을 대상 자체가 없다.
- **CI 리스크 없음** [실측] — `V2` 는 D12 변이 하네스 전용이고 CI 는 `INJECT` 없이 기본 모드만 돌린다 (`.github/workflows/shader-pixel-guard.yml:215-220` — 같은 파일 `:214` 주석이 「변이 주입은 두 번째 서버 · 소스 변이가 필요해 CI 에 배선하지 않는다」 로 선언한다). 따라서 §A11.17.7 의 「CI 렌더러 미확인」 은 `V2` 판정에 닿지 않는다. ⚠️ cross-validate 는 이를 「본말전도 — 본 ADR 의 목적이 향후 D12 를 CI 게이트로 안착시키는 것」 이라 반박했으나 **기각한다**: D12 의 CI 배선 불가는 계획이 아니라 **구조적 제약** (두 번째 서버 + 소스 재빌드) 이고 계약은 「PR 시점 1회 의무」 로 규정한다 (§A11.19 이견 2). 단 같은 지적에 딸린 **로컬 렌더러 변동** 우려는 유효하고, 위 두 술어가 그것까지 흡수한다 (기준선이 범위·표본 수에 불변).

**판별력 격자 (a) — `NI_land` 범위 실측** [`diag/m1d` · `m1g` · `m1h` · `m1i` · `m1j`] — `n 4104` (건강판 lit `655`). 「영향 px」 = 고장 overlay 프레임이 건강 overlay 프레임과 8-bit 채널이 다른 픽셀 수. `V2-①` = `O↔P1` 의 `chMax ≥ 3` · `V2-②` = `O↔P1` `>1 LSB` 의 크기 ≥2 성분 수 · 참고로 기각한 `(F)` (`d2 > 1 LSB`) 와 `(C)` (상대오차) 를 함께 싣는다.

| 고장 | 영향 px | **V2-①** | **V2-②** | `(F)` | `(C)` | `d1` |
| --- | --: | --: | --: | --: | --: | --: |
| **건강** | `0` | **`0`** | **`0`** | `0` | `0.0184 %` | `1` |
| 전역 — `S×1.01` | `445` | `10` | `29` | `172` | `1.0424 %` | `1` |
| 전역 — `S×1.05` | `557` | `376` | `37` | `439` | `5.0097 %` | `1` |
| 전역 — `ALPHA_COMBINE` | `4104` | `4104` | `7` | `4104` | `132.65 %` | `1` |
| 국소 — 주파수 `×(1+1e-4)` | `278` | `60` | `23` | `113` | **`0.0809 %`** | `1` |
| 국소 — 주파수 `×(1+5e-4)` | `359` | `216` | `54` | `267` | **`0.4627 %`** | `1` |
| 국소 — 주파수 `×(1+2e-3)` | `440` | `352` | `68` | `386` | `2.2347 %` | `1` |
| 국소 — 밝은 `7×7` kill | `19` | `15` | `1` | `16` | `1.2998 %` | `1` |
| 국소 — 어두운 `7×7` kill | `14` | `10` | `2` | `11` | **`0.2062 %`** | `1` |
| **누출 A — 주간 반구** | **`0`** | **`0`** ❗ | **`0`** ❗ | **`0`** ❗ | `0.0184 %` ❗ | `1` |
| 누출 B — `lightGate = 1.0` | `44` | `35` | `9` | — | `2.9014 %` | `1` |
| 전제 — `NA` strength `×1.01` | `0` | `0` | — | `172` | `0.0184 %` | **`172`** |

- **`NI_land` 범위에서는 누출 A 를 어떤 술어도 잡지 못한다** (❗ 행 — 전 판정량이 건강판과 동일). 그것이 범위를 `disk` 로 넓힌 이유다.
- **L2 계열의 `chMax` 꼬리** [실측] `{… 6:3}` / `{… 33:1}` / `{… 116:1}` — 미세 변조라 `1~2` 에 머물 것이라는 예상은 **반증됐다** (메인 우려, §A11.19 Claude 셀프 체크 1).
- ⚠️ `V2-②` 열의 성분 수는 **`NI_land` 범위 실측**이다. `disk` 범위의 고장별 성분 수는 전량 재지 않았다 — 고장 픽셀이 `NI_land` ⊂ `disk` 이므로 **`disk` 값은 이 값 이상**이다 [도출, 단조성]. 건강판과 누출 A 만 `disk` 에서 직접 쟀다 (아래 (b)).

**판별력 격자 (b) — 범위 확장 실측** [`diag/m1i` · `m1j`]

| 판본 · 범위 | n | `O↔P1 > 1` | **`V2-①`** | **`V2-②`** | `(F)` = `d2 > 1` |
| --- | --: | --: | --: | --: | --: |
| 건강 · `NI_land` | `4104` | `1` | `0` | `0` | `0` |
| 건강 · **`disk`** | `30768` | `1` | **`0`** | **`0`** | `271` |
| 건강 · full frame | `921600` | `1` | `0` | `0` | `891103` |
| 누출 A · `NI_land` | `4104` | `1` | `0` | `0` | `0` |
| 누출 A · **`disk`** | `30768` | `442` | **`441`** | **`1`** (크기 `441`) | `712` |
| 누출 A · full frame | `921600` | `442` | `441` | `1` | (무효) |

- 누출 A 는 `ndl ≥ 0.15` 영역에만 기여를 더하므로 `NI_land` (`ndl ≤ −0.15`) 와 **교집합이 구조적으로 `0`** 인 순수 누출이다. 주입 4중 실증 (치환 `1/81` 청크 · 페이지에서 읽은 실제 셰이더 소스 확인 · **host 3 프레임 full frame `0 px`** · 마커 없는 음성 대조 `0 px`).
- 누출 B (`lightGate = 1.0`) 는 해안선·극관 계조에서 게이트가 올라가 `NI_land` 도 `44 px` 건드리므로 현행 범위가 **우연히** 잡는다. 누출의 `1480 / 1521 px` 은 범위 밖이다. `DI` 가 `0` 인 이유는 `nightFactor` 가 `ndl ≥ 0` 에서 정확히 `0` 이라 게이트만 열어도 주간 누출이 생기지 않기 때문 — 그래서 순수 주간 누출을 재려면 누출 A 처럼 `nightFactor` 까지 우회해야 한다.

국소 고장의 주입 유효성은 4중으로 실증했다 [실측] — 셰이더 치환 `1/81` 청크 (0건이면 throw) · 페이지에서 읽은 실제 셰이더 소스 확인 · **host 3 프레임 (`P2` · `NA` · `H`) 불변 `0 px`** · 마커 없는 음성 대조가 건강판과 동일. 마커는 `craterDepth` (rocky 분기에서 출력 영향 `0` 인 uniform) 를 썼다 — `sunIntensity` 계열을 게이트로 쓰면 `NA` 프레임 (`sun 0`) 이 **같은 서명**이라 V2-a 가 구조적으로 눈이 먼다.

**기각한 후보**

1. **(C) 평균 기여 상대오차 `≤ 0.5 %` — 기각. fail-open 이 실측됐다.** 위 격자에서 **고장 3종** (주파수 `1e-4` · `5e-4` · 어두운 kill) 이 통과한다. 사각의 축은 「국소성」이 아니라 **「손실 픽셀의 기여 몫」과 「부호 상쇄」** 다 — 같은 `7×7` 사각형인데 기여 합이 5.8배 차이 나 밝은 쪽은 `1.2998 %` (FAIL) · 어두운 쪽은 `0.2062 %` (통과) 이고, 주파수 변이는 일부가 밝아지고 일부가 어두워져 평균이 보존되면서 `278 px` 를 바꾼다. ⚠️ 이 반증은 **메인이 「국소 완전 손실이면 평균이 통과할 것」 이라는 [추론] 으로 요청한 측정에서 나왔고, 추론의 결론은 맞았으나 기전은 정정됐다.**
2. **`(F)` `d2 > 1 LSB px == 0` — 기각 (초판의 주판정).** `NI_land` 에서는 성립하나 **범위를 넓힐 수 없어** 누출 A 를 구조적으로 놓친다 (위 §초판 처방의 반증 3). `NA` 주입이라는 새 경로를 요구하는 것도 비용이다.
3. **`(i′)` `d2` 에 `chMax ≥ 3` 적용 — 기각. fail-open 이 실측됐다.** 전역 `S×1.01` 의 `d2` 는 `2` 에 `172 px` 이 몰려 `≥3` 이 **`0`** 이다. 같은 술어가 `O↔P1` 에서는 `10` 으로 잡는다 — `O↔P1` 은 `d1` 반올림 잡음이 신호에 더해져 일부를 `3` 으로 밀어올리고, `d2` 는 잡음이 조용해 (`17 px at 1`) 신호가 `2` 에 머문다. **잡음이 큰 쪽이 높은 임계에서 더 잘 잡는다** — 임계를 올릴 때 어느 비교량에 올리는지를 함께 정해야 한다는 뜻이고, 이것이 V2-① 의 비교량을 `O↔P1` 로 못박은 이유다.
4. **`(iii)` `d1` 연동 동적 임계 — 기각** (위 §초판 처방의 반증 말미. 허용치를 다른 측정량에 연동하면 그 측정량이 새 공격면이 된다).
5. **`> 2 LSB px == 0` 으로 완화 — 기각.** 초판 맥락에서 판정했다. 건강판 (최대 `2`) 은 통과하나 최소 현실 고장 (`S×1.01`) 을 잡는 근거가 `chMax 3` 인 **`10 px` 뿐**이라 여유가 가장 얇다 [실측]. ⚠️ 확정안 V2-① 이 바로 이 술어와 **같은 형태**다 — 차이는 (a) 판정 집합이 `disk` 라 누출을 함께 보고 (b) V2-② 가 「작지만 뭉친 오차」 축을 보완한다는 것이다. 여유 `10 px` 이 얇다는 판정 자체는 유효하므로 **단독으로 쓰지 않는다.**
6. **판정 집합에서 삼각형 경계 픽셀 제외 — 기각.** 결함 픽셀도 함께 빠질 수 있고, 판정 집합을 렌더 구조로 정의하는 것은 #1197 M-i 인접이다.
7. **가드 렌더를 `antialias:false` 로 — 기각.** `V2` 는 `0 px` 로 통과하고 판별력도 유지되나 (`167`), **전 판정량이 바뀌어** baseline 전면 재도출이 필요하고 「가드가 제품 렌더 설정을 바꿔서 잰다」 는 새 비대칭을 들인다.

**미확인 (근거로 인용 금지)**

- 확정한 `V2-①` · `V2-②` 로 **MN-6 을 실제로 통과시켜 D6 FAIL 을 확인한 실행** — 구현 전이다. `D6` 의 변이 검출력은 **아직 미확인**이고, 그것이 이 개정의 목적이다.
- **`disk` 범위에서 고장별 `V2-②` 성분 수** — 건강판과 누출 A 만 쟀다. 나머지는 `NI_land` 값이 하한이라는 도출에 기댄다.
- 국소 고장 3종 · 누출 고장 2종은 **화면 좌표 사각형 또는 게이트 치환** 기반 합성 결함이다 — 제품 코드로 도달하는 결함의 분포와 같은지는 재지 않았다.
- `O ↔ P1` 의 건강판 `1 px` 이 다른 JD · 다른 카메라 · 다른 해상도에서도 `chMax ≤ 2` 에 머무는지. ⚠️ cross-validate 가 지적한 대로 `|round(A+B) − round(A) − round(B)| ≤ 1.5 LSB` 이므로 **`2 LSB` 는 산술 상한이 아니라 실측 관측**이다 (§A11.19 합의 4). 상한을 넘는 픽셀이 나오면 V2-① 이 허위 발화하고, 그때는 `chMax ≥ 4` 가 아니라 **왜 상한을 넘었는지**를 먼저 본다.
- cross-validate 가 격자 누락으로 지적한 결함 형태 — 텍스처 어드레싱·UV 래핑 / 프레임 동기화 지연 (`onBeforeRenderObservable` 복사 uniform 과 draw call 의 1프레임 lag) / LOD 서브메시 전환 시 overlay 형상 불일치. 셋 다 주입하지 않았다 (§A11.19 고유 발견 4).
- CI 렌더러에서의 위 전 수치 (전부 로컬 swiftshader) — 단 위 「CI 리스크 없음」 대로 `V2` 판정 경로에는 닿지 않는다.

---

### A11.18 MN-5b 예측 반증 · MN-7 — **Accepted** (cross-validate agy 2026-09-18, §A11.19 반영 완료)

#### A11.18.1 §A11.16 기각 1 의 예측이 반증됐다 (MN-5b)

§A11.16 기각 1 은 극관 억제 `(1 − iceMask)` 인자 삭제 결함의 픽셀이 *「G 에서 `R = landMask · (1 − iceMask) = 0` 이라 **`NI_sea` 로 분류**된다. D5 는 `NI_sea` 무변화를 요구하므로 그 자리에서 잡힌다 — 배제가 아니라 검출 경로다」* 로 예측했다. **실측은 이 예측과 두 겹으로 어긋난다.**

[실측 `mutations/mn5b.log`] 결함 판본이 `exit 0` (전 게이트 PASS). `NI` · `NI_land` · `NI_sea` 판정량이 baseline 과 소수 6자리까지 **동일**하고 (`NI 0.02187` · `NI_land 0.038502` · `NI_sea changed 0`) `disk` 만 다르다 (`changed 995 → 1045` · `fullChanged 1006 → 1061`).

변경 픽셀 집합을 `page.route` 셰이더 치환으로 재현해 전량 덤프했다 [실측 `diag/m3-mn5b-defect.log` — 등가는 `995 → 1045` · `1006 → 1061` 재현 + P2 결함판 ↔ 건강판 `0 px` 로 실증]. 집합 A (`P1d ≠ P2d` ∧ `P1b == P2b`) = **정확히 `50 px`**:

| 항목 | 값 |
| --- | --- |
| `ndv` 범위 | **`[0.0482, 0.3752]`** — 전량 `INNER_NDV_MIN 0.6` 미만 |
| `r/R` 범위 | **`[0.927, 0.9988]`** (disk 주변부) |
| `\|lat\|` 최소 | **`61.63°`** (북 `27` / 남 `23`) |
| `NI` 탈락 사유 | `ndv<0.6 ∧ ndl>−0.15` **38** · `ndv<0.6` 단독 **12** ⇒ **50/50 전부 `ndv < 0.6`** |
| 게이트 맵 `R` | `R == 0` **`41 px`** · `R > 0` **`9 px`** |
| **`NI` 안에 들어온 픽셀** | **`0`** |

**어긋난 두 겹**: (i) `41/50` 은 실제로 `R == 0` 이지만 **애초에 `NI` 대역 (`ndv ≥ 0.6`) 에 들어오지 않아** `NI_sea` 표본 자체에 포함되지 않는다 — `D5` 가 「잡고도 통과」한 것이 아니라 **표본 밖**이다. (ii) `9/50` 은 `R > 0` 이라 예측의 전제 (`R == 0`) 조차 성립하지 않는다.

**⚠️ 초판의 구조 논증은 전칭 주장이었고 반증됐다.** 초판은 *「가드는 카메라를 적도면에 고정하므로 `ndv ≤ cos φ` 이고, `ndv ≥ 0.6` 은 `|φ| ≤ 53.13°` 를 요구하는데 극관은 `|lat| ≥ 56.61°` 라 **어떤 JD 를 골라도** 교집합이 공집합」* 이라 적었다. cross-validate 가 [Critical] 로 지목하고 (obliquity `23.44°` 가 극을 카메라 쪽으로 숙이면 `cos(33.17°) ≈ 0.837 ≥ 0.6`), 후속 실측이 **결론은 확인하고 기전은 반증**했다.

**실측으로 확정한 축 — obliquity 가 아니라 카메라 고도 `beta`**

| 축 | 결과 [실측] |
| --- | --- |
| JD 5개 (춘분 `2451623.816` · 하지 `2451716.575` · 추분 `2451810.2278` · 동지 `2451900.0674` + 가드 `2451808.0`) | `NI ∩ land ∩ ice` **전부 `0`** · 극축↔카메라 각 **전부 `90.000°`** |
| 카메라 방위각 `α` 7개 (`+0°`~`+180°`) | **전부 `0`** · 극축↔카메라 각 전부 `90.000°` |
| **카메라 고도 `beta`** | `90°`(가드)→`0` · `80°`→`0` · **`70°`→`25`** · `60°`→`176` · `50°`→`417` · `40°`→`520` · `30°`→`833` |

**기전** [실측 `diag/m3c2-beta-ice.log`]: 가드 프레임 (`?rotate=off`) 은 `rotationQuaternion = null` 이라 **자전축 기울기가 `0.0000°`** 다 (자전축 world `[0, 1, 0]`). `rotate` ON 프레임에서는 `66.56°` 다. 카메라는 `upVector [0,1,0]` (코드에 `upVector` 대입이 **없다** — grep `0` hit, Babylon 기본값) 이고 가드가 `beta = π/2` 를 주므로 시선의 world-Y 성분이 `0` 이다. 가드 프레임의 자전축이 정확히 world `+Y` 이므로 **시선 ⊥ 자전축이 JD · `α` 와 무관하게 성립**한다 — 12 구성 전부 `90.000°` 인 이유다.

⇒ **cross-validate 의 기전은 이 가드에서 발화하지 않는다** (obliquity 가 `0` 이라 극이 카메라 쪽으로 숙지 않는다). 그러나 **초판의 전칭 주장도 거짓이다** — 여는 축이 JD 가 아니라 `beta` 이고, 임계는 `beta ≈ 75 ± 5°` 사이다. 정확한 서술은 **「`beta = π/2` 고정의 결과」** 이며, 살아나는 대안은 JD 변경이 아니라 **카메라 고도 변경**이다.

⚠️ `|lat| ≥ 56.61°` 는 게이트 맵 2장으로 `landMask` · `iceMask` 를 복원한 **대리 측정**이고 혼입이 있다 — disk 실루엣에서 MSAA 가 배경색 `(8,9,13)` 을 섞어 `iceMask` 를 가짜로 만든 `18 px` (`iceMask ∈ {0.004, 0.008, 0.012}` · `r/R ≥ 0.9959` · `|lat| < 50°`) 를 제외한 `278 px` 가 근거다. **정본은 결함 렌더 실측 `50 px`** 이고 대리 측정은 보조로만 쓴다.

⚠️ **부수 관측 — `?rotate=off` 가 자전축 기울기까지 제거한다** [실측]. **이것은 의도된 설계다** — `apps/web/src/core/parse-rotate-mode.ts` 주석이 *「`off`: 자전 정지 — `?rotate=off` 진입 시 **자전 도입 전 픽셀 100% 복귀** (snapshot 가드 격리)」* 로 계약을 명시하고, Amendment 2 (#782) 가 self-rotation 과 obliquity 를 **함께** 도입했으므로 둘 다 되돌리는 것이 정합이다. 남는 질문은 「`rotate=off` 로 판정량을 도출한 위도 의존 가드 12종이 기울기 `0°` 기하에 노출되는가」 이고, 별건 [#1230](https://github.com/coseo12/astro-simulator/issues/1230) 으로 분리했다 (사용자 결정 2026-09-18 · `deferred:no-incident` — 실피해 미관측, 해제 트리거는 **접촉**).

**처분 — 「단위 테스트가 잡는 자리」로 남긴다** (사용자 결정 2026-09-18). 가드 판정량은 **무변경**이다. 근거는 「구조적으로 불가」 가 아니라 **「가드가 고정한 카메라 고도에서 표본이 존재하지 않음」** 이다 (JD 는 무관하다 — 5/5 `0`).

- 단위 테스트가 실제로 잡는다 [실측 `mutations/unit-tests.txt` — `mn5b unit core: 1 failed | 1129 passed`]. 대상은 `procedural-planet-night-lights.test.ts` 의 극관 억제 단언이다.
- 기각한 대안: **`NI` 의 `ndv` 하한 인하** — 이 결함을 보려면 `0.0482` 까지 내려야 하고 (실루엣 포함) 그 대역은 위 MSAA 배경 혼입 구간이며 `verify:1202` 와 공유하는 대역 상수를 건드린다 (계약 「대역 상수 신설 `0`」 충돌). **극관 전용 판정 대역 신설** — `|lat| ≥ 61.63°` · `r/R ∈ [0.927, 0.999]` 에 새 상수 2개 + 실루엣 혼입 처리가 따라온다. **카메라 `beta` 기울이기** — 결정적 프레임이 바뀌어 전 판정량 재도출.
- ⇒ §A11.14 에 이 결함의 픽셀 검증 가능성이 **구조적으로 없음** 을 기록한다 (표본 부재가 아니라 **대역 정의와 카메라의 조합**이다 — §A11.16 기각 1 이 「판별 불가의 실제 원인은 표본 부재」 로 적은 것도 정정된다).

#### A11.18.2 MN-7 — 환경 요인, 해소

원 실행은 판정에 도달하지 못했다 (`mutations/mn7.log` 8행 전부 `Target page, context or browser has been closed`). 동일 조건 재실행 **3/3 전부 판정 도달** [실측 `diag/m2-mn7-run1~3.log`] — 전부 `exit 1` 이고 **`D7(2)` 가 `low` 정착 쌍 disk 변화 `12 px` 로 잡는다** (나머지 9 게이트 PASS · 판정량은 baseline 과 동일). 소요 `86.5~87.0 s` · max RSS `1.11~1.15 GB`.

원 크래시 stack 의 `runMain:725` 는 그 시점 파일에서 **6번째 브라우저 컨텍스트 생성** 지점이고, mn7 주입 지점보다 3단계 뒤다. [추론, 미실측] 컨텍스트 7개 · MSAA 4x · 1280×720 의 메모리 압력 — 당시 머신 상태는 소급 측정 불가하므로 이 연결만 추론이다. **코드 처방 대상이 아니다.** 재발 시 판별에 쓸 관측은 「peak RSS `≥ 1.1 GB` · 6번째 컨텍스트」다.

#### A11.18.3 측정 방법의 계약 이탈 (기록)

`pnpm --filter @astro-simulator/core build` 가 권한 분류기에 반복 차단돼 「소스 변경 + dist 재빌드」 경로를 쓸 수 없었다. 위 두 측정은 페이지 레벨 개입으로 대체했고 (MSAA = `getContext` 후킹 · MN-5b = JS 응답 본문 셰이더 치환) 개입 유효성과 판정량 등가를 매 건 실증했다. MN-5b 의 계약 문면 판본은 `mutations/mn5b.log` (소스 + 재빌드) 가 이미 갖고 있다. ⚠️ 남은 D12 (확정한 V2 로 MN-6 실발화) 는 소스 변이가 필요할 수 있어 빌드 권한이 선행 조건이다.

---

### A11.19 교차검증 반영 사항 (개정 1) — **반영 완료** (agy 2026-09-18, Accepted 전이)

- **호출**: `cross_validate.sh architecture` (outcome `applied`, exit `0`, plan-bypass 가드 사후 diff empty). 입력 = §A11.17.3 · §A11.17.5 · §A11.17.9 · §A11.18 **절 경계 전체** + `installOverlay`/`uninstallOverlay` **함수 경계** (`37,597 B`, 임의 행 범위 `0`), 필수 토큰 grep 6/6 통과. 병행 dev 체크아웃 보호를 위해 **유휴 worktree 를 cwd 로** 사본 실행했다 (L3 롤백 사거리 회피).
- agy 판정은 **조건부 보류** (Critical 1 · High 2) 였고, 지적 3건의 처분과 그에 따른 재측정 (`diag/m1i` · `m1j` · `m3b` · `m3c` · `m3c2`) 을 거쳐 전이한다. **처방이 바뀐 것이 이 라운드의 결과다** (§A11.17.9 초판 처방의 반증).

**합의 (반영)**

1. **조건 5 의 결함 진단** — 「트리거를 변화량으로 쓴 것이 축」 에 agy 가 「정확」 으로 동의.
2. **조건 1·2 는 델타 결함 없음** — 조건 1 은 릴리스 마일스톤 이벤트, 조건 2 는 절대 상태값. agy 독립 판정이고 수용한다.
3. **조건 3·4 에 같은 결함 잔존** — 수용해 §A11.17.5 에서 개정했다. 조건 4 는 §A11.18.1 이 **실제 발현 사례**다.
4. **`V2` 의 `== 0` 은 구조적 필연이 아니다** — `|round(A+B) − round(A) − round(B)| ≤ 1.5 LSB` 이므로 `2 LSB` 는 산술 상한이 아니라 실측 관측이다. 초판의 「합성 경로가 정렬돼 구조적으로 성립」 은 과장이었다. §A11.17.9 §미확인 에 명시했다.
5. **MN-5b 처분 타당 + 기각 대안 3종 근거 타당** — agy 도 같은 결론.
6. **기각 근거 타당** — (C) 상대오차 (「fail-open 이 완벽히 입증됐다」) · 삼각형 경계 제외 (#1197 M-i 위배) · `antialias:false` (제품·테스트 비대칭).
7. **MSAA 개입 실험의 근인 규명** — agy 평가 「탁월」. 기전은 이 라운드에서 유지된 유일한 초판 결론이다.

**이견 (근거와 함께 처분)**

1. agy **[Critical] obliquity `23.44°` 반례** — **기전 기각 · 결론 수용.** [실측] 가드 프레임 (`?rotate=off`) 은 `rotationQuaternion = null` 이라 **자전축 기울기가 `0.0000°`** 이고 (rotate ON 은 `66.56°`), 극축↔카메라 각이 JD 5개 × 방위각 7개 **12 구성 전부 `90.000°`** 다. 따라서 「obliquity 가 극을 카메라 쪽으로 숙인다」 는 이 가드에서 발화하지 않고 JD 를 바꿔도 열리지 않는다 (5/5 `0`). **그러나 초판의 전칭 주장도 거짓이다** — 여는 축은 카메라 고도이고 `beta ≤ 70°` 에서 `25~833 px` 로 열린다. ⇒ agy 는 **자리를 옳게 짚고 기전을 틀렸다.** 이 조합은 「처방·기전이 갈려도 자리는 유효하다」 의 사례로 §A11.18.1 에 박제했다.
2. agy **「CI 리스크 없음 논증은 본말전도」** — **기각.** agy 는 「본 ADR 의 목적이 향후 D12 를 CI 게이트로 안착시키는 것」 이라 추정했으나, D12 의 CI 배선 불가는 계획이 아니라 **구조적 제약** (두 번째 서버 + 소스 재빌드) 이고 `.github/workflows/shader-pixel-guard.yml:214` 주석과 계약 (「PR 시점 1회 의무」) 이 그렇게 선언한다. 단 같은 지적에 딸린 **로컬 렌더러 변동** 우려는 유효하고, 확정 술어가 그것까지 흡수한다 (기준선이 범위·표본 수에 불변 — `NI_land`→full frame 에서 `1` 고정).
3. agy **「색상 채널 왜곡이 `|Δlum|` 의 대표적 사각」** — **부분 기각.** 술어는 처음부터 `chMax` (채널별 최대 차이) 기반이라 휘도 보존 색 왜곡을 잡는다. `|Δlum|` 은 진단 인쇄값이다. 단 초판 문면이 `|Δlum| 0.005059` 를 앞세워 오해를 유발했으므로 격자에서 `chMax` 를 앞세우도록 정정했다.
4. agy **대안 A (`d1` 연동 동적 임계)** — **기각.** [실측] `NA` 고장이 `d1` 을 `172` / `511` / `507` 로 부풀리므로 허용치가 `173~512` 로 열려 전역 b (`444 px`) 까지 통과할 수 있다 [도출]. **허용치를 다른 측정량에 연동하면 그 측정량이 새 공격면이 된다** — agy 가 임계 `3` 을 「의사 형식주의」 로 비판한 것과 같은 논거가 자기 대안에 돌아온다.
5. agy **[High] 「`V2-b` 가 공유 결함 축을 덮는다는 판단은 빈틈」** — **수용하되 처방이 다르다.** agy 는 `V2-b` 의 범위 확장을 요구했으나, `NA` 를 **쓰지 않으면** 「`NA`·`O` 공통 결함」 축 자체가 소멸한다. 확정안은 `NA` 불도입이라 이 지적이 가리킨 결함이 존재할 자리가 없다.
6. agy **「`copy()` 의 객체 맵을 deep copy 하라」** — **보류 (기록).** 현 레시피는 `_floats` 스칼라만 덮어쓰므로 참조 공유가 발현하지 않고, **host 3 프레임 (`P2`·`NA`·`H`) full frame `0 px`** 실측이 그것을 실증한다. 벡터·색을 덮어쓰는 변경이 생기면 발현하므로 §A11.17.9 §미확인 에 남긴다.

**외부 모델 고유 발견**

1. **공간 마스킹 사각** (`V2-a`·`V2-b` 가 둘 다 `NI_land` 한정 → 터미네이터·해양·주간면·우주 배경 누출이 원천 사각) — **실증 후 수용.** 누출 A 주입에서 `NI_land` 판정량이 건강판과 완전히 같음을 실측하고, 판정 범위를 `disk` 로 넓혔다. **이 라운드 최대 수확이고, 발췌에 없던 축이다.**
2. **임계 비발명 대안 2종** — 대안 B (크기·연결성 필터) 가 확정안 `V2-①`·`V2-②` 의 직접 근거가 됐다. 「MSAA 오차는 고립 1픽셀로만 나타난다」 는 주장은 실측으로 확인됐다 (건강판 성분 히스토그램 `{1: 1}`).
3. `Object.assign` **얕은 복사 → 객체 참조 공유** (`_vectors3`·`_colors3`·`_textures`) — 위 이견 6 으로 처분.
4. **격자 누락 결함 4종** — 텍스처 어드레싱·UV 래핑 / 색상 채널 왜곡 (위 이견 3) / 프레임 동기화 지연 / LOD 서브메시 전환. 셋은 §A11.17.9 §미확인 에 기록했다.
5. **`V2` 측정은 P2 인데 변이 대상은 P3** → P3 의 렌더 큐·깊이 상호작용 결함이 P2 측정에 드러나지 않는다. §A11.17.9 §미확인 에 기록.
6. **단위 테스트 위임의 잔여 위험 3종** — (i) CPU 단위 테스트는 셰이더 컴파일러 최적화·`mediump` 정밀도 손실·유니폼 바인딩 누락 등 **GPU 실구동 결함을 원리적으로 못 잡는다** (ii) 「기하학적으로 까다로우면 단위 테스트로 위임」 이라는 **선례가 가드 검출력 보증 계약을 약화**시킬 수 있다 (iii) 극관 시각 결함이 가드 영구 사각으로 남는다. 수용해 여기 박제한다.
7. **극관 전용 보조 렌더 패스** (baseline 무영향 장기 구상) — 기록만. 실피해 미관측이라 이슈화하지 않는다 (CLAUDE.md §검증 강도 게이트).
8. `copy()` 의 `0` 강제가 3개뿐이라 주간 조명 인자 추가 시 `NA`·`O` 가 함께 누출 — `NA` 불도입으로 소멸.

**Claude 편향 셀프 체크**

1. **전칭 단정 (재발).** 「어떤 JD 를 골라도 검증 불가」 를 **한 프레임 `50 px` 실측**으로 유도했다. 반증됐고, 이 저장소가 반복 박제한 클래스 그대로다. 교훈은 「전칭 양화사를 쓰려면 양화 대상 축을 실제로 훑어야 한다」 이고, 이번에 훑은 것은 JD 5개 · 방위각 7개 · 고도 7개다 (그래서 축이 **고도**임이 드러났다).
2. **우려가 반증됐다.** 「L2 는 미세 변조라 `chMax` 가 `1~2` 에 머물러 `≥3` 술어가 놓칠 것」 — 실측 `≥3` 이 `60` / `216` / `352` 였다. 내 우려로 요청한 측정이 내 우려를 반증한 것이고, **요청 자체는 옳았다** (측정 없이 `≥3` 을 채택했다면 근거가 비어 있었다).
3. **임계 발명을 형식으로 가렸다.** `3` 을 「1/3 관례의 역방향 적용」 으로 정당화했는데, agy 지적대로 `1` 은 표준편차가 아니라 단일 knife-edge 아티팩트다. **형식을 빌리는 것은 근거가 아니다.** 확정안은 임계를 `0` 개로 만들어 이 축을 없앴다.
4. **낙관.** 확정한 `V2-①`·`V2-②` 로 **MN-6 을 실발화시킨 실행이 없다.** `D6` 의 변이 검출력은 이 개정 후에도 **미확인**이고, 그것이 이 개정의 목적이다 — 구현·실발화 전에 「해소됐다」 고 적지 않는다.
5. **결합 간과.** `disk` 범위의 **고장별** `V2-②` 성분 수를 전량 재지 않고 단조성 도출에 기댔다 (건강판·누출 A 만 직접 측정). §A11.17.9 §미확인 에 명시했다.

**전이**: 위 반영 (조건 3·4·5 술어 개정 · `V2` 확정안 교체 · §A11.18.1 전칭 철회와 축 정정 · 미확인 6항 추가) 후 **Provisional → Accepted (cross-validate agy 2026-09-18)**.

---

### A11.20 D12 소스 변이 독립 재현 (qa, 2026-09-18) — 계약 충족 + 측정 방법 교훈 3건

빌드 권한이 열려 **소스 변이 7종 (MN-1 · 2 · 3 · 4 · 5 · 5b · 8) 을 qa 가 독립 재현**했다. 그 전까지 이 7종은 dev 로그 의존이었고 qa 가 검증한 값이 아니었다. 원자료 `docs/reports/1226-night-lights/qa/d12-source/` (레시피 `git diff` 7종 포함).

**측정 환경 동일성이 먼저 실증됐다** — qa 의 clean baseline 이 커밋된 dev 값과 소수점까지 같다 (`D2 0.021870` · `D4 0.840400` · `D6 0.699314` · `D7(1) 0.021659`).

| 변이 | 계약 기대 | 실제 FAIL 게이트 | `exit` | dev 일치 |
| --- | --- | --- | :-: | :-: |
| MN-1 | D3 FAIL | **D3** `110 px` | `1` | ✅ |
| MN-2 | D2 FAIL | **D2 `0.000000`** + D3·D5·D6·D7(1)·D7(2)·D8×2 = **8종** | `1` | ✅ |
| MN-3 | D3 · D4 FAIL | **D3 `956`** · **D4 `0.001706`** | `1` | ✅ |
| MN-4 | D4 FAIL · D2 PASS | **D4 `0.001706`** (D2 `0.354185` PASS) | `1` | ✅ |
| MN-5 | D5 FAIL | **D5 `300 px`** | `1` | ✅ |
| MN-5b | **`exit 0`** (예상됨) | 없음 — 판정량이 baseline 과 전 항목 동일 | `0` | ✅ |
| MN-8 | 측정 불가 | 전제 `(5)` (`dod` 3쌍 / `d9` 2쌍) | `2` | ✅ |

- **MN-2 의 사각이 실측으로 닫혔다** — 가드 **8 게이트 FAIL** ↔ 단위 테스트 **`1130/1130` 통과**. 「테스트가 있다 ≠ 그 테스트가 작동한다」 의 반대 방향 실증이고, 신규 가드의 고유 검출력이다.
- **MN-5b** 는 단위 테스트가 `1 failed / 1129` 로 잡는다 (처분 그대로 — §A11.18.1).

#### 측정 방법 교훈 3건 (이 재현의 핵심 산출)

1. **`shasum packages/core/dist/index.js` 는 셰이더 변이에 판별력이 `0` 이다.** 그 파일은 **re-export 배럴**이라 셰이더 코드를 담지 않는다. [실측] **MN-1 변이를 빌드한 상태에서도 해시가 불변**이었고, 같은 시점 `dist/scene/procedural-planet-shader.js` 에는 `MN-1` 마커가 있었다. ⚠️ 메인이 이 해시를 「원복 검증의 정본」 으로 지시했고 **그 지시가 틀렸다** — qa 가 대체 기준 (`dist/scene/procedural-planet-shader.js` 해시, clean `f8f54a47…` + 변이 7 판본 전부 상이) 을 박제해 매 변이의 dist 전파를 실증했다. **dist stale 판정은 수정한 파일의 emit 산출물을 봐야 한다** (`dist/` 는 gitignored 라 `git status` 에도 안 보인다 — volt #70 클래스).
2. **소스 변이 상태의 단위 테스트 실패 «수» 는 레시피 형태에 의존한다.** MN-3 · MN-4 에서 dev 기록은 `1 failed` 인데 qa 는 `2` 였다. [실측 3 판본] 삭제형 `2` / 주석형 `2` (`CODE` 가 `//` 를 제거 — #995) / **`d1` 보존 + 덮어쓰기형 `1`** (= dev). 기전은 `N4 painted-on` 단언이 `valueNoise(…)` 호출의 **존재**를 재는 것이다. ⇒ **레시피 없이 이 수치를 인용하면 재현 불가**다. 픽셀 판정량은 형태에 불변이므로 D12 본체 근거로 적합하다. (본 ADR 이 인용하는 것은 MN-5b 의 `1 failed` 뿐이고 그것은 형태 무관하게 재현됐다.)
3. **MN-8 의 계약 문면 「D9 (a) FAIL + 측정 불가」 는 구현에서 양립하지 않는다.** 전제 5 (불빛 OFF 페이지의 불빛 uniform 비-`0`) 가 게이트보다 **먼저** 발화해 `exit 2` 로 나가므로 게이트 줄이 인쇄되지 않는다. dev 로그도 같다 ⇒ **계약 문면 ↔ 구현 표현의 차이이고 구현이 옳다** (#1215 X2 — 하네스 설정 실패는 전제, 제품 속성은 게이트). 계약 문면은 소급 수정하지 않고 여기 기록한다.

#### 원복 검증 4/4

`git diff -- packages apps` **`0` 줄** · `dist/scene/procedural-planet-shader.js` 가 clean 해시 `f8f54a47…` 로 복귀 · 기본 모드 `exit 0` 으로 게이트 10줄이 clean 기록과 **완전 동일** · `pnpm -r test` core `1130` / web `567` / shared `4` / physics-wasm `11` 전건 통과.
