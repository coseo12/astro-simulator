...
⚠️ **부수 관측 — `?rotate=off` 가 자전축 기울기까지 제거한다** [실측]. Amendment 2 (#782) · 5 (#1130) 는 axialTilt 를 `mesh.rotationQuaternion` 으로 적용한다고 적었으므로, `rotate=off` 를 쓰는 **다른 가드들** (`verify:1202` · `783` 극관 검증 등) 의 전제에 닿는지는 별건으로 분리했다 (사용자 결정 2026-09-18). 실피해는 미관측이다.
