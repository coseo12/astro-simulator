...
| (진단 인쇄 — 술어 아님) | `> 1 LSB` px 계수 · `chMaxHist` · 최대 `\|Δlum\|` · 평균 기여 상대오차 | `1` · — · `0.005059` · `0.0184 %` | 위배 시 판독용 |

- **새 임계 `0` 개.** 둘 다 `== 0` 이고, 두 상수 (`3` · `2`) 는 임계가 아니라 **MSAA 오차의 실측 상한** (`chMax ≤ 2`) 과 **최소 비고립 크기** (`2`) 다. §A11.17.8 이견 4 의 「수치 발명 금지」 를 위반하지 않는다.
