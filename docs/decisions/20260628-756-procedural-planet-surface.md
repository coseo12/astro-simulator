...
| 9   | **번들 — 절대 바이트 임계** (reviewer 차단 1 수용): ① 마스크 파일 `≤ 28,660 B` (정본 `27,295` + 5% 여유) ② `apps/web/.next/static` 총계 `≤ 10,970,000 B` ③ **JS 총계 증가 정확히 `0` B** ④ 마스크 IHDR `colorType == 0 && bitDepth == 8` | build 전후 `find … -exec stat` 합계 대조 (baseline `10,933,577` B @ `37c0a49`, 마스크 반영 예상 `10,960,872`). ⚠️ 초판 술어 _"증가가 마스크 크기와 일치"_ 는 **자기참조**라 어떤 크기든 통과했다 — 절대 임계로 교체 |
