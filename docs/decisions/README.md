...
| 2026-08-14 | [1027-pr-base-edit-guard](20260814-1027-pr-base-edit-guard.md) | **Provisional** | PR `base` 편집 우회 봉인 — `edited` 를 required 컨텍스트에 들이지 않고 **신규 비-required 컨텍스트 `pr-base-edit`** 로 분리해 [`20260807-971`](20260807-971-required-status-checks.md) **Phase 1 면제 근거의 `types:` 전수 다리를 문자 그대로 보존**. `types` 에 `edited` 추가안은 **기각이 아니라 유예** (재도입 트리거 4조건). 971 §10-5 **재검토 조건 14** 신설. 선행 ADR: [`20260812-970`](20260812-970-pr-base-rule-guard.md) §8-1 한계 0 / §9 후속 1 |
