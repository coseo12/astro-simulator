...
| 2026-08-14 | [1027-pr-base-edit-guard](20260814-1027-pr-base-edit-guard.md) | **Accepted** (cross-validate agy 2026-08-15) | PR `base` 편집 우회 봉인 — `edited` 를 required 컨텍스트에 들이지 않고 **신규 비-required 컨텍스트 `pr-base-edit`** 로 분리해 [`20260807-971`](20260807-971-required-status-checks.md) **Phase 1 면제 근거의 `types:` 전수 다리를 문자 그대로 보존**. `types` 에 `edited` 추가안은 **기각이 아니라 유예** (재도입 트리거 4조건). 971 §10-5 **재검토 조건 14** 신설. 선행 ADR: [`20260812-970`](20260812-970-pr-base-rule-guard.md) §8-1 한계 0 / §9 후속 1 |
...
| 2026-08-14 | [1040-changelog-tilde-recovery](20260814-1040-changelog-tilde-recovery.md) | **Accepted** (cross-validate agy 2026-08-31 — §교차검증 반영 사항 4축 통합 완료. `Provisional` 에서 전이) | **소급 편집 금지의 기계 판정 가능한 예외** — 릴리스 확정 CHANGELOG 구간의 GFM 취소선 손상(21 줄 / 44 발생) 1회 회수. 예외 경계는 사람의 _"의미는 안 바뀌니까"_ 선언이 아니라 **4항 술어**(손상 보유 라인 한정 · 백틱/물결 정규화 후 문자열 동일 · 잔존 0). 상설 가드는 **미채택**(신규 유입이 이미 0 → precision 0). 선행 ADR [`20260814-982`](20260814-982-changelog-tilde-guard.md) 의 위임 행을 닫는다 / [`20260808-983`](20260808-983-measurement-recording-convention.md) §Amendment 1 (_"금지 대상은 소급 **무흔** 편집"_) 이 해석 정본 |
...
| 2026-08-16 | [850-phase1-threshold-noop](20260816-850-phase1-threshold-noop.md) | Accepted | [#850](https://github.com/coseo12/astro-simulator/issues/850) Phase 1 계약 임계 재조정(P1-1·P1-6 지표 대체) + 「gitignored 설계 문서 ↔ 머지 이력 미대조」 구조 박제 (NO-OP — 구현은 PR #936 / v0.57.0) |
| 2026-08-16 | [1082-web-test-typecheck](20260816-1082-web-test-typecheck.md) | Accepted | `apps/web` 45 + `packages/physics-wasm` 1 테스트 파일의 타입 검사 강제 지점 — [`20260814-1060`](20260814-1060-packages-test-typecheck.md) 분리분. 배선은 `ci.yml` `detect-and-test` 의 `setup-and-build` composite **직후** 스텝 1 개 (`if:` 는 composite 과 동일 — docs-only PR 오탐 방지). 저장소에서 `next build` 가 도는 job 이 여기 하나뿐이라 선행 조건 2축 ([`20260814-960`](20260814-960-worktree-typecheck-recipe.md)) 보정이 **0 스텝**이다. `packages/physics-wasm` 은 `tsconfig` 신설 + `typecheck` echo no-op 교체 + **사문화된 `@ts-expect-error` 삭제** (양쪽 상태 모두 미발화 실측). 저장소 설정 무접촉 ([`20260807-971`](20260807-971-required-status-checks.md) 결정 1 관할) |
| 2026-08-16 | [1073-clause13-observation-wiring](20260816-1073-clause13-observation-wiring.md) | Accepted | [`20260807-971`](20260807-971-required-status-checks.md) **§10-5 항 13 관측 의무 배선** — 조건은 신설됐으나 «누가 매 사이클 돌리는가» 가 없던 3세대째 위임을 닫는다. **관측 = 기계**(이벤트 구동 비-required workflow `release-escape-watch.yml`) / **판단 = 사람**(T1 대장 · T2 재개봉). 사람 규약 기각의 결정적 근거는 부담이 아니라 **«미실행 ↔ `clean` 구별 불가»**(월 `26`회 빈도는 보강). [`20260814-1031-1064`](20260814-1031-1064-committed-claim-guard-rejected.md) §결정 6 (_"대조 주체는 사람"_) 과 **정합 — 개정 불요**(축 6개 대조표). 부수로 항 13 관측 시점이 모집단의 진부분집합이던 결함(prep PR `65`건 누락) 정정. 항 14 는 비용 계급이 달라 **후속 분리** |
| 2026-08-17 | [1103-conflict-marker-guard](20260817-1103-conflict-marker-guard.md) | **Accepted** (cross-validate agy 2026-08-17 — `Provisional` 에서 전이) | 머지 충돌 마커 **전수** fail-fast 가드 신설 — `lint-staged` 의 `prettier --write` 가 마커를 md 문법으로 정규화해(들여쓰기 / 백슬래시 escape / blockquote) 커밋 전 검사 형태와 커밋물 형태가 갈린 실사고 1건(`fa497b6`). [`20260814-982`](20260814-982-changelog-tilde-guard.md) 와 **뿌리는 같고 모집단·스코프가 다르다** — 마커는 prettier 미소유 파일에도 원형으로 남아 소유 경계가 성립하지 않고, 「고칠 수 없는 존량」 범주가 없어 diff 로 좁힐 이유도 없다 → **확장 기각 · 신설 채택**. 변형은 **마커 종류 × 앞줄 상태** 2축이라 선행 공백 허용이 `sep` 만의 문제가 아님을 실측 (이슈 본문 제안 술어 반증). 오탐 축 4종 전건 `0` (setext 는 등호 2연속조차 `0`) |
