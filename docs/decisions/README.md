...
| 2026-08-12 | [970-pr-base-rule-guard](20260812-970-pr-base-rule-guard.md) | Provisional | PR `base` 선택 규칙의 기계 SSoT — `base=main` 은 release PR(head=develop)/hotfix 전용. head 를 **shape(접두사 수준)** 로 분류해 브랜치명 가드와 **관할을 분리**하고, `base=<type>/*` 는 **봇 전용**으로 한정 (실측: 그 17건이 100% 봇). 선행 ADR: [`20260806-962`](20260806-962-branch-name-guard.md) §11-1 후속 F1 / [`20260807-971`](20260807-971-required-status-checks.md) (배선 job 이 required check 인 근거) |
