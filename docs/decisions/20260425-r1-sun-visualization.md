...
| × 75     | 213             | 3.87%             | 320              | ~~6.18%~~ 3.88%[^c1] | 197            | ~~19.6%~~ 12.19%[^c2] | 모바일 12.19% 인지 가능              |
...
- 모바일 12.19% 는 3% 임계 통과 + 인지 가능 + 화면 차단 안 함 (< 25%)
...
**보존 정책**: 본문 직접 정정 + strikethrough (`~~6.18%~~ 3.88%`) + footnote 참조 (`[^c1]`) 박제로 _역사적 박제값_ + _정정값_ 동시 추적. 원본 박제값 git history 보존.
...
- **회귀 가드** (line 307) — sunScale 75 시점 baseline. 현재 운영 baseline 은 §"신규 R1 baseline (Amendment 2026-05-01)" 박제값 (brightRatio ≥ 0.5% / pxDiameter ≥ 100px / diskAreaRatio ≥ 4% / 모바일 brightRatio ≥ 3%) 이 SSoT. 본 Amendment v2 는 _역사적 박제값_ 정정만 — 현재 R1 회귀 가드 인프라 변경 0
...
기존 §재검토 트리거 (#1~#6) 보존. 추가 트리거 없음 — 본 Amendment 는 _박제값 정정_ 으로 결정·구현·인프라 변경 0, 후속 재검토 조건 별도 박제 불필요.
...
## Amendment v3 (2026-05-20) — §결정 1 표 모바일 (375×667) 점유율 박제값 19.6% → 12.19% 정정 ([#427](https://github.com/coseo12/astro-simulator/issues/427))

### 배경

PR [#426](https://github.com/coseo12/astro-simulator/pull/426) reviewer 정적 리뷰에서 비차단 권고로 발견 — `sunScale × 75` 행의 모바일 (375×667) 점유율이 ADR 박제 **19.6%** vs reviewer 독립 Python 산출 **12.19%** 로 차이 약 7.4 percentage point. Amendment v2 (2026-05-06) 가 1920×1080 한정 정정으로 분리됐기에 모바일 분리 박제.

### 산출 식 재현 (#427 forensic)

ADR §결정 1 §점유율 산출식 (line 70-82, vertical fov 0.8 rad 기준):

```
pixel diameter ≈ diameter_scene × viewportHeight / (radius × 2 × tan(fov / 2))
              = 0.1169 × 75 × 667 / (35 × 2 × 0.4228)
              = 197.5 px
...
→ **reviewer 독립 산출 12.19% 가 정답**. ADR 박제 19.6% 는 R1 본 구현 시점 산출 오류 (분모 또는 axis 매핑 추정).

### 데스크톱 박제값 재검증 (회귀 가드)

- 1280×720: `π × (213/2)² / (1280 × 720) = 35631 / 921600 ≈ 3.87%` ✓ (박제값 일치)
- 1920×1080: `π × (320/2)² / (1920 × 1080) = 80425 / 2073600 ≈ 3.88%` ✓ (Amendment v2 정합)

→ 데스크톱 박제값 변경 없음. 모바일만 정정.

### 결정

- ADR §결정 1 표의 `sunScale × 75` 행 모바일 점유율 ~~19.6%~~ → **12.19%** 정정 (strikethrough 보존으로 원본 인용 추적). 평가 문구 "모바일 19.6% 침습 가능" → "모바일 12.19% 인지 가능" 갱신
- 후보 X (현재 R1 결정 — `BODY_SCALE.sun = 75 + 모바일 점유율 cap`) 박제 논리 동일 유지 — 12.19% 도 3% 임계 통과 + 자연스러움 (< 25%) 양립

### 운영 영향 0

본 정정 시점 (2026-05-20) 운영 sunScale = 50 (Amendment 2026-05-01). 본 ADR §결정 1 표는 R1 본 구현 시점 (sunScale 75) 의 _역사적 분석 컨텍스트_ — 동작 변경 없음. R2~R10 의 다른 박제값과 무관.

### 정정 영향 확인 (volt #14 인계 항목 실측 재검증)

- 코드 변경 0 (ADR 본문만 수정)
- 회귀 가드: 본 Amendment 가 데스크톱 박제값 (3.87% / 3.88%) 재현 산출도 박제 → 미래 재산출 시 SSoT 비교 가능
...
[^c2]: 2026-05-20 Amendment v3 박제로 19.6% → 12.19% 정정 확정. 본 ADR §"Amendment v3 (2026-05-20)" 참조. 원본 박제값 19.6% 는 R1 본 구현 시점 (sunScale 75) 산출 오류로, PR [#426](https://github.com/coseo12/astro-simulator/pull/426) reviewer 독립 Python 산출 (2026-05-06) 에서 발견. strikethrough 보존으로 원본 인용 추적 가능. 산출 재현: `π × (197.5/2)² / (375 × 667) = 30631 / 250125 ≈ 12.18%`.
