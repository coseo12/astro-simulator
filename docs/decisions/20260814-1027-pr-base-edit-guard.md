...
**전면 기각은 하지 않았다.** 후보 2 채택으로 _"어떤 가드도 막지 않는다"_ 는 상태는 해소되고, **탐지는 100% · 차단은 0%** 라는 정확한 상태로 이동한다. 이 상태를 흐리지 않기 위해 §8 산출물의 각 문서에 같은 문장을 박제한다:

> `pr-base-edit` 은 **required 가 아니다.** base 편집으로 규칙을 위반하면 붉은 X 가 뜨지만 **머지는 기계적으로 막히지 않는다.**

⚠️ **사본 개수를 여기 복제하지 않는다** ([20260814-1031-1064](20260814-1031-1064-committed-claim-guard-rejected.md) §결정 5 — 감시값 SSoT 는 실행 술어 쪽). 초판은 _"세 곳"_ 이라 적었으나 개정 주석·CHANGELOG 가 부수 사본을 늘려 **박제 당일 이미 어긋났다** (PR [#1091](https://github.com/coseo12/astro-simulator/pull/1091) reviewer 실측). 회수 시점의 실제 모집단은 아래 술어로 도출한다:

```bash
git grep -lF '머지는 기계적으로 막히지 않는다' | sort
git grep -lF 'pr-base-edit' | wc -l   # 양성 대조군 — 실재하는 문자열이므로 >= 1 이어야 한다
```

> ⚠️ **대조군은 «없어야 하는 문자열» 이 아니라 «있어야 하는 문자열» 로 잡는다.** 부재 문자열을 쓰면 그것을 **여기 적는 순간 저장소에 실재**하게 되어 대조군이 자기 자신을 매칭한다 — 본 블록 초안에서 실제로 발생했다 (기대 `0`, 실측 `1`). 저장소가 이미 겪은 «가드 주석 자기-매칭» 클래스([#995](https://github.com/coseo12/astro-simulator/issues/995))다.
...
- **재도입 시 선행 작업** (순서 구속):
  ...
  4. **§6 박제 문장의 사본 전수 회수** — required 승격 순간 *"머지는 기계적으로 막히지 않는다"* 는 **전 사본이 일제히 거짓**이 된다. 개수를 세지 말고 §6 의 술어(`git grep -lF` + 양성 대조군)로 그 시점 모집단을 도출해 전건 갱신한다. 이 단계를 빼면 승격이 **거짓 문장 N개를 저장소에 남기는 작업**이 된다 (PR [#1091](https://github.com/coseo12/astro-simulator/pull/1091) reviewer 권고 3).
  5. 그다음에야 `types` 1줄 + `pr-base-edit` workflow 철거 여부 판단.
