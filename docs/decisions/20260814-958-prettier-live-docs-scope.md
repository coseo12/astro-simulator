...
# ⚠ 디렉토리 재포함 줄이 없으면 하위 파일 재포함은 성립하지 않고, 재포함은 **한 단계씩만** 성립한다
#   — 그래서 경로마다 줄이 3개다 (자신 / 중첩 디렉토리 / 파일). 가운데 `**/` 줄이 빠지면 깊이 1 만 산다.
...
1. **소유 단위는 markdown 뿐이다.** `.json` / `.png` 등 비-markdown 산출물은 `docs/**` 로 제외 유지. 실측 — 편입 **40 파일 전건 `.md`**, JSON·PNG **0건** (중첩 번들의 PNG 42 · JSON 8 포함 전건 제외 유지)
...
않는다.** 디렉토리 재포함은 **한 단계씩만** 성립하므로 `!docs/reports/` 는 `docs/reports/` **자신만** 되돌리고,
...
| 중첩 JSON (`379-forensic/output.json` 등 **8**) | **`ignored: true` 유지** (H1 무손상) |
| 중첩 PNG (`379-forensic/*.png` 등 **42**) | **`ignored: true` 유지** (H1 무손상) |
| 인접 `docs/lessons/` 등 | `ignored: true` 유지 |
| `format:check` | exit `0` — **churn `0`** (편입 파일이 이미 prettier 정합) |
| 모집단 계수 | `44` → **`45`** / 편입 delta `39` → **`40`** |

**대안 (나)(깊이 1을 의도된 경계로 선언하고 `**/*.md` → `*.md` 로 표기를 낮춤)를 기각한 이유** — `docs/reports/<이슈>-<주제>/`
번들은 **확립된 반복 패턴**(현재 **8**개: `378`·`379`·`391`·`397`·`411`·`546`·`762`·`818`)이고, 그 안의
`README.md` 는 forensic ADR 이 본문에서 참조하는 **live 문서**다. (나)를 택하면 향후 모든 번들 README 가
**조용히** 포맷 밖에 남는다 — fail-closed 라 안전하긴 하나, 선언된 의도(_"4경로는 포맷 대상"_)와 어긋난 채
누적된다. (가)는 churn `0` 으로 동작을 선언에 맞추므로 비용 없이 의도를 회복한다.
