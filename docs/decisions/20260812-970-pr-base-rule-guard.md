...
> **부수 실측 — 동시에 열린 PR 들은 각자 다른 head SHA 여야 한다.** 세 브랜치(본 PR `feature/970-pr-base-rule-guard` + 일회용 2개)가 같은 커밋을 가리키면 한 SHA 가 3개 PR 의 head 가 되어 `branch-name` 체크런이 `{success, failure, success}` 로 공존한다 — [20260807-971](20260807-971-required-status-checks.md) §2-12 원인 ③ (PR 다중성 축) 이 `4f7366e` 에서 실측한 바로 그 형태이고, **required check 위에서 본 PR 자신의 판정이 오염된다**. 그래서 일회용 브랜치마다 검증용 빈 커밋을 1개씩 얹어 head SHA 를 분리했다 — 실측된 세 head SHA 는 `201ca1c` (#1023, 당시 feature 브랜치 head = 두 일회용 브랜치의 분기점) / `74b6a5c` (#1024) / `a246ece` (#1025) 다. 뒤이은 #1026 도 같은 처리로 `7c34c49` 를 얻었다.
...
4. `release/*` shape 를 접두사로 판정하므로 `release/v0.28.0` (`-prep` 없음) 도 base=develop 통과다. `-prep` 강제는 축 B 관할이며, [#962](https://github.com/coseo12/astro-simulator/issues/962) 후속 F3 (`v` 표기 통일) 과 함께 다룰 항목이다.
   > **갱신 (2026-08-14, [#972](https://github.com/coseo12/astro-simulator/issues/972))** — F3 는 착지했으나(축 B 정규식에서 `v?` 제거 — [20260806-962](20260806-962-branch-name-guard.md) §9) **본 항목은 의도적으로 묶지 않았다.** 본 가드가 판정하는 것은 base *선택* 이지 head 이름의 *적법성* 이 아니며, `release/v0.28.0` 픽스처의 존재 이유가 정확히 그 관할 분리의 실증이다. 따라서 이 조건은 **계속 열려 있다** — 다만 F3 를 트리거로 삼는 서술은 소진됐으므로, 재론 트리거는 *"base rule 단독으로 `-prep` 미부착을 막아야 할 실사례 관측"* 이다. 부수로 `CROSS_CHECKS` 의 `release/v0.53.0-prep` 이 축 B FAIL 이 되어 교집합 밖으로 나갔고, 그 대조가 F3 구현 중 **먼저 발화해** 갱신을 강제했다(설계 의도대로 — §5 참조).
...
**원인은 두 겹이고, 초판은 그중 하나만 적었다.** 메인은 `cross_validate.sh` 출력을 **`tail -25` 로만** 확인하고 4축 요약을 작성했다. 그 창(L113~137)에는 **제안 2·3 과 결론**이 담겨 있었다. 따라서 ① **제안 1(L99)은 읽지 않았고** ② **제안 2(L113)는 읽고도 누락했다** — *"유일한 실행 권고"* 라고 쓴 시점에 제안 2 는 화면에 있었다. ②가 ①보다 무겁다. ①은 도구 사용의 실수라 *"끝까지 읽는다"* 로 교정되지만, ②는 **본 것을 세지 않은 것**이라 같은 처방이 듣지 않는다. 이 절의 초판이 원인을 ①로만 적은 것 자체가 세 번째 사례다 — **정정문조차 덜 불리한 쪽으로 기울었고**, reviewer 2라운드(🟡B)가 다시 잡았다. *"승인 일색"* · *"기여 0"* 은 검증의 결론이 아니라 **잘림과 누락이 만들어낸 문장**이고, 하필 **자신의 PR 에 유리한 방향**으로 만들어졌다. 편향 셀프 체크를 표방한 문단이 **자기 검증의 부재를 자기 칭찬으로 포장**한 셈이라, 이 절은 스스로가 가장 강한 경계 대상이었다.

**제안 2·3 의 처분** — 제안 2(`verifyGuideTable` 파싱 예외 안내 강화)는 **기각**한다. agy 자신이 같은 문단에서 *"현재 구현으로도 충분히 타이트하게 잡고 있음"* 이라고 마감했고(L116), 요구가 주석 문구 보강에 그쳐 동작 변화가 0 이다. 제안 3 은 ADR 상태 전이 절차 확인이라 **이 §10 의 작성 자체가 이행**이다.
