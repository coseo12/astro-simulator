...
| [gh-cli-execsync-pitfall.md](gh-cli-execsync-pitfall.md) | 셸 경유 마크다운·코드 전달 (`gh` 계열 + `git commit -m` + `--title`) — 큰따옴표를 관통하는 것은 최소 백틱 · `$` · `\` · `"` 넷 (치환 축 + 소실 축) 이고 전부 exit 0 인 채 본문을 지운다. Node.js 는 spawnSync + stdin, 셸 타이핑은 `-F -` / `--body-file -` + 따옴표 친 heredoc 의무 | [#114](https://github.com/coseo12/volt/issues/114) / [#996](https://github.com/coseo12/astro-simulator/issues/996) / [#1045](https://github.com/coseo12/astro-simulator/issues/1045) |
