...
- **손상 축 (`\` · `"`)** — 치환은 안 일으키지만 **문자 자체가 소실**된다. `\` 는 `$` · `` ` `` ·
  ...
  된다. `"` 는 짝수 개면 조용히 사라지고(argv 병합), **홀수 개일 때만** `unexpected EOF` 로
  시끄럽게 죽는다.
...
> **`!` 는 결론은 안전이나 기전이 다르다.** 큰따옴표가 막는 것이 **아니다** — `!` 는 위 POSIX
> 특수문자 넷에 애초에 들어 있지 않고, 막는 것은 **비대화형 셸에 history expansion 이 꺼져
> 있다**는 사실이다. 실측: `bash -c 'shopt -o histexpand'` → **`off`** (그래서 `"wow!ls"` 원문
> 보존) / `bash -i` (stdin 파이프) → **`on`** 이고 같은 `"wow!ls"` 가 **큰따옴표 안에서도**
> `bash: !ls": event not found` 로 **실패**한다. 에이전트 경로는 전부 비대화형이라 실무 위험은
> `0` 이지만, _"큰따옴표가 막아준다"_ 로 적으면 대화형 셸에서 반증된다. 다행히 이 실패는
> **시끄러운 쪽**이라 조용한 손상 목록에는 들어가지 않는다.
