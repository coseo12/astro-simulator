...
`근거:` 줄은 **볼트·이슈 유래가 있는 블록에서 고정 요소**다 (§1.4). ⚠️ **전칭으로 읽지 말 것** — 실측(2026-08-08) 상 `## 실전 교훈` 36 블록 중 `- 근거:` 불릿 보유는 **3건**이고 **링크·번호를 아예 갖지 않는 블록이 다수**다 (볼트 유래 없는 일반 교훈은 원리적으로 충족 불가). 따라서 본 규약은 **신규·개정 블록**과 **유래가 실재하는 블록**에 적용되며, 기존 블록의 소급 일괄 부착을 요구하지 않는다. 전칭 선언은 *"선언 즉시 대다수가 위반"* 상태를 만들어 규약 자체의 신뢰를 깎는다 — 본 PR 이 근거 문장을 교체한 이유(*"약한 논거는 미래 다이어트에 반박당한다"*)와 같은 실패 형태다 (PR [#987](https://github.com/coseo12/astro-simulator/pull/987) 리뷰 🟡-1). CLAUDE.md `## 실전 교훈` 머리말의 포맷 컨벤션(내용 불릿 → `근거:` 불릿 → 선택적 `일반화된 설계 지식:`)과 동일하다.
