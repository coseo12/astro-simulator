...
> **실측 (2026-08-08, `CLAUDE.md` L95–203 = `## 실전 교훈` 범위, 20 블록)** — ⚠️ **술어를 함께 읽을 것**
>
> | 항목 | 술어 | 값 |
> |---|---|---|
> | 회수 키 보유 | 이슈 번호(`#N`) **또는** 링크(`](`·`http`) | **17 / 20 (85%)** |
> | 회수 키 전무 | 위 둘 다 없음 = **원리적 충족 불가** | **3 / 20 (15%)** |
> | (참고) 번호만 | `#N` 한정 | 14 / 20 |
> | L73 형식 충족 | `- 근거:` 또는 `- 상세:` 불릿 | **12 / 20** |
> | `- 상세:` 불릿 | | 10 |
> | `- 근거:` 불릿 | | 2 |
>
> 재현: `## 실전 교훈` 범위를 잘라 `### ` 블록별로 위 **술어 각각**을 계수 (단일 출력에서 일괄 도출 — #983).

**이 실측이 한정의 근거다** — 회수 키는 이미 **85% 준수**이므로 규범이 비현실적 요구가 아니다. 실제 격차는 **불릿 형식**(L73 충족 12/20)이고, 그래서 형식을 `근거:` 또는 `상세:` 둘 중 하나로 넓혔다. 원리적으로 충족 불가한 **15%**(볼트·이슈 유래가 아예 없는 일반 교훈) 때문에 **전칭 선언을 피한다** — 전칭은 *"선언 즉시 대다수가 위반"* 을 만들어 규약 신뢰를 깎고, 이는 §1.4 가 교체한 취약 논거(*"약한 논거는 미래 다이어트에 반박당한다"*)와 **같은 실패 형태**다.
