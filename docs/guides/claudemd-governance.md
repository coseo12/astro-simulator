...
> **실측 (2026-08-08, `CLAUDE.md` L95–203 = `## 실전 교훈` 범위, 20 블록)**
>
> | 항목 | 값 |
> |---|---|
> | 링크·번호 보유 | **14 / 20 (70%)** |
> | 링크·번호 전무 | 6 / 20 (30%) — 볼트 유래 없는 일반 교훈이라 **원리적 충족 불가** |
> | `- 상세:` 불릿 | 10 |
> | `- 근거:` 불릿 | 2 |
>
> 재현: `awk` 로 `## 실전 교훈` 범위를 잘라 `### ` 블록별 계수 (단일 출력에서 일괄 도출 — #983).
>
> **이 실측이 한정의 근거다** — 링크 보유는 이미 **70% 준수**이므로 규범이 비현실적 요구가 아니다. 실제 격차는 **불릿 형식**(`근거:` 2 vs `상세:` 10)이고, 그래서 형식을 둘 중 하나로 넓혔다. 충족 불가한 30% 를 위해 **전칭 선언을 피한다** — 전칭은 *"선언 즉시 대다수가 위반"* 을 만들어 규약 신뢰를 깎고, 이는 §1.4 가 교체한 취약 논거(*"약한 논거는 미래 다이어트에 반박당한다"*)와 **같은 실패 형태**다 (PR [#987](https://github.com/coseo12/astro-simulator/pull/987) 2차 리뷰 🔴 — 초판이 `36 블록 / 3건 / 전무 다수` 로 **세 수치 모두 부정확**했고 특히 *"다수"* 는 방향이 반대였다).
