...
5. **R-Phase 사이클 재사용 패턴 (v4 표준 후보)**: PM 권장안 일괄 → 이슈 → architect ADR (Provisional→agy→Accepted) → developer (Concrete Prediction 실측) → reviewer → baseline 선제 dispatch → qa (정량 측정) → D-T2 → 머지+수동 close ⚠️ **(2026-08-09 표기 — 이 절차는 회고 시점의 이력이다. `수동 close` 는 [#915](https://github.com/coseo12/astro-simulator/issues/915) `auto-close-issues.yml` 도입으로 폐지됐고 현행은 `gh issue view <N> --json state` 결과 확인이다. 현행 SSoT = [`docs/ops/operational-friction.md`](../ops/operational-friction.md) §1. 재사용 시 복붙 금지 — [#999](https://github.com/coseo12/astro-simulator/issues/999))** (auto-close 미발동 16회차+) → 독립 릴리스.
