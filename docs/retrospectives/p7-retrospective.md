...
4. **`time-play` silent-fail 대규모 잔존** — P7-B QA 에서 `browser-verify-*.mjs` 22개 스크립트가
   `.catch(() => {})` 로 셀렉터 부재를 삼키는 패턴 발견. 본 P7-E 에서 일괄 정비 (`pressTimePlay`
   유틸 분리 + 각 스크립트 pre-assert). P7-B PR 본체는 JD Δ 간접 증명으로 독립 통과시킨 덕에 범위
   축소 — 그러나 22개 분산 정비가 P7-E PR 변경 파일 수를 부풀림.
