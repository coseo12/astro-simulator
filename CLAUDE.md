...
- **qa 게이트 예외 규약** (#915 — 전례 PR #910/#911/#916/#917 추출): 메인은 아래 4조건 **전건 충족** 시에만 qa 디스패치를 생략할 수 있다. (a) **앱 runtime 표면 0** — docs·리포 자산 / CI·workflow 설정 / 인프라 스크립트 전용, `apps/**`·`packages/**` 소스 무접촉을 diff 로 실증 (b) **동적 검증 대체 실증** — 본 PR CI run 가드 실발화 / 실 스크립트 1회 실행 / reviewer 독립 재현 등 대체 증거 명시 의무 ("docs 라서 생략" 단독 사유 금지) (c) **근거 코멘트 박제** — `## qa 게이트 예외 판단` 제목으로 (a)(b) 근거를 PR 코멘트에 박제 (d) **메인 직접 전이** — `stage:qa → stage:done` 은 메인이 직접 수행. **예외 불가 (정식 qa 의무)**: runtime 의존/라우팅/렌더 경로 변경 (next 업그레이드 #906 / i18n 라우팅 #914 전례), 시각 효과 (실 Chrome GUI 수동 검증 별도 의무 — 아래 headless 교훈)
