...
- **메인, 복귀 직후**: `ps -axww -o pid=,etime=,command= | grep -E "cargo|next dev|physics_wasm-" | grep -v grep` + `pgrep -f "agent-browser-chrome[-]"` (bracket 유지 + **`-a` 금지** — #1054) → 세션 이전 것만 정리. `grep -v grep` 은 명령행 전체를 걸러 형제 명령까지 방어하나 **bracket 은 패턴 문자열 자신만** 보호한다.
...
3. **pgrep self-match 오탐** — `pgrep -f "패턴"` 이 자기 셸 명령행 매칭 → **bracket `[-]`** (`agent-browser-chrome[-]`). pkill 은 자기 셸 kill 위험이라 **안전 개선**. hook 은 `grep -v` 로 이미 안전. ⚠️ bracket 의 보호 범위는 **패턴 문자열 자신뿐** — `.*` 로 우회되거나 형제 명령의 un-bracketed 리터럴에 무력화된다. **macOS `pgrep -a` 금지** (조상 포함 → self-match 유발). 좀비 카나리아 정본은 `ps … | grep -E … | grep -v grep` (#1054, §3).
