 ...
 *
 *  D1 — **판별력 0** (`#1123`/`#1127` 클래스). 셰이더의 `mu` 를 재형해 disk 내부 주연감광만
 *       소멸시킨 변이(`mu ← 1 − (1−mu)^8`, rim 밖에만 잔존) 를 심으면 disk 내부 방사 프로파일이
 *       `240.64 → 230.32`(r 0~0.7) 로 **평탄**해지는데, 구 창의 `edgeCenterRatio` 는 halo 를 재서
 *       `0.2762`(임계 `0.85`) → **PASS 로 통과**했다 (구 창 실측, 같은 날). 즉 DoD 2 가 이름으로
 *       내건 성질이 사라져도 가드가 침묵한다. 신 창에서는 같은 변이가 `exit 1` 이다 (PR 본문 표).
