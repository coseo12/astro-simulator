 ...
 *       같은 이유로 M-A(`limb ← vec3(1.0)`) 의 신 DoD 3 `FAIL` 은 마진 `0.15%` 로
 *       **granulation clamp 잔차**(`base` 비 `0.6588` 대비 center `+0.59%` / edge `+0.74%`) 의
 *       차이지 색온도 검출이 아니다. **본 가드의 판별력 근거는 DoD 2 축에 있다.**
