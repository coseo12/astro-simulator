...
function judge(r) {
  ...
  // low 에 전제 3 을 걸지 않는 이유 [실측]: low 쌍 OFF 낮면 내부 평균 휘도 `0.03559` 는 씬 배경색 (clear color
  // 8비트 `(8, 9, 13)`) 의 Rec.709 휘도 `0.03559` 와 같다 — low 대역은 배경이라 휘도 전제가 성립하지 않아 표본 수만 건다.
  // C6 의 판정 근거는 `isVisible === false` 구조 읽기이고, 픽셀 항 (`changed == 0`) 은 「배경 위에 구름이
  // 그려짐」 (MC-5 `14211 px`) 만 잡는다. ⚠️ 한계: low 프레임에 지구가 실제로 그려졌는지는 보증하지 않는다 —
  // 전제 2 는 기하 계수이고 C1 은 high 프레임 값이다 (reviewer 주입: low 정착 뒤 지구 계열 mesh 를 렌더에서 빼도 exit 0).
  const pairPremises = [
    ['mid', r.mid, true],
    ['fade', r.fade, true],
