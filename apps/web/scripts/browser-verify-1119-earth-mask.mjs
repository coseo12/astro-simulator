...
async function runDod(browser) {
  ...
  // 집행되는 것은 `IOU_THRESHOLD` 하한과 단순 부호 비교 2건이다. 「유의」를 재려면 마진 상수가
  // 필요한데 — **여기서는 불필요하다.** 다만 두 비교의 성격이 서로 다르다.
  //
  // (1) `on.iou > forcedOff.iou` 는 **최종 게이트에 함의된다.** 이 블록이 `on.iou >= IOU_THRESHOLD`
  //     를 요구하고 DoD 2 가 `forcedOff.iou < IOU_THRESHOLD` 를 요구하며, 최종 게이트가
  //     `if (!(dod1 && dod2))` 다. 즉 exit code 를 바꾸지 않는다.
  //     ⚠️ 다만 **인쇄되는 DoD 1 판정**은 바꿀 수 있다 (게이트는 두 DoD 의 곱이지만 로그는
  //     `dod1` 을 따로 찍는다 — reviewer 프로브 S2).
  // (2) ⚠️ `on.iou > surfaceOff.iou` 는 **게이트를 바꾼다 — 함의되지 않는다.**
  //     `surfaceOff.iou` 에는 어떤 임계도 걸려 있지 않고(등장 지점 전수 = 이 비교 + 로그 2곳),
  //     이 비교는 **`?surface=off` 가 실제로 표면 셰이더를 끄는지**를 검사하는 유일한 가드다.
  //     그 경로가 no-op 으로 회귀하면 `surfaceOff.iou == on.iou` 가 되어 비교가 거짓이 되고
  //     DoD 1 이 실패한다 (reviewer 프로브 S1: 원본 `exit 1` / 이 비교 제거 시 `exit 0`).
  //     ⚠️ **#1163 초판은 이 비교도 「구조적으로 무해」로 판정했고 그것이 반증됐다** — 근거로
  ...
  //
  // ⇒ **그럼에도 마진은 불필요하다.** (2) 가 잡는 것은 **동률**(차이 0)이지 부분 열화가 아니라
  //   마진이 그 축을 강화하지 않고, (1)(3) 은 임계가 하중을 진다. 마진을 넣으면 발화하지 않는
  //   상수가 된다. §가드 설계 원칙 — 검사 표면을 늘리지 않는다.
  //   ⚠️ 이 판단은 **`IOU_THRESHOLD` 유지에 조건부**다 (헤더 `:12-13` 이 하향을 금지한다).
  //   임계가 낮아지면 마진이 발화하는 대역이 생긴다 (reviewer 프로브 S3).
  //
  // ⚠️ **`#1159` 를 여기에 기계 복제하지 말 것.** 그쪽(773 `contrastMean` / 774 DoD 3)은 판정량에
  //   임계가 **없어서** 부호만 남았고 신호 `98.8%` 소멸이 통과하는 것이 실측됐다. 여기는 (1)(3)
  //   에서 임계가 하중을 지고 (2) 는 동률만 겨눈다 — 형태는 같고 결과가 다르다 (#1163).
  //
