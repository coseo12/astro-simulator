 ...
 * 종속 비율량이라 정규화가 필요하다"* 였다. 여기엔 그 근거가 없다 — `iou` 는 이미 `[0,1]`
 * 무차원이고, **같은 판정량에 절대 임계 `IOU_THRESHOLD` 가 이미 걸려 있으며**, 같은
 * 플래그(`?surface=off`)·같은 body(earth)를 재는 자매 가드 `browser-verify-756-surface.mjs` 의
 * `HF_ENTROPY_MARGIN` 도 **차** 마진이다. 분모를 `on.iou` 로 다시 잡으면 아래 무오발 상한이
 * 변동 분모에 종속된다.
 *
 ...
 *    (갭 `0.1243`, `3`회 sd `0`). 마진이 이보다 커야 그 유출이 발화한다.
 *  - **상한 (무오발) `< 0.2938`** — **구조 상한**이다. `?surface=off` 는 단색 disk 라 화면
 *    분류(`b < g`)가 균일해 `screenLand ∈ {0, domain}` ⇒ `surfaceOff.iou ∈ {0, maskLandPct}`
 *    두 값만 가능하다. 현행 earth `colorHint #3B7AB5` (B `181` > G `122`) 는 전자라 실측이
 *    `0.0000` 이지만 `colorHint` 는 **데이터**라 후자(`0.5062`)로 바뀔 수 있다. 그리고 이 마진이
 *    게이트를 가르는 것은 `on.iou >= IOU_THRESHOLD` 가 참일 때뿐이라 최악은 `on.iou = 0.8`
 *    ⇒ `M < 0.8 − 0.5062 = 0.2938`.
...
async function runDod(browser) {
  ...
  // (1) 축은 여전히 부호 비교이고, 그것은 아래 근거에 따른 **의도된 선택**이다.
  //
