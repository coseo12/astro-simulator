...
async function setupPage(browser, query, { equatorialView = true, julianDate = T_JD } = {}) {
  ...
  if (equatorialView) {
    ...
    // #1205 — 정지 중 카메라 조작이 이제 LOD 를 건드린다. [실측] 이 `beta` 대입 하나로
    // 분포가 `3/2/27 → 9/6/17` (11 body 전이) 로 바뀌고 cross-fade 가 `12ms ~ 211ms` 동안
    // 진행한다 (#1205 이전 같은 시퀀스: 분포 `3/3/26` 불변, 전이 0 건 — 양쪽 실측).
    ...
    if (lodSettle.timedOut) {
      console.warn(
        `[1119] LOD 정착 상한 초과 (${lodSettle.waitedMs}ms, dist=${lodSettle.dist} fading=${lodSettle.fading}) — 마지막 상태로 진행`,
      );
    }
  }
