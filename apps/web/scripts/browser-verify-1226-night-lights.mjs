...
async function measureV2(ctx, { a, ref, base, land }) {
  return ctx.page.evaluate(
    async ({ a, ref, base, land, P }) => {
      ...
      // 표본 하한 — 판정 집합이 비면 두 술어 (둘 다 `== 0` 불변성) 가 **증거 0 으로 통과**한다
      // (#1201 §감사 시그니처 2 — 기저 신호 없는 불변성 검사). 임계가 아니라 **공집합 판정**이므로
      // §A11.17.9 「새 임계 0 개」 계약을 깨지 않는다. 다른 `measureV2` 실패 (mesh/scene 부재 ·
      // 캔버스 개수 ≠ 1 · 기하 무효) 와 **같은 `(6) 측정 오류` 경로**로 내보낸다.
      if (n === 0) return { error: '판정 집합 disk 표본 0 px — 두 불변성 술어의 기저 신호 부재' };
