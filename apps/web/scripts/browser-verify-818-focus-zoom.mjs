 ...
 * | # | body | 경로 | 기대 |
 * |---|---|---|---|
 * | S1 | jupiter | inner 정착 → 줌인 crossing → body | D1~D4 PASS |
 * | S2 | saturn (ring 동반 대형) | 동일 | D1~D4 PASS |
 * | S3 | earth (무회귀) | 동일 버그 잠복 → (c) 자동 수정 | D1~D4 PASS |
 *
...
async function runScenario(browser, { name, bodyId }) {
  console.log(`\n[${name}] ${bodyId} focus → 휠 줌인 (tier 진동/실거리 단조 판정)`);
  ...
  try {
    ...
    console.log(
      `  D1 역진동=${a.backwardTransitions} (=0?${a.d1 ? 'PASS' : 'FAIL'}) | ` +
...
async function main() {
  ...
  let allPass = true;
  try {
    result.scenarios.s1 = await runScenario(browser, { name: 'S1', bodyId: 'jupiter' });
    result.scenarios.s2 = await runScenario(browser, { name: 'S2', bodyId: 'saturn' });
