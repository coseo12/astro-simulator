...
**v0.79.1** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.79.0 이 **운영 규약의 도달 범위**였다면 v0.79.1 은 **가드가 실제로 무엇을 재고 있었나**다. 두 건 모두 임계를 한 글자도 바꾸지 않았고, 바뀐 것은 **재는 자리**다 — 분석 창이 회전하는 mesh 의 외접 상자 투영이라 자전 위상마다 커졌다 작아졌고(#1146), _"우주 배경 배제"_ 를 선언한 휘도 마스크는 배경(휘도 `9.2`)이 임계 `8` **위**라 한 번도 배경을 거르지 않았다. `verify:774-sun` 은 그 결과 **주연감광 판정이 disk 밖 글로우를 재고 있었고**, 주연감광이 사라져도 통과했다 — 셰이더 변이로 실증했다(#1155). (2026-08-26)

> **초록불과 지키는 것은 다르다** — 세 가드 모두 통과하고 있었다. `#1146` 은 3일에 4회 빨간불로 존재를 드러냈지만, `#1155` 의 `774` 는 **드러내지 않았다**. 가드가 조용하다는 것은 지키고 있다는 뜻이 아니라 **아무것도 묻지 않고 있다**는 뜻일 수 있다.
