...
**v0.89.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). 지구 **밤면 육지에 도시 불빛**이 켜졌다(#1226) — 별도 mesh 없이 표면 셰이더 안에서 합성해 `?nightlights=off` 가 픽셀 단위 no-op 이고, 불빛이 어디 몰릴지는 이미 있던 대륙 노이즈를 **임계로** 재사용해 정한다(추가 hash `0`). 그런데 이 릴리스의 이야기는 불빛이 아니라 **처방이 두 번 폐기된 것**이다. 신규 가드의 변이 하나가 4회 연속 「측정 불가」로 막혔고, 원인은 결함이 아니라 **MSAA 4x resolve** 가 삼각형 경계 한 픽셀에서 술어의 전제를 깬 것이었다 — `antialias:false` 개입으로 기전을 확정했다. 첫 처방은 임계 `3` 을 「기존 관례의 역방향 적용」으로 정당화했는데 외부 교차검증이 **「의사 형식주의」**로 기각했다(`1` 은 표준편차가 아니라 단일 knife-edge 아티팩트다) — **형식을 빌리는 것은 근거가 아니다.** 두 번째 폐기는 더 컸다: 그 술어가 `NI_land` 안만 보고 있어, 낮면에만 불빛을 새게 하는 결함에서 **판정량이 건강판과 완전히 같았다**. 확정안은 오히려 단순해졌다 — 기준 프레임을 하나 지우자(`NA` 불도입) 「전제의 전제」·「기준과 대상이 같은 결함을 공유하는 사각」·「진단을 술어로 올릴지」가 **함께 소멸**했고 새 임계는 `0` 개다. 그 교체로 **구름이 불빛을 가리는지 재는 게이트의 변이 검출력이 처음 확인**됐다. 외부 검증의 Critical 하나는 **자리는 옳고 기전은 틀렸다** — 자전축 기울기로 반례를 구성했는데 가드 프레임은 기울기가 `0°` 라 그 경로가 발화조차 못 했고, 그럼에도 내 「구조적으로 불가」라는 전칭 주장도 거짓이었다(여는 축은 **카메라 고도**였다). 양쪽이 틀리고 실측이 제3의 답을 냈다. (2026-09-18)

**v0.88.1** — 지구 표면 위에 **표면과 다른 속도로 도는 구름 레이어**가 생겼다(#1215). v0.87.0 이 **점 패치 셋이 같은 결함을 가리키고 있었을 때**였다면 v0.88.0 은 **클래스를 알고 들어갔는데도 그 클래스를 재생산했을 때**다 — 계약이 fail-open 시그니처(#1214)를 지목했고 설계가 양성 대조군을 박았는데도, 신규 가드 첫 판본은 **지구를 화면에서 지워도 전건 통과**했다(변이 R2 `exit 0`). 외부 교차검증과 reviewer 변이가 **각각 독립으로** 같은 경로를 잡아 머지 전에 닫혔다. 설계 쪽에서는 LOD cross-fade 창에서 구름이 사라지는 결함을 **정렬 키 치환**으로 없앴다 — 구름과 지구 계열이 bounding 중심이 같아 거리가 완전 동률이고, 순서를 삽입 순서가 정하고 있었다. 후속 패치 v0.88.1 은 **가드가 재던 것이 천체가 아니었다** — `verify:675-glow-marker` 가 캔버스 위에 겹친 HUD 글자를 luminance cluster 로 세고 있었고(`?marker=off` 40 AU `131~134` 중 천체는 `3`), 그 글리프가 로드마다 갈려 릴리스 CI 를 두 번 막았다(#1219). 임계는 한 글자도 바꾸지 않고 **측정 경로**만 결정화했다. (2026-09-13)

> **전제와 게이트를 섞으면 결함이 전제 뒤로 숨는다** — 「fade 재현이 성립했는가」를 묻는 유효성 전제에 「구름이 투명 큐에 있는가」(=게이트가 판정할 제품 속성)를 함께 넣자, 구름 블렌딩을 끄는 변이가 **FAIL 이 아니라 「측정 불가」로 빠져나갔다**. 전제는 **하네스가 통제하는 설정**만 묻고, 제품 속성은 게이트가 묻는다. ⚠️ 그 혼입을 지시한 것은 메인이고, 실행해서 분류가 바뀐 것을 보고한 것은 dev 다.
