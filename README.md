...
- **포함** — 이슈 비종속 상시 검증. 뷰포트/플랫폼 매트릭스 · 스케일 전환 · FPS · a11y · WebGPU ·
  워크스페이스 테스트 설정.
- **제외** — `verify:<이슈번호>-*` 꼴의 이슈별 회귀 가드, CI 가 자체 배선한 정적 가드. 필요하면 개별
  실행한다. 등록된 `verify:*` 전건의 정본은 루트와 `apps/web` 의 `package.json` 이다.
- ⚠️ **CI 초록 ≠ `verify:smoke` 실행됨.** 체인 원소 중 어느 워크플로든 실제로 부르는 것은
  `verify:test-coverage` **뿐이다** (rev `780cb92` 실측). 나머지는 로컬에서만 돈다.
