...
**v0.88.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). 지구 표면 위에 **표면과 다른 속도로 도는 구름 레이어**가 생겼다(#1215). v0.87.0 이 **점 패치 셋이 같은 결함을 가리키고 있었을 때**였다면 v0.88.0 은 **클래스를 알고 들어갔는데도 그 클래스를 재생산했을 때**다 — 계약이 fail-open 시그니처(#1214)를 지목했고 설계가 양성 대조군을 박았는데도, 신규 가드 첫 판본은 **지구를 화면에서 지워도 전건 통과**했다(변이 R2 `exit 0`). 외부 교차검증과 reviewer 변이가 **각각 독립으로** 같은 경로를 잡아 머지 전에 닫혔다. 설계 쪽에서는 LOD cross-fade 창에서 구름이 사라지는 결함을 **정렬 키 치환**으로 없앴다 — 구름과 지구 계열이 bounding 중심이 같아 거리가 완전 동률이고, 순서를 삽입 순서가 정하고 있었다. (2026-09-12)
