...
**v0.85.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.84.0 이 **문서가 자기 예시를 실행할 때**였다면 v0.85.0 은 **정본이 자기 규약을 어길 때**다. ADR 이 「분리 호출 + `--fail-if-no-match`」를 **pnpm 기반 CI 가드 전반의 규약**으로 박제했는데, 정작 같은 파일의 빌드 스텝이 그 규약이 금지한 형태를 그대로 쓰고 있었다(#1094). `-r` 이 붙어 있어 다른 축일 가능성이 남아 별도 실측했고, **`-r` 은 침묵을 바꾸지 않았다**. 부수로 ADR 축 2 의 「필터 `0` 매칭」을 **전체 `0` 매칭 / 부분 미매칭**으로 분해했다 — 플래그가 무력한 것은 후자뿐이고, 그래서 **「분리」는 스타일이 아니라 검출력의 필요조건**이다. (2026-09-05)

> **가드를 안 만드는 것도 결정이다** — 교정 후 보호 대상이 `0` 이 되고, 술어는 **구문 형태**(`--filter` 두 번)를 지목하는데 계약은 **침묵 의미론**에 관한 것이라 커버율이 `1/16` 이며, `-F` 축약형은 아예 미탐이었다. 「검토 후 기각」을 근거 수치와 함께 박제했다 — 침묵 기각은 다음 사람이 재검토를 못 한다.
