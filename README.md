...
**v0.78.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.77.0 이 **제품 표면**이었다면(전 행성 자전축을 궤도 법선 기준으로 정정 — obliquity 가 IAU 값과 **9/9 소수 3자리 일치**, #1130) v0.78.0 은 **검증 인프라**다. 세 건이 우연히 같은 클래스였다 — **가드가 묻는 질문이 계약이 요구하는 질문보다 좁다**: 들여쓴 코드펜스 안의 예시가 진짜 링크로 읽히던 것(#1134) / 릴리스 escape 조건은 박제됐는데 **그것을 돌리는 주체가 없던 것**(#1096) / LOD 가드가 **4개월간 한 번도 자동 발화하지 않았고** 그 임계는 **화면이 통째로 검게 죽어도** 못 잡던 것(#1127). (2026-08-23)

> **가드가 초록인 것과 가드가 무언가를 지키는 것은 다르다** — v0.78.0 의 세 건은 전부 「이 문자열이 있는가」를 묻고 「그것이 계약을 만족하는가」는 묻지 않던 술어였다. 셋 다 **판별력을 변이 주입으로 실증한 뒤에야** 배선했다.
