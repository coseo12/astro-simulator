...
**v0.77.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.76.0 에서 지구에 **실제 대륙**이 생기자(#1119) 곧바로 그 다음 것이 드러났다 — **전 행성의 자전축이 90° 누워 있었다**(#1130). 자전축 기준면을 궤도면 «안» 이 아니라 **궤도 법선**으로 잡도록 정정해 obliquity 가 IAU 값과 **9/9 소수 3자리 일치**한다 (지구 `23.440°` / 천왕성 `97.770°` / 금성 `177.360°`). 「옆으로 누운 행성」 천왕성이 오히려 똑바로 서 있던 것이 진단을 확증했다. 함께 들어간 인프라는 **에이전트 파일 헤딩 글루 가드**(#1125 — 헤더가 앞 불릿 꼬리에 붙어 헤딩으로 파싱되지 않는데도 CI 가 초록이던 축) (2026-08-22)

> **시각 검증의 판별력은 표면의 구체성에 의존한다** — 표면이 절차적 fbm 이던 동안에는 자세가 90° 틀려도 육안으로 드러나지 않았고, 실제 대륙이 생기자 사용자가 즉시 발견했다.
