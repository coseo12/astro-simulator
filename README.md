...
**v0.83.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.82.0 이 **재는 쪽이 쓰던 눈금이 어긋나 있을 때**였다면 v0.83.0 은 **고치는 절차가 스스로를 되돌릴 때**다. reviewer 차단으로 `stage:dev` 가 된 PR 에 수정 커밋을 밀면 `stage:review` 가 **덧붙어** 라벨이 둘이 됐고, 같은 스크립트가 연결 이슈의 단계까지 뒤로 돌렸다 — 즉 **고치라는 지시를 따를수록 상태가 어긋났다**(#1184). 직전 사이클에서 `3` 회 발화해 매번 수동 정리가 필요했던 실피해다. 채택안은 스킵 목록 확장이 아니라 **부착 시 단일화**다 — 전자는 다중 부착을 없애는 대신 「차단 → 푸시 → 재리뷰 대기」 자동 복귀를 죽이는데, 그 전이는 가정된 편의가 아니라 **관측된 사용 경로**였다. (2026-09-02)

> **고치려고 만든 자동화가 스스로 밟은 함정** — 이 사이클의 PR 본문이 close 키워드를 **예시로** 적었는데, 파서는 마크다운을 해석하지 않아 코드 스팬 안에서도 매칭돼 **무관한 이슈**에 라벨을 붙였다. 종전 파서도 같은 정규식이었으므로 신규 회귀가 아니라 **드러난 선재 함정**이다. 자기 자신을 예시로 쓰는 문서는 그 예시가 실행된다.
