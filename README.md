...
**v0.87.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). 지구 가장자리에 **대기 산란 rim** 이 생겼고(#1202), **일시정지 중에도 LOD 가 카메라를 따라온다**(#1205). v0.86.0 이 **처방이 자기 규범을 어길 때**였다면 v0.87.0 은 **점 패치 셋이 같은 결함을 가리키고 있었을 때**다. ring-anchor stale(#782/#785) · 광원 영벡터(#1204) · LOD 동결(#1205) 은 **카메라 종속 판정이 시간 위상에 얹혀 있던** 같은 생성원의 세 증상이고, 앞의 둘은 `setTier` 에 수식을 복제해 닫았다. 네 번째를 만드는 대신 **프레임 위상을 갈랐다**. ⚠️ #1202 는 그 사슬의 일부가 아니라 **발견 맥락**이다 — 대기 rim 을 검증하다 #1204 가 나왔고 그것을 고치다 #1205 의 실체가 드러났을 뿐, 셰이더 기능 추가와 위상 결함은 생성원이 다르다. (2026-09-08)

> **깨진 술어를 지적하는 능력과 대체 술어를 세우는 능력은 다르다** — 외부 검토가 원안 가드의 결함을 정확히 짚고 대안까지 냈는데, **그 대안 자체가 결함 보유판에서 통과**했다 (두 조건이 같은 퇴화 상수로 수렴한다). 「고쳐 줬다」에서 멈췄으면 초록인 채 머지됐다. 전자는 외부 검토가 강하고, 후자는 **결함 보유판에 대고 돌려 보는 것**만이 판정한다.
