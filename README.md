...
**v0.82.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.81.0 이 **검증되지 않은 채로 남아 있던 것**이었다면 v0.82.0 은 **재는 쪽이 쓰던 눈금이 어긋나 있을 때**다. 두 건 모두 판정의 기준으로 쓰이던 문장이 스스로와 맞지 않았다 — ADR `20260814-1040` 의 재검토 조건 1 은 **표제에서 「원용」, 본문에서 「제안」** 을 써 왔고 두 기준은 서로 다른 행동을 지시하는데, PR #1137 이 그 차이가 실제로 갈리는 첫 사례를 만들었을 때 **절차는 온전히 밟혔는데 ADR 에는 아무 기록이 없었다**(#1138). `stage:done` 은 라벨 설명이 `완료 — 사용자 머지 대기`, 즉 머지 **전** 상태인데 「머지 시 `stage:done` 으로 전이」하는 자동화를 만들려 했다 — 목표 상태가 이미 반대 뜻이었다(#1178). 후자는 정의를 바꾸는 대신 「파이프라인 밖으로 나간 PR 은 단계 라벨을 갖지 않는다」로 풀어, 실제 개정 대상이 `1` 줄이 됐다. (2026-09-01)

> **기준을 고치면 그 기준이 낸 판정도 함께 뒤집힌다** — `#1178` 에서 개정한 `1` 줄은 「머지 PR `67` 건이 종단 라벨 없이 정지」라는 판정을 낸 바로 그 줄이었다. 고치고 나니 미처리로 남긴 `62` 건은 정정 대상이 아니라 **이미 올바른 상태**였다. 드리프트를 고치려다 드리프트의 정의가 틀렸음이 드러난 셈이다.
