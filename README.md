...
**v0.81.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.80.0 이 **고치려는 쪽이 원인일 때**였다면 v0.81.0 은 **검증되지 않은 채로 남아 있던 것**이다. 두 건 모두 「돌고 있다고 믿었으나 실제로는 아무도 재지 않던 자리」를 닫는다 — `next build` 는 타입 검사를 하지만 `*.test.*` 는 **파일명 축**으로 제외하므로 `apps/web` 45 + `physics-wasm` 1 파일이 CI 타입 검사 밖에 있었고(#1082), 공전 궤도면 경사는 #1130 에서 「8/8 정합」으로 확인됐으나 **그 측정 경로가 리포에 없어** 다음에 깨지면 잡을 수단이 없었다(#1132). 후자의 가드는 회전 사본 3개 × 궤도 보유 31 body 를 195 케이스로 고정하며, 경사각만 재면 `z` 부호 반전과 회전 순서 교환을 `0/31` 로 놓치기 때문에 부호 포함 닫힌형 불변식을 함께 건다. (2026-08-30)
