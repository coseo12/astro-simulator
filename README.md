...
**v0.84.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.83.0 이 **고치는 절차가 스스로를 되돌릴 때**였다면 v0.84.0 은 **문서가 자기 예시를 실행할 때**다. PR 본문의 close 지시를 읽는 파서가 마크다운을 해석하지 않아, 코드 스팬 안에 **예시로** 적은 번호도 실제 연결 이슈로 잡혔다 — 직전 사이클의 도입 PR 자신이 그 함정을 밟아 무관한 이슈에 라벨을 붙였다(#1190). 머지 PR `690` 건 전수 대조에서 처방이 결과를 바꾸는 건 `3` 건뿐이고, **셋 다 저자가 「내 지시가 아니다」로 인용한 것**이었다 — 현행 파서가 그 신호를 거꾸로 읽고 있었으므로 델타는 회귀가 아니라 정정이다. (2026-09-03)

> **실패 방향을 뒤집는 것이 이득이다** — 종전에는 실패가 「무관한 이슈의 조용한 변경」이었고, 이제는 「라벨 미전이·auto-close 미발동」이라 기존 수동 폴백이 받는다. 같은 전처리를 이슈 close 파서에도 함께 넣은 이유는, 한쪽만 고치면 **라벨은 안 붙는데 이슈는 닫히는** 신규 비대칭이 생기기 때문이다.
