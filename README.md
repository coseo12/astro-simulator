...
**v0.76.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.51~0.75 가 **검증 인프라 강화**였고, v0.76.0 에서 **제품 표면이 v0.50.0 이후 처음 움직였다** — 지구가 절차적 fbm 대륙에서 **실제 Natural Earth 육지 마스크**로 바뀌어 아프리카·아메리카·유라시아가 알아볼 수 있는 형상으로 보인다 (#1119). 함께 들어간 인프라는 **머지 충돌 마커 전수 가드**(포맷터가 만드는 변형형까지 — #1103) · **공유 `gh` 인증 덮어쓰기 차단**(#1099) · **검증 강도 게이트**(#1117) (2026-08-18)
