...
**v0.87.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). 지구 가장자리에 **대기 산란 rim** 이 생겼고(#1202), **일시정지 중에도 LOD 가 카메라를 따라온다**(#1205). v0.86.0 이 **처방이 자기 규범을 어길 때**였다면 v0.87.0 은 **증상 셋이 한 생성원이었을 때**다. 대기 rim 을 검증하다 광원 재앵커 결함이 나왔고(#1204), 그것을 고치다 LOD 동결의 실체가 드러났다 — 셋 다 **카메라 종속 판정이 시간 위상에 얹혀 있던** 하나의 원인이었다. 앞의 둘은 `setTier` 에 수식을 복제한 **점 패치**였고(ring-anchor #782/#785 에 이어 두 번째·세 번째), 이번에 세 번째를 만드는 대신 **프레임 위상을 갈라** 생성원을 없앴다. (2026-09-08)
