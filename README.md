...
**v0.79.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). v0.78.0 이 **검증 인프라**였다면(가드가 묻는 질문이 계약보다 좁던 세 건 — #1134 / #1096 / #1127) v0.79.0 은 **운영 규약의 도달 범위**다. 두 건이 한 축이다 — 매 세션 밟는 마찰인데 규약이 읽히는 자리에 없었다: 백틱 코드 스팬이 셸에서 **명령으로 실행되는데** 종료 코드 · stderr · U+FFFD **세 축이 전부 초록**이라 CRITICAL #4 를 통과하던 것(#1147) / 그 절들이 신설돼도 `CLAUDE.md` 는 여전히 §1~§4 만 열거해 `developer.md` 를 로드하지 않는 경로에서는 **도달 자체가 불가**하던 것(#1149). 후자는 계수 `4` 를 `10` 으로 **고치지 않고 없앴다** — 하드코딩된 수 자체가 drift 생성원이라서다. (2026-08-24)

> **박제된 것과 도달하는 것은 다르다** — §7·§8 은 `developer.md` 만 가리키고 있었고 §9·§10 은 신설 시점에 가리키는 표면이 `0` 이었다. 규약은 읽히는 자리에 있을 때만 규약이다.
