...
**v0.86.0** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). 지구 바다에 깊이 색 그라데이션이 생겼다 — 해안은 밝은 청록, 원양 중심부는 짙은 파랑(#1197). v0.85.0 이 **정본이 자기 규약을 어길 때**였다면 v0.86.0 은 **처방이 자기 규범을 어길 때**다. 「판별력의 정본은 서술이 아니라 변이 주입 후 게이트 재실행」이라고 ADR 에 적던 바로 그 사이클에, **실행 없이 발행한 가드 술어가 두 번 반증됐다** — 겨냥한 변이도, 스스로 선언한 실패 모드도 잡지 못해 결국 삭제했다. (2026-09-07)

> **판별식이 「값의 형태」를 보면 형태를 보존하는 실패에 눈이 먼다** — uniform 바인딩이 사라지면 감쇠 계수가 `(0,0,0)` 으로 남아 **채널 공통 스칼라 배**가 되고, 색 순서는 그 배수 아래 불변이라 색으로는 원리적으로 도달할 수 없다. 채택한 술어는 대신 **바인딩이 렌더 결과에 기여하는 양**을 잰다 — 같은 실행 안에 계수를 `0` 으로 고정한 프레임을 하나 더 두고 갭의 상승을 요구하면, 바인딩이 없을 때 두 프레임이 같은 상태가 되어 상승이 `0` 으로 붕괴한다.
