...
**v0.89.3** — 로드맵 v3 (Incremental Body-by-Body Build) 완주 + 트랙 A/B (몰입·온보딩). 지구 **밤면 육지에 도시 불빛**이 켜졌다(#1226) — 별도 mesh 없이 표면 셰이더 안에서 합성해 `?nightlights=off` 가 픽셀 단위 no-op 이고, 불빛이 어디 몰릴지는 이미 있던 대륙 노이즈를 **임계로** 재사용해 정한다(추가 hash `0`). 그런데 이 릴리스의 이야기는 불빛이 아니라 **처방이 두 번 폐기된 것**이다. 신규 가드의 변이 하나가 4회 연속 「측정 불가」로 막혔고, 원인은 결함이 아니라 **MSAA 4x resolve** 가 삼각형 경계 한 픽셀에서 술어의 전제를 깬 것이었다 — `antialias:false` 개입으로 기전을 확정했다. 첫 처방은 임계 `3` 을 「기존 관례의 역방향 적용」으로 정당화했는데 외부 교차검증이 **「의사 형식주의」**로 기각했다(`1` 은 표준편차가 아니라 단일 knife-edge 아티팩트다) — **형식을 빌리는 것은 근거가 아니다.** 두 번째 폐기는 더 컸다: 그 술어가 `NI_land` 안만 보고 있어, 낮면에만 불빛을 새게 하는 결함에서 **판정량이 건강판과 완전히 같았다**. 확정안은 오히려 단순해졌다 — 기준 프레임을 하나 지우자(`NA` 불도입) 「전제의 전제」·「기준과 대상이 같은 결함을 공유하는 사각」·「진단을 술어로 올릴지」가 **함께 소멸**했고 새 임계는 `0` 개다. 그 교체로 **구름이 불빛을 가리는지 재는 게이트의 변이 검출력이 처음 확인**됐다. 후속 패치 v0.89.1 은 **사용자가 본 결함 둘을 닫았다** — tier 경계 줌은 「경계를 넘는 프레임이 구 단위 radius 로 렌더된다」가 원인이라 tween 을 지우고 즉시 대입으로 바꿨고(#1232), 장면 부팅 20 s 타임아웃은 **원인 후보 둘을 실패 표본이 차례로 기각**한 끝에 `navigator.gpu.requestAdapter()` 가 **영영 settle 하지 않는 것**으로 확정됐다(#1234). 뒤엣것은 마지막 검증 회차에서 **자연 발생했고 상한이 흡수했다** — `11.7 s`·`13.3 s` 에 부팅이 끝났다. 고친 것은 미결 자체가 아니라 그것이 부팅 전체를 잡는 구조다. 이어진 v0.89.2 는 **가드가 스스로를 재는 회차**였다 — bench 판정선이 **측정량과 단위가 달라** 8 PR 연속 발화하던 것을 상대 판정으로 바꿨고(#1209), 머지된 브랜치 101 개가 쌓인 이유가 **삭제를 「집행 주체 없는 별도 단계」로 밀어낸 우리 절차**였음을 확인해 주간 workflow 로 이관했다(#1247). 파일로만 있던 browser-verify 2종을 CI 에 걸었는데(#1207), 걸고 보니 그중 하나에 **단언 없이 통과하는 경로가 둘** 있었다 — 전건 붕괴보다 **부분 붕괴가 더 나빴다**(매트릭스 절반이 사라진 채 정상 통과). 이번 v0.89.3 은 **선언과 실제가 갈라져 있던 자리**를 닫았다 — browser-verify 34 곳에서 `waitForFunction` 의 옵션 객체가 `arg` 자리에 있어 **선언한 타임아웃이 한 번도 적용된 적이 없었고**, 예외도 경고도 없어 실제 상한은 줄곧 playwright 기본 30 s 였다(#1256). 그런데 이슈가 적어 둔 처방(「선언값을 그대로 적용하라」)이 **실측에서 뒤집혔다** — 로컬에서는 넉넉해 보이는 여유가 CI 에서는 3 배 좁았고(핸들 대기 로컬 최대 `2,736 ms` ↔ CI 정상 가드 최대 `8,271 ms`), 가장 빠듯한 선언을 그대로 켰다면 그 자리가 flake 가 됐을 것이다. 그래서 새 임계를 발명하는 대신 이미 저장소에 있던 값으로 정렬했다. ⚠️ 그리고 **그 결함을 없애려고 만든 가드가 같은 결함을 재생산했다** — 헤더가 옵션 키 목록을 「나머지 전부」라고 단정했는데 거짓이었다(`signal` 이 빠졌다). 차단 사유는 미검출이 아니라 **같은 헤더의 다른 경계들은 좁음을 선언하는데 이 축만 반대로 적었다**는 쪽이었다. 나머지 둘은 가드가 **재려던 상황이 성립하지 않는 경우**를 통과로도 회귀로도 읽지 않게 갈랐다 — `379-lod` 의 단언 없이 통과하던 경로 5 건(#1250)과, 버려지는 마운트 체인을 붙들고 있어 판별력이 `0` 이던 glow-marker 축 6(#1239)이다. (2026-09-24)

> **외부 판정은 기전과 자리를 분리해 받는다** — 외부 교차검증의 Critical 하나는 자리는 옳고 기전은 틀렸다. 자전축 기울기로 반례를 구성했는데 가드 프레임은 기울기가 `0°` 라 그 경로가 발화조차 못 했다. 그렇다고 기각으로 끝낼 수도 없었다 — 반대편의 「구조적으로 불가」라는 전칭 주장도 거짓이었고(여는 축은 **카메라 고도**였다), 양쪽이 틀린 자리에서 실측이 제3의 답을 냈다. ⚠️ 그 전칭 주장을 쓴 것은 메인이다.
