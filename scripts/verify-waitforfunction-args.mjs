 ...
 *       처럼 **명시**해 둔다. 픽스처 **F21** 이 고정한다. 저장소 발생 `0` (전 hit 이 문자열·
 *       주석·`browser-verify-utils.test.mjs` 의 mock **정의부**이고 정의부는 F13 이 고정한다).
 *       구조분해 쪽은 점 앵커(`prevSignificantChar(...) !== '.'`)가 단독으로 거부한다 — 그
 *       게이트만 무력화하면 `exit 1` 로 뒤집힌다 (2026-09-23 실측 / PR
 *       [#1257](https://github.com/coseo12/astro-simulator/pull/1257) reviewer 독립 재현).
 *       ⚠️ 계산된 멤버 쪽은 **배제 이유를 적지 않는다.** 초판은 «F4(문자열 안 리터럴 오탐
 *       배제)와 같은 기전의 대가» 라고 적었고 같은 reviewer 가 게이트를 무력화해 반증했다.
 *       배제 *이유*를 단정하면 그 문장은 구현보다 먼저 낡는다 — 여기서 관측 가능한 사실은
 *       «검출하지 않는다» 뿐이고, 그것은 위 경계 선언과 F21 이 이미 고정한다.
 *
...
function runSelfTest() {
  ...
  //    둘 다 저장소 발생 0. exit 0 은 결함이 아니라 결정이다.
  //    ⚠️ 배제 «이유» 는 헤더 §범위 경계 (iv) 가 다룬다 — 계산된 멤버 쪽은 거기서도 적지
  //    않는다 (초판의 인과 단정이 PR #1257 reviewer 에게 반증됐다).
  {
