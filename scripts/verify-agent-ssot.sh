...
#   f) **B1′(선두 헤딩 마커 제거)가 넓힌 표면** — 헤딩 **텍스트 안**의 토큰이 이제
#      검사 대상이다. 실측된 신규 오탐 형태 **2종**: `## C# 언어` / `### F# …` 같은
#      언어명(경계 b 가 헤딩 안으로 확장된 것), 그리고 `## 제목## ` 처럼 **후행 공백이
#      붙은 ATX 유사 닫기**. 코드 스팬 안(`` ## `C# ` `` 형태)은 placeholder 치환으로
#      걸리지 않는다. tracked 245 md 전수 **신규 hit 0**.
#      ⚠️ 반면 **4칸 들여쓴 비펜스 코드블록**의 `x## comment` 류는 **신규가 아니다** —
#      마커 제거 전(헤딩 라인 통째 skip) 방식에서도 동일하게 걸렸다 (실측 확인).
...
#   ⚠️ 근거를 정정한다 — 초판 주석은 *"검출 이득 없이 오탐 표면만 넓힌다"* 라고 썼으나
#   **실측으로 거짓**이다. `≤3` 변형을 실제로 구현해 재면:
#       무손상 5파일      : 현행 `0` / `≤3` 변형 `0`  (오탐 증가 없음)
#       짝수-상쇄 픽스처  : 현행 `0` / `≤3` 변형 `1`  (**현행이 미탐, 변형이 검출**)
#   즉 이득은 `0` 이 아니라 **아래 「짝수 상쇄」 잔여 1클래스**다. 초판 주석은 같은 문단
#   안에서 *"미종료 검사가 대신 막는다"* ↔ *"짝수 상쇄되면 END 는 발화하지 않는다"* 로
#   **자기모순**이었다 — 미종료 검사가 막는 것은 **미종료** 스트레이뿐이다.
#
#   그럼에도 반려하는 이유는 **이득 × 대가의 비교**다:
#     - 이득 = 잔여 **1클래스** (들여쓴 스트레이 펜스가 뒤의 진짜 펜스로 짝수 상쇄되는
#       exotic 형태. 5파일 현재 발생 0건)
#     - 대가 = 정당한 리스트 내포 펜스 **3블록**(indent≥4 **6줄** —
#       `architect.md:124,126` / `developer.md:28,47` / `reviewer.md:43,46`)의 **구조적
#       오분류**. 그중 `developer.md:28` 은 **앵커 섹션의 JSON 블록 자체**다. 지금은 그
#       블록들 안에 글루형 문자열이 없어 오탐 0 이지만, 내용이 바뀌면 조용히 켜진다.
#     - 게다가 CommonMark 의 `≤3` 은 **컨테이닝 블록 상대**값이라 절대 비교는 애초에
#       부정확하다 — 정확히 하려면 리스트 컨텍스트 추적이 필요하고 이 가드에 과하다.
#   ⚠️ 따라서 **짝수 상쇄는 미해결 잔여로 남는다**(미종료 검사로 덮이지 않는다).
#      (2) 의 문자·길이 매칭이 범위를 줄일 뿐이다. 재발하면 이 계산을 다시 하라.
#
      ...
      if (ch == "`" || ch == "~") {
        ...
        if (n >= 3) {
          if (!fence) {
            ...
            # ⚠️ CommonMark: **백틱** 펜스의 info string 은 백틱을 포함할 수 없다.
            # 그래서 행 선두 `` ```json``` `` 같은 인라인 표기는 펜스가 아니라 **문단**이다.
            # 이 조건이 없으면 마크다운 문법을 서술하는 이 5파일이 false FAIL 한다 (#1125 R1).
            # 틸드 펜스의 info string 은 백틱을 포함할 수 있으므로 백틱 펜스에만 적용한다.
            if (ch != "`" || index(substr(s, n + 1), "`") == 0) {
              fence = 1; fchar = ch; flen = n; fline = FNR
              next
            }
            # info string 에 백틱이 있으면 펜스가 아니다 — 아래 글루 검사로 계속 흘린다
