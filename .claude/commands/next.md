...
5. **이상 상태 감지**:
   ...
   - 알 수 없는 라벨 → 경고 + 스킵
   - 머지된 PR 에 `stage:*` 잔존 → 제거 제안 (#1178 — 머지된 PR 은 파이프라인 밖이라 단계 라벨을 갖지 않는다. `.github/workflows/strip-stage-labels.yml` 이 자동 제거하므로 잔존은 배선 도입 이전 머지분이거나 workflow 미발동분이다. `stage:done` 여부는 판정 기준이 **아니다** — `stage:done` = "완료 — 사용자 머지 대기" 는 머지 **전** 상태)
