...
5. **이상 상태 감지**:
   ...
   - **`strip-stage-labels.yml` 도입 이후 머지된 PR** 에 `stage:*` 잔존 → **조사 대상** (#1178 / #1182). 원인이 **두 갈래**이므로 `gh run list --workflow=strip-stage-labels.yml` 로 먼저 가른다:
     - **run 이 `success` 가 아님** → workflow 미발동. run 이 **있으면**(`failure` · `skipped` · 진행 중 등) 그 로그를 조사하고, **아예 없으면** 조사할 로그가 없으므로 트리거 성립 여부부터 본다 — 그 머지가 `pull_request` `closed(merged==true)` 를 실제로 냈는지, **그 PR 의 merge ref** 에 workflow 파일이 있었는지 (`git ls-tree <merge-sha> .github/workflows/strip-stage-labels.yml`). ⚠️ **base 브랜치에 파일이 없다는 것은 미발화 근거가 못 된다** — #1179 는 머지 직전 base 에 파일이 없는 상태에서 발화했다. 어느 쪽이든 라벨은 수동 제거한다
     - **run `success`** → 머지 **이후** 부착된 라벨이다. 그 workflow 는 `types: [closed]` 만 청취하고 `labeled` 를 청취하지 않으므로 사후 부착분은 정상 발화에도 잔존한다. 이 경우 조사할 결함이 없으니 **라벨을 제거**하면 된다 — `gh api --method DELETE "repos/{owner}/{repo}/issues/<PR번호>/labels/<라벨명>"` — `{owner}`·`{repo}` 는 `gh` 가 치환하고 꺾쇠 둘은 **사람이 채운다**. 라벨명의 콜론은 인코딩 불필요. 판정은 **잔존한 그 라벨**의 `labeled` 이벤트 시각이 `mergedAt` 보다 뒤인지로 한다 (같은 PR 이 머지 전후로 라벨 이벤트를 둘 다 가질 수 있으므로 라벨명을 특정해서 본다)
