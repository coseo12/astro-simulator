...
- **이전 세션 좀비 카나리아 검증** (incident #440 / 2026-05-10) — QA 시작 직전 + 반환 직전 의무: `pgrep -af "next dev|next-server|cargo.*test|pnpm.*dev"` 검사 후 ETIME 본 세션 시작 이전 (예: 30분+) 프로세스 발견 시 `non_blocking_suggestions` 에 좀비 후보 박제 + 메인에 정리 권고. `spawned_bg_pids` 가 sub-agent 가 spawn 한 PID 만 추적하므로 **이전 세션 좀비는 추적 단위 외부** — 본 카나리아가 별도 가드. 좀비 발견 시 사용자 D-T2 EADDRINUSE 사고 (PR #437 incident) 재발 차단.
