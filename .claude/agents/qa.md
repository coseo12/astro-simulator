...
- **이전 세션 좀비 카나리아 검증** (다운스트림 가드 — astro-simulator incident #440 / 2026-05-10) — QA 시작 직전 + 반환 직전 의무: `ps -axww -o pid=,etime=,command= | grep -E "next dev|next-server|cargo .*test|pnpm.*dev" | grep -v grep` 검사 (가드 C hook 과 **동일 형태·동일 패턴** — pgrep 금지 근거는 #1054, 아래) 후 **ETIME 30분 이상** 프로세스 발견 시 `non_blocking_suggestions` 에 좀비 후보 박제 + 메인에 정리 권고. 임계값 30분은 qa/dev 사이클 1회 이상 경과 → 본 세션이 spawn 한 게 아닐 가능성 매우 높음. CLAUDE.md 가드 B + `.claude/hooks/session-start-zombie-check.sh` 가드 C 와 동일 임계값 (SSoT). `spawned_bg_pids` 가 sub-agent 가 spawn 한 PID 만 추적하므로 **이전 세션 좀비는 추적 단위 외부** — 본 카나리아가 별도 가드. 좀비 발견 시 사용자 D-T2 EADDRINUSE 사고 (PR #437 incident) 재발 차단. **`pgrep -af` 로 되돌리지 말 것** (#1054 실측) — macOS `pgrep` 의 `-a` 는 Linux procps 의 _"명령행 출력"_ 이 **아니라** _"조상 프로세스를 매칭 대상에 포함"_ 이라, 자기 셸을 좀비로 보고하면서 **명령행도 ETIME 도 출력하지 않는다** (위 30분 판정 자체가 불가). bracket (`de[v]`) 은 **패턴 문자열 자신만** 보호하므로 ① `.*` 가 bracket 앞뒤를 이어 무관한 토큰 (`< /dev/null`) 에 도달하거나 ② 같은 command line 의 형제 명령이 un-bracketed 리터럴 (`next dev`) 을 노출하면 무력화된다. `grep -v grep` 은 **명령행 전체**를 거르고, 패턴 리터럴을 실은 셸은 정의상 `grep` 파이프라인이라 항상 걸러진다.
