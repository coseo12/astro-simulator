...
4. **파괴적 리팩토링 체크리스트 (volt [#69](https://github.com/coseo12/volt/issues/69))** — 상수 제거·SSoT 이동·함수 폐기 같은 **파괴적 리팩토링** 이 포함된 PR 이면 추가 점검:
   - **(Grep) 저장소 전수 검색** — 제거·변경된 상수의 잔존을 전 저장소에서 검사. 위성 모듈 독립 선언 잔존 시 "은닉 상수 drift" (상대 비율 / 단위 / 스케일 drift 를 조용히 생성) 로 차단. **아래 5항을 모두 밟기 전에는 _"잔존 0"_ 을 선언하지 않는다** — PR [#987](https://github.com/coseo12/astro-simulator/pull/987) 에서 이 선언이 **2회 연속 반증**됐고, 두 번 다 grep 이 **에러 없이 조용히 빈 결과**를 냈다 ([#989](https://github.com/coseo12/astro-simulator/issues/989)):
     ...
     1. **`git grep -F` 정본 — `-E` 의 `\b` 는 이 환경에서 침묵 실패한다.** git 내장 POSIX ERE 엔진은 `\b` 를 지원하지 않고 **리터럴 `b` 로 해석**한다. 즉 못 찾는 데 그치지 않고 **정반대를 찾는다** — 격리 픽스처에서 `git grep -E '35k\b'` 는 `35kb` 를 매치하고 `35k` 를 놓쳤다. 에러도 exit code 도 없어 신호가 없고, 시스템 `grep -E` 에서는 정상 동작해 더 속기 쉽다. 5변형 실측은 [`exhaustive-grep-protocol.md`](../../docs/guides/exhaustive-grep-protocol.md) §1. 정규식이 꼭 필요하면 `-P` (PCRE) 를 쓰고 **`-F` 결과와 교차 확인**한다. 수가 갈리면 `-F` 를 믿는다
     2. **경로 무제한 — `<src dirs>` 로 좁히지 않는다.** `--include` 확장자 제한도 같은 함정이다 (확장자 없는 파일 / `.json` / `.txt` / `.yml` 이 빠진다). **상수의 호출처는 대개 `.github/` 에 있다** — #987 이 놓친 7번째 지점이 정확히 `.github/workflows/project-guards.yml:58` 이었고, 하필 그 가드의 **유일한 호출처**였다. 제외는 **검색 시점이 아니라 분류 시점 (4항) 에** 한다.
        - ⚠️ **경로와 직교하는 축 하나 더 — 파일 상태.** `git grep` 은 **tracked 파일만** 본다. 미staged 신규 파일은 **경로를 아무리 넓혀도** 누락되므로 `--untracked` 를 병행한다
        - ⚠️ **직교 축 하나 더 — 매체.** PR·이슈·코멘트 본문은 작업 트리 **밖**이라 `--untracked` 로도 안 나온다. 훑지 않았으면 **_"tracked 파일 기준 실잔존 0 (술어: `git grep` 모집단 — 외부 본문 제외)"_** 로 적는다. 조건부 확장 절차는 [`exhaustive-grep-protocol.md`](../../docs/guides/exhaustive-grep-protocol.md) §3
     3. **표기 변형 전수** — 같은 값이라도 표기가 갈린다. `35k` / `35 K` / `35,000` / `35_000` / `35000` **5종을 각각** 건다 — 이는 **숫자 임계의 최소 집합**이지 닫힌 열거가 아니다. 색상 코드·경로·식별자는 파생 규칙이 달라 별도 변형을 세운다 (`-e` 반복 + `-i` 로 대소문자 흡수). 한 변형만 걸면 나머지가 침묵한다. **서술** 변형 축도 있다 — 대상을 **가리키는 낱말** 외에 그 동작을 **주장하는 술어** (`실발동` / `미발동` / `발동하지 않` / `동형`) 를 최소 1개 건다. **낱말 그물은 주장 문장을 통과시킨다** (실측: 프로토콜 §1.3)
     4. **hit 5분류 후 선언** — 전 hit 을 5계급 표로 코멘트에 첨부한다. **먼저 아래 두 규칙을 적용한 뒤** 순서대로 판정한다 — **첫 YES 에서 멈춘다**.

        - **(가) 위치가 아니라 기능으로 판정한다** — 규약 절 **안**의 rev 앵커 측정 기록도 **이력 기록**이다. 이 규칙을 리스트 **뒤에** 두면 이미 1항에서 멈춘 뒤라 늦는다.
        - **(나) 한 블록의 hit 을 한 계급으로 뭉쳐 세지 않는다** — 스키마 리터럴과 그 **설명 산문**은 별개 hit 이다.

        1. **그 문장이** 지금 행동을 지시하는가 (명령형 / 현재형 규범 / 계약 스키마) → **활성 선언** (어긋나면 **stale** = 정정 대상, 맞으면 정합). ⚠️ *"지시문 안에 있다"* 가 아니라 **그 값이 지시의 일부인가**를 묻는다
        2. 그 시점의 사실 서술인가 (측정 / 결정 / **구 값·구 명령의 반례 인용**) → **이력 기록** (소급 편집 = **기록 위조**)
        3. 구 값을 **의도적으로** 참조해 재도입을 감지하는가 → **역참조 회귀 가드** (예: `verify-claudemd-size.mjs` 의 _"상수가 35,000 으로 되돌아가면 이 케이스가 FAIL 한다"_)
        4. 규약 **자신의** 예시·리터럴인가 (본 §4 의 `35k`) → **규약 본문 자체**
        5. 전부 아니오 → **무관** (`-F` 과매칭: `+0.35 KB` / `535 km`)

        **오판 피해는 계급마다 다르다** — 3·4 를 1 로 오판한 *"정정"* 은 **가드 삭제**·**자기 참조 붕괴**, 2 를 1 로는 **기록 위조**로 **되돌릴 수 없다**. 1 을 2~5 로 오판하면 stale 이 살아남을 뿐이다. 그래서 1 항이 애매하면 멈추지 말고 **2~4 항을 끝까지 밟는다** (첫 YES 정지는 1 항이 **명확할 때만**).

        > **계급은 5로 고정한다.** #995·#999 의 오분류는 계급 부족이 아니라 **정의 오적용**이었다 (근거: [`exhaustive-grep-protocol.md`](../../docs/guides/exhaustive-grep-protocol.md) §5). 새 hit 이 어느 계급에도 안 맞으면 **계급 신설 전에 결정 절차를 다시 밟는다.** 사본(`claudemd-governance.md` / `tier-proportion.test.ts`)은 계급 **집합**만 인용한다 — 판정 **순서**는 본 항에서만 규범이라 사본의 나열 순서는 drift 가 아니다.
   ...
   - **(Comment-Only) 주석 전용 변경 주장 검증** — _"주석만 바뀌었다"_ 는 **`grep -vE` 단독 판정을 받지 않는다** (`  * b;` 곱셈 연속행을 주석으로 오인). 정본은 **`tsc --removeComments` emit 후 `cmp`** + **시맨틱 주석 패턴 0건** 병행 (목록·절차: [`exhaustive-grep-protocol.md`](../../docs/guides/exhaustive-grep-protocol.md))
   - **(Dead Reference) 주석 SSoT 참조 확인** — `// SSoT: <파일>` / JSDoc `@see` 등 주석 메타데이터가 **폐기된 파일을 가리키지 않는지** 확인 (dead reference). 참조 대상 **파일 제거**뿐 아니라 **참조된 절·표제의 개명**도 dead 로 만든다 — 파일이 남아 있어 `verify-docs-links.mjs` 로 안 잡힌다 (실례: [`exhaustive-grep-protocol.md`](../../docs/guides/exhaustive-grep-protocol.md) §6)
