...
4. **파괴적 리팩토링 체크리스트 (volt [#69](https://github.com/coseo12/volt/issues/69))** — 상수 제거·SSoT 이동·함수 폐기 같은 **파괴적 리팩토링** 이 포함된 PR 이면 추가 점검:
   - **(Grep) 저장소 전수 검색** — 제거·변경된 상수의 잔존을 전 저장소에서 검사. 위성 모듈 독립 선언 잔존 시 "은닉 상수 drift" (상대 비율 / 단위 / 스케일 drift 를 조용히 생성) 로 차단. **아래 4항을 모두 밟기 전에는 _"잔존 0"_ 을 선언하지 않는다** — PR [#987](https://github.com/coseo12/astro-simulator/pull/987) 에서 이 선언이 **2회 연속 반증**됐고, 두 번 다 grep 이 **에러 없이 조용히 빈 결과**를 냈다 ([#989](https://github.com/coseo12/astro-simulator/issues/989)):
     ...
     2. **경로 무제한 — `<src dirs>` 로 좁히지 않는다.** `--include` 확장자 제한도 같은 함정이다 (확장자 없는 파일 / `.json` / `.txt` / `.yml` 이 빠진다). **상수의 호출처는 대개 `.github/` 에 있다** — #987 이 놓친 7번째 지점이 정확히 `.github/workflows/project-guards.yml:58` 이었고, 하필 그 가드의 **유일한 호출처**였다. 제외는 **검색 시점이 아니라 분류 시점 (4항) 에** 한다 ⚠️ **`git grep` 은 tracked 파일만 본다** — 아직 `git add` 되지 않은 신규 파일은 누락된다 (cross-validate agy 2026-08-09 관찰). 작업 트리에 미staged 신규 파일이 있으면 `--untracked` 를 병행한다.
     3. **표기 변형 전수** — 같은 값이라도 표기가 갈린다. `35k` / `35 K` / `35,000` / `35_000` / `35000` **5종을 각각** 건다 — 이는 **숫자 임계의 최소 집합**이지 닫힌 열거가 아니다. 색상 코드·경로·식별자는 파생 규칙이 달라 별도 변형을 세운다 (`-e` 반복 + `-i` 로 대소문자 흡수). 한 변형만 걸면 나머지가 침묵한다
     4. **hit 3분류 후 선언** — 전 hit 을 **활성 선언 / 이력 기록 / 무관** 으로 나눈 표를 코멘트에 첨부한다. **이력 기록은 보존이 옳다** — CHANGELOG entry·ADR 시점 결정은 그 시점의 사실이고 소급 편집은 기록 위조다 (현재형 오독이 우려되는 운영 문서는 삭제가 아니라 `(당시)` + 각주로 막는다). **무관**은 `-F` 과매칭의 정상 부산물이다 (`+0.35 KB` / `535 km`). 이 분류를 거쳐야 _"실잔존 0"_ 이 검증 가능한 주장이 된다
        - ⚠️ **3분류가 담지 못하는 2계급** (PR #994 리뷰 권고 1, 실측) — 오분류 시 실질 피해가 있다.
          - **역참조 회귀 가드** — 예: `verify-claudemd-size.mjs` 의 *"상수가 35,000 으로 되돌아가면 이 케이스가 FAIL 한다"*. **활성 선언으로 오판해 정정하면 회귀 가드가 삭제된다.** 구 값을 **의도적으로** 참조하는 코드는 별도 계급이다
          - **규약 본문 자체** — 정본 명령이 예시로 그 값을 포함한다 (본 §4 가 `35k` 를 리터럴로 담는 것처럼). 자기 참조라 정정 대상이 아니다
