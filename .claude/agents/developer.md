...
- **격리 worktree 에서 워크스페이스 패키지(`@astro-simulator/*`)를 해석하는 명령을 돌리려면 `pnpm build` 선행** (#960, #1062) — `pnpm install --frozen-lockfile` 은 `node_modules` 만 세우고 **빌드 산출물을 만들지 않는다**. 그래서 `@astro-simulator/*` 를 해석하는 명령이 전부 깨진다 (rev `38b6c8a` 실측 — `pnpm typecheck` exit 2 / `pnpm --filter web typecheck` exit 2 `error TS` **76 행** / `pnpm test:unit` (= `pnpm -r test`) exit 1 `Test Files 30 failed | 26 passed (56)` / `pnpm --filter web test` exit 1 / `pnpm --filter web build` exit 1). **결손 2축**: `TS2882` = `apps/web/next-env.d.ts` 부재 / `TS2307` = `packages/{shared,core}/dist` + `packages/physics-wasm/{pkg,pkg-bundler}` 부재. 루트 `pnpm build` (exit 0, 소요는 `install` 과 **같은 자릿수** — 실측 스냅샷은 §8 측정 조건) 가 두 축을 동시에 닫는다. CI `setup-and-build` composite 과 **같은 2 명령**이다.
  - ⚠️ **트리거를 `typecheck` 로 좁혀 읽지 말 것** — 본 규칙 초판이 그렇게 적었고, `test:unit` 만 돌린 에이전트가 *"테스트 30개 실패"* 를 자기 변경 탓으로 오진할 뻔했다 (#1062). `lint` / `format:check` / 정적 `verify:*` 는 **비의존 집합**이라 `install` 만으로 exit 0 이다 — 전수 매트릭스는 §8-1.
  - ⚠️ **`pnpm dev` 는 exit code 로 드러나지 않는다** — 서버는 `✓ Ready` 로 뜨고 `GET /` 만 **HTTP 500** 이다. 브라우저 검증에서 *"화면이 비었다"* 로 보여 앱 회귀로 오진하기 가장 쉽다.
  - ⚠️ `next-env.d.ts` 를 "표준 2줄" 로 손수 쓰지 말 것 — 그 내용은 `next dev` / `next typegen` 사이에서 **표류한다** (ADR [`20260814-960`](../../docs/decisions/20260814-960-worktree-typecheck-recipe.md) §B-1).
  - 절차 SSoT: [`docs/ops/operational-friction.md`](../../docs/ops/operational-friction.md) §8 / §8-1.
