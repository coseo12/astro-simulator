...
- 머지 시 `--delete-branch` 를 사용하지 않는다 — 멀티 워크스페이스 (Conductor worktree) 브랜치 점유와 충돌 (운영 마찰 규약 §2). **원격 삭제는 주간 `branch-cleanup` workflow 가 집행하므로 머지 후 할 일이 없다** (#1247 — 수동 단계로 두었더니 머지된 브랜치 90 개가 잔존했다). `git push origin --delete <브랜치>` 는 즉시 지우고 싶을 때의 폴백이지 의무가 아니다.
