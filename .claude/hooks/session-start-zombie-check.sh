...
#   ⚠️ 경계는 **양쪽 다** 필요하다. 한쪽(앞)만 거는 `pnpm( |$).*( dev|dev )` 형태는 ` development` /
#      ` dev-preview` 를 흘린다 (구조 차이 — zombie-process-guards.md §10).
#      ⚠️ 2026-08-16 정정: 종전 이 자리의 *"이 코퍼스에서는 동률"* 은 **거짓**이다 (PR #1095
#      reviewer). 그 형태는 `…/bin/pnpm dev`(EOL)를 놓쳐 pnpm 축 **거짓 음성 1** 이다 —
#      `pnpm( |$)` 가 공백을 소비하면 뒤의 `( dev|dev )` 가 좌·우 공백을 **둘 다** 못 찾는다.
#      #1066 에서 승계된 오기이며 **기각 결론 자체는 유효**하다 (구조 근거가 결과 근거로 강화됨).
...
#     (3) `( +…)* +` — 구분자를 공백 **1개**로 못박아 `pnpm  dev`(공백 2개)를 놓쳤다
#         (#1066 채택안 hit 1 → #1086 채택안 hit 0).
...
#   거짓 양성 **#440~ 16 → #1086 채택안 0**, 거짓 음성 0 → 0.
#   ⚠️ 종전 이 자리는 `구 11` 이었다 — 코퍼스가 22 → 29 건으로 늘어난 뒤에도 **옛 22건 값이
#      그대로 남은 부분 재측정 잔재**다 (PR #1095 reviewer / ADR 20260808-983 §(i) 위반).
#      같은 커밋의 zombie-process-guards.md §10 표는 이미 `16` 이었고, 두 파일이 같은 술어에서
#      갈렸다. 최종값은 위 29건 코퍼스에서 **일괄 1회 도출**한 것이다.
#   ⚠️ "검출 능력 손실 0" 은 **본 코퍼스 한정**이며
